namespace AGE
{
    ...
    public class ActionHelper : MonoBehaviour
    {
        ...
        public void BeginAction(string _actionHelperName)
        {
            ...
            if (this.actionHelperMap.TryGetValue(_actionHelperName, out storage) && storage.waitForEvents)
            {
                ...
                if (component != null)
                {
                    ...
                    {
                        bool flag2 = false;
                        for (int i = 0; i < component.get_layerCount(); i++)
                        {
                            if (component.GetCurrentAnimatorStateInfo(i).get_nameHash() == storage.GetDetectStatePathHash())
                            {
                                flag2 = true;
                                break;
                            }
                            AnimatorStateInfo nextAnimatorStateInfo = component.GetNextAnimatorStateInfo(i);
                            if (component.IsInTransition(i) && (nextAnimatorStateInfo.get_nameHash() == storage.GetDetectStatePathHash()))
                            {
                                flag2 = true;
                                break;
                            }
        ...
        {
            ActionHelperStorage storage = null;
            if ((this.actionHelperMap.TryGetValue(_actionHelperName, out storage) && storage.waitForEvents) && (base.get_gameObject().GetComponent<Animator>() != null))
            {
        ...
        private void Start()
        {
            this.animator = base.get_gameObject().GetComponent<Animator>();
            this.actionHelperMap.Clear();
            foreach (ActionHelperStorage storage in this.actionHelpers)
            {
                this.actionHelperMap.Add(storage.helperName, storage);
        ...
        private void Update()
        {
            if (this.animator != null)
            {
                foreach (ActionHelperStorage storage in this.actionHelpers)
                {
                    ...
                    {
                        bool flag = false;
                        for (int i = 0; i < this.animator.get_layerCount(); i++)
                        {
                            if (this.animator.GetCurrentAnimatorStateInfo(i).get_nameHash() == storage.GetDetectStatePathHash())
                            {
                                flag = true;
                                break;
                            }
                            AnimatorStateInfo nextAnimatorStateInfo = this.animator.GetNextAnimatorStateInfo(i);
                            if (this.animator.IsInTransition(i) && (nextAnimatorStateInfo.get_nameHash() == storage.GetDetectStatePathHash()))
                            {
                                flag = true;
                                break;
                            }
