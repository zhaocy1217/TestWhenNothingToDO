    ...
    public class ActionManager : MonoSingleton<ActionManager>
    {
        ...
        public void ForceStop()
        {
            if (MonoSingleton<ActionManager>.instance != null)
            {
                ...
                IEnumerator enumerator = base.get_transform().GetEnumerator();
                try
                {
                    while (enumerator.MoveNext())
                    {
                        DestroyGameObject(((Transform) enumerator.Current).get_gameObject());
                    }
                }
                finally
                {
                    ...
                    try
                    {
                        while (enumerator.MoveNext())
                        {
                            ...
                            if (current.Tag == "TemplateObject")
                            {
                                ...
                                item.name = current.Attribute("objectName");
                                item.id = int.Parse(current.Attribute("id"));
                                item.isTemp = bool.Parse(current.Attribute("isTemp"));
        ...
        private void Initialize()
        {
            ...
            {
                foreach (ActionHelper helper in Object.FindObjectsOfType<ActionHelper>())
                {
        ...
        private void LoadRefParamNode(Action result, SecurityElement paramNode)
        {
            ...
            switch (str)
            {
                ...
                case "vector3":
                {
                    float num5 = float.Parse(paramNode.Attribute("x"));
                    float num6 = float.Parse(paramNode.Attribute("y"));
                    float num7 = float.Parse(paramNode.Attribute("z"));
                ...
                case "quaternion":
                {
                    ...
                    float num15 = float.Parse(paramNode.Attribute("w"));
                    Quaternion quaternion = new Quaternion(num12, num13, num14, num15);
        ...
        private void SetEventField(Action action, BaseEvent _trackEvent, SecurityElement _fieldNode)
        {
            ...
            if (field != null)
            {
                ...
                switch (str)
                {
                    ...
                    case "vector3":
                    {
                        float num5 = float.Parse(_fieldNode.Attribute("x"));
                        float num6 = float.Parse(_fieldNode.Attribute("y"));
                        float num7 = float.Parse(_fieldNode.Attribute("z"));
                        Vector3 vector = new Vector3(num5, num6, num7);
                    ...
                    case "quaternion":
                    {
                        ...
                        float num15 = float.Parse(_fieldNode.Attribute("w"));
                        Quaternion quaternion = new Quaternion(num12, num13, num14, num15);
