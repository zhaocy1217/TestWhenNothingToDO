namespace AGE
{
    ...
    public class Action : PooledClassObject
    {
        ...
        public void AddTempObject(PlaySpeedAffectedType type, GameObject obj)
        {
            ...
            if (type == PlaySpeedAffectedType.ePSAT_Anim)
            {
                ...
                if (componentsInChildren != null)
                {
                    for (int j = 0; j < componentsInChildren.Length; j++)
                    {
                        ...
                        if (animation.get_playAutomatically() && (animation.get_clip() != null))
                        {
                            AnimationState state = animation.get_Item(animation.get_clip().get_name());
                            if (state != null)
                            {
                                state.set_speed(single);
                            }
                        }
                    }
                }
                ...
                if (animatorArray != null)
                {
                    ...
                    {
                        animatorArray[k].set_speed(single);
                    }
                }
            }
            else
            {
                ...
                if (systemArray != null)
                {
                    ...
                    {
                        systemArray[m].set_playbackSpeed(single);
                    }
                }
            }
        }
        ...
        public Track AddTrack(Type _eventType)
        {
            ...
            item.trackIndex = this.tracks.Count;
        ...
        public void Stop(bool bForce)
        {
            if (bForce)
            {
                ...
                {
                    PoolObjHandle<Action> action = new PoolObjHandle<Action>(this);
        ...
        private void Update()
        {
            ...
            {
                int num = ActionUtility.SecToMs(Time.get_deltaTime() * this.playSpeed.single) + this.time;
        ...
        public void UpdateLogic(int nDelta)
        {
            if ((ActionManager.Instance.frameMode && this.enabled) && !this.playSpeed.IsZero)
            {
                ...
                int num = 0;
                num = (this.playSpeed.nom != this.playSpeed.den) ? ((int) ((nDelta * this.playSpeed.nom) / this.playSpeed.den)) : nDelta;
                num += this.time;
                this.ForceUpdate(num);
            }
        }
