namespace AGE
{
    ...
    public class CurvlData
    {
        ...
        public Color lineColor = Color.get_red();
        ...
        public static void CalculateCtrlPoint(Vector3 formPoint, Vector3 prevPoint, Vector3 curnPoint, Vector3 lattPoint, out Vector3 ctrlPoint1, out Vector3 ctrlPoint2)
        {
            ...
            float num3 = vector6.get_magnitude();
            float num4 = vector7.get_magnitude();
            Vector3 vector8 = curnPoint - prevPoint;
            float num5 = vector8.get_magnitude() * 0.5f;
            if (num5 < num3)
            {
                sCtrlPtScale = num5 / num3;
            }
            if (num5 < num4)
            {
                num2 = num5 / num4;
        ...
        public void DrawCurvel(bool useUnifyInterp)
        {
            ...
            if (this.isInUse && !this.isHide)
            {
                ...
                {
                    Vector3 vector = this.curvePoints[i];
                    Vector3 vector2 = this.curvePoints[i + 1];
                    Debug.DrawLine(vector, vector2, this.lineColor);
                }
            }
        }
        ...
        private void InitData()
        {
            ...
            {
                string str = "CurvlRootObject_" + this.curvlID;
                this.curvlRootObj = GameObject.Find(str);
                if (this.curvlRootObj == null)
                {
                    ...
                    {
                        obj2 = new GameObject();
                        obj2.set_name(CurvlHelper.sCurvlHelperObjName);
                    }
                    this.curvlRootObj = new GameObject();
                    this.curvlRootObj.set_name(str);
                    this.curvlRootObj.get_transform().set_parent(obj2.get_transform());
                }
            }
        }
        ...
        public void RefreshDataToGfx()
        {
            ...
            while (num2 < this.nodeLst.Count)
            {
                ...
                if (num2 >= count)
                {
                    ...
                    {
                        for (int j = 0; j < curvlRootObj.get_transform().get_childCount(); j++)
                        {
                            item = curvlRootObj.get_transform().GetChild(j).get_gameObject();
                            char[] separator = new char[] { '_' };
                            string[] strArray = item.get_name().Split(separator);
                            int result = -1;
                    ...
                    if (item == null)
                    {
                        ...
                        item.set_name(str);
                        item.get_transform().set_parent(this.curvlRootObj.get_transform());
                ...
                item.SetActive(true);
                transform.set_position(node.pos);
                transform.set_rotation(node.rot);
                num2++;
            }
        ...
        public void ReverseNodes()
        {
            if ((this.nodeLst != null) && (this.nodeLst.Count > 1))
            {
                ...
                {
                    if (!this.transNodeObjs[i].get_activeInHierarchy())
                    {
                        ActionManager.DestroyGameObject(this.transNodeObjs[i]);
        ...
        public void SetCurvlID(int id)
        {
            ...
            if (this.curvlRootObj != null)
            {
                this.curvlRootObj.set_name("CurvlRootObject_" + this.curvlID);
            }
        }
            ...
            if ((this.nodeLst != null) && (this.nodeLst.Count != 0))
            {
                ...
                if (this.transNodeObjs != null)
                {
                    for (int i = 0; i < this.nodeLst.Count; i++)
                    {
                        ...
                        Transform transform = this.transNodeObjs[i].get_transform();
                        node.pos = transform.get_position();
                        node.rot = transform.get_rotation();
                        node.scl = transform.get_localScale();
                    }
                }
            }
        }
