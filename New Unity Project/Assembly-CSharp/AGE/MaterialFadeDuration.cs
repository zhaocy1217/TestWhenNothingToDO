        ...
        public override void Leave(Action _action, Track _track)
        {
            if (this.materials != null)
            {
                ...
                for (int i = 0; i < this.materials.Count; i++)
                {
                    ...
                    if (this.FadeIn)
                    {
                        Shader fadeShader = FadeMaterialUtility.GetFadeShader(material.get_shader(), false);
                        if ((fadeShader != null) && (fadeShader != material.get_shader()))
                        {
                            material.set_shader(fadeShader);
                        }
                    }
                }
                this.materials = null;
