...
public class TdirMgr : MonoSingleton<TdirMgr>
{
    ...
    private void CheckPreEnable()
    {
        for (int i = 0; i < this.mTreeNodes.Count; i++)
        {
            object param = null;
            TdirTreeNode node = this.mTreeNodes[i];
            if (this.ParseNodeAppAttr(node.staticInfo.appAttr, TdirNodeAttrType.PreEnable, ref param) && !string.IsNullOrEmpty((string) param))
            {
                this.preVersion = (string) param;
    ...
    public VerUpdateInfo CheckPreviousVersionUpdate()
    {
        ...
        if (this.isPreEnabled)
        {
            ...
            string[] strArray = GameFramework.AppVersion.Split(separator);
            int result = 0;
            ...
            int num2 = 0;
            int.TryParse(this.preVersion, out num2);
            if (this.SelectedTdir.isPreSvr)
            {
                if (result == num2)
                {
                    info.svrInfo = LoginSvrInfo.Login_valid;
                    info.svrType = enIIPSServerType.PreviousOfficial;
                    return info;
                }
                if (result < num2)
                {
                    info.svrInfo = LoginSvrInfo.Login_needUpdate;
                    info.svrType = enIIPSServerType.PreviousOfficial;
                    return info;
                }
                info.svrInfo = LoginSvrInfo.Login_invalid;
                info.svrType = enIIPSServerType.PreviousOfficial;
                return info;
            }
            if (result >= num2)
            {
                info.svrInfo = LoginSvrInfo.Login_invalid;
                info.svrType = enIIPSServerType.Official;
            }
            info.svrInfo = LoginSvrInfo.Login_valid;
            info.svrType = enIIPSServerType.Official;
            return info;
    ...
    public void ConnectLobby()
    {
        if (this.m_connectIndex < 0)
        {
            ...
            if ((num == 1) && !string.IsNullOrEmpty(SystemInfo.get_deviceUniqueIdentifier()))
            {
            ...
            {
                this.m_connectIndex = Random.Range(0, 0x2710);
            }
        ...
        for (int i = 0; i < strArray.Length; i++)
        {
            ...
            for (int k = 1; k < strArray2.Length; k++)
            {
                ...
                else
                {
                    ...
                    info2.ip = strArray2[0];
                    info2.port = strArray2[k];
    ...
    {
        enIIPSServerType official = enIIPSServerType.Official;
        if (this.isPreEnabled)
        {
            char[] separator = new char[] { '.' };
            string[] strArray = GameFramework.AppVersion.Split(separator);
            if ((strArray.Length > 2) && this.preVersion.Equals(strArray[1]))
            {
                official = enIIPSServerType.PreviousOfficial;
            }
        }
        return official;
    }
            ...
            for (int i = 0; i < strArray.Length; i++)
            {
                ...
                if ((((strArray2 != null) && (strArray2.Length == 2)) && (!string.IsNullOrEmpty(strArray2[0]) && !string.IsNullOrEmpty(strArray2[1]))) && (strArray2[0].ToLower() == str))
                {
                    switch (attrType)
                    {
                        ...
                        case TdirNodeAttrType.msdkLogTypeFilter:
                        case TdirNodeAttrType.msdkLogLevelFilter:
                            goto Label_0233;
    ...
    {
        <QueryTdirAsync>c__Iterator2F iteratorf = new <QueryTdirAsync>c__Iterator2F();
        iteratorf.info = info;
        iteratorf.failCallBack = failCallBack;
        iteratorf.reasync = reasync;
        iteratorf.successCallBack = successCallBack;
        iteratorf.<$>info = info;
        iteratorf.<$>failCallBack = failCallBack;
        iteratorf.<$>reasync = reasync;
        iteratorf.<$>successCallBack = successCallBack;
    ...
    private void SetGlobalConfig()
    {
        if ((this.mTreeNodes != null) && (this.mTreeNodes.Count != 0))
        {
            ...
            {
                object param = new object();
                if (this.GetTreeNodeAttr(this.mTreeNodes[i], TdirNodeAttrType.enableTSS, ref param))
                {
    ...
    public void TdirAsync(ApolloAccountInfo info, Action successCallBack, Action failCallBack, bool reasync)
    {
        ...
        this.mSyncTime = 0;
        this.SetIpAndPort();
        this.m_GetTdirTime = Time.get_time();
    ...
    [CompilerGenerated]
    private sealed class <QueryTdirAsync>c__Iterator2F : IDisposable, IEnumerator, IEnumerator<object>
    {
        ...
        public bool MoveNext()
        {
            ...
            {
                this.<time>__1 += Time.get_deltaTime();
        ...
        Label_03BF:
            if (this.<>f__this.mAsyncFinished != AsyncStatu.AsyncSuccess)
            {
                ...
                {
                    this.<>f__this.m_GetTdirTime = Time.get_time();
                    MonoSingleton<CTongCaiSys>.instance.isCanUseTongCai = false;
                ...
                float num2 = Time.get_time() - this.<>f__this.m_GetTdirTime;
            ...
            else
            {
                ...
                try
                {
                    if (!File.Exists(Application.get_persistentDataPath() + "/ServerListCfg.xml"))
                    {
                ...
                float num3 = Time.get_time() - this.<>f__this.m_GetTdirTime;
                ...
                Singleton<CLoginSystem>.GetInstance().m_fLoginBeginTime = Time.get_time();
            }
            this.$PC = -1;
        Label_0773:
            return false;
