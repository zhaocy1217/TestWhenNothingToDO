...
public class NoticeSys : MonoSingleton<NoticeSys>
{
        ...
        else if ((this.m_bGoto && this.m_bShow) && (this.m_btnUrl != string.Empty))
        {
            ...
            switch (btnDoSth)
            {
                case BTN_DOSOMTHING.BTN_DOSOMTHING_URL:
                    CUICommonSystem.OpenUrl(btnUrl, true, 0);
                    break;
    ...
    private void OnCloseIDIPForm(CUIEvent uiEvent)
    {
        ...
        this.m_bShow = false;
        if (((this.m_ImageContent != null) && (this.m_ImageContent.get_sprite() != null)) && this.m_bLoadImage)
        {
    ...
    public void PreLoadImage()
    {
        ...
        for (int i = 0; i < noticeUrl.Count; i++)
        {
            ...
            {
                <>f__am$cache13 = delegate (Texture2D text2) {
                };
            }
            base.StartCoroutine(MonoSingleton<IDIPSys>.GetInstance().DownloadImage(noticeUrl[i], <>f__am$cache13, 0));
        }
        ...
        for (int j = 0; j < list2.Count; j++)
        {
            ...
            {
                <>f__am$cache14 = delegate (Texture2D text2) {
                };
            }
            base.StartCoroutine(MonoSingleton<IDIPSys>.GetInstance().DownloadImage(list2[j], <>f__am$cache14, 0));
        }
    }
    ...
    private void ProcessButton(uint nLevel, bool isGoto)
    {
        if (this.m_Form != null)
        {
            Transform transform = this.m_Form.get_gameObject().get_transform().Find("Panel/Image/Button_Complte");
            if (nLevel <= 5)
            {
    ...
    private void ProcessShowNoticeWindown(ApolloNoticeData noticeData)
    {
        ...
        if (msgType == APOLLO_NOTICETYPE.APO_NOTICETYPE_ALERT)
        {
            ...
            Transform transform = this.m_Form.get_gameObject().get_transform().Find("Panel/BtnGroup/Button_Complte");
            ...
            this.m_bShow = true;
            this.m_TextContent = Utility.GetComponetInChild<Text>(this.m_Form.get_gameObject(), "Panel/ScrollRect/Content/Text");
            this.m_ImageContent = Utility.GetComponetInChild<Image>(this.m_Form.get_gameObject(), "Panel/Image");
            this.m_ImageDefault = Utility.GetComponetInChild<Image>(this.m_Form.get_gameObject(), "Panel/ImageDefalut");
            this.m_Title = Utility.GetComponetInChild<Text>(this.m_Form.get_gameObject(), "Panel/GameObject/Title/ContentTitle");
            this.m_TitleBoard = this.m_Form.get_gameObject().get_transform().Find("Panel/GameObject/Title").get_gameObject();
            ...
            {
                this.m_ImageDefault.get_gameObject().CustomSetActive(false);
            }
            ...
            switch (contentType)
            {
                ...
                case APOLLO_NOTICE_CONTENTTYPE.APO_SCONTENTTYPE_TEXT:
                {
                    ...
                    if (component != null)
                    {
                        component.set_sizeDelta(new Vector2(0f, this.m_TextContent.get_preferredHeight() + 50f));
                    }
                    return;
                }
            }
            if ((contentType == APOLLO_NOTICE_CONTENTTYPE.APO_CONTENTTYPE_WEB) && (this.m_urlAction != null))
            {
                ...
                if (this.m_ImageDefault != null)
                {
                    this.m_ImageDefault.get_gameObject().CustomSetActive(true);
                }
                this.m_ImageContent.get_gameObject().CustomSetActive(false);
