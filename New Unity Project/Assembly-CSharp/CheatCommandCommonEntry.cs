...
internal class CheatCommandCommonEntry
{
    ...
    {
        Vector3 vector = Camera.get_main().get_gameObject().get_transform().get_localPosition();
        vector.Set(vector.x, vector.y, vector.z + 0.5f);
        Camera.get_main().get_gameObject().get_transform().set_localPosition(vector);
        return ("当前高度偏差" + vector.z);
    }
    ...
    public static string FramerateMinus5()
    {
        ...
        Application.set_targetFrameRate(GameFramework.c_renderFPS);
