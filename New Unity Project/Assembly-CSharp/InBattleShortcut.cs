...
public class InBattleShortcut
{
            ...
            if ((chatType == COM_INBATTLE_CHAT_TYPE.COM_INBATTLE_CHATTYPE_SIGNAL) && (Singleton<CBattleSystem>.instance.TheMinimapSys.CurMapType() == MinimapSys.EMapType.Mini))
            {
                ...
                for (int i = 0; i < allHeroes.Count; i++)
                {
                    ...
                    if ((root != null) && (root.TheActorMeta.ConfigId == herocfgID))
                    {
                        ...
                        {
                            Vector2 sreenLoc = currentCamera.WorldToScreenPoint(root.HudControl.GetSmallMapPointer_WorldPosition());
    ...
    public void OpenForm(CUIFormScript battleForm, CUIEvent uiEvent, bool bShow = true)
    {
        ...
        {
            this.m_CUIForm.get_gameObject().CustomSetActive(true);
            if (this.contentList != null)
            {
                this.contentList.SelectElement(-1, false);
            }
            GameObject obj2 = Utility.FindChild(this.m_CUIForm.get_gameObject(), "chatTools/node/InputChat_Buttons");
            if (obj2 != null)
            {
        ...
        else
        {
            ...
            if (this.m_CUIForm != null)
            {
                ...
                this.contentList = this.m_CUIForm.get_transform().Find("chatTools/node/ListView/List").GetComponent<CUIListScript>();
                DebugHelper.Assert(this.contentList != null, "InbattleMsgView contentList == null");
                if (this.contentList != null)
                {
                    GameObject obj3 = battleForm.get_transform().Find("panelTopRight/SignalPanel/Button_Chat").get_gameObject();
                    DebugHelper.Assert(obj3 != null, "InbattleMsgView btnObj == null");
                    if (obj3 != null)
                    {
                        obj3.CustomSetActive(true);
                    }
                    this.m_cdButton = new CDButton(obj3);
                    GameObject obj4 = Utility.FindChild(this.m_CUIForm.get_gameObject(), "chatTools/node/InputChat_Buttons");
                    if (obj4 != null)
                    {
                    ...
                    if (!bShow)
                    {
                        this.m_CUIForm.get_gameObject().CustomSetActive(false);
                    }
                    this.Refresh_List();
                }
            }
    ...
    {
        if (((this.m_cdButton != null) && (this.m_cdButton.startCooldownTimestamp == 0)) && ((index >= 0) && (index < Singleton<InBattleMsgMgr>.instance.inbatEntList.Count)))
        {
            if (Singleton<InBattleMsgMgr>.instance.inbatEntList.Count == 0)
            {
                Singleton<InBattleMsgMgr>.instance.BuildInBatEnt();
            }
            TabElement element = Singleton<InBattleMsgMgr>.instance.inbatEntList[index];
            if (element != null)
            {
    ...
    public void Show(bool bShow)
    {
        if (this.m_CUIForm != null)
        {
            this.m_CUIForm.get_gameObject().CustomSetActive(bShow);
        }
    }
