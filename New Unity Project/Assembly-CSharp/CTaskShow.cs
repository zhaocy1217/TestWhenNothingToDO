...
public class CTaskShow : MonoBehaviour
{
    ...
    private void Awake()
    {
        this.el_0 = base.get_gameObject().GetComponent<CanvasGroup>();
    }

    private void decrease()
    {
        this.el_0.set_alpha(this.el_0.get_alpha() - this.step);
    }
    ...
    public void Play(Type type, CUIFormScript formScript, uint newTaskid = 0)
    {
        ...
        {
            this.bFinish = false;
            this.el_0.set_alpha(1f);
        }
        else if (type == Type.Show)
        {
            this.el_0.set_alpha(0f);
        }
    ...
    public void PlayParticle(bool bShowEffect)
    {
        ...
        Singleton<CUIParticleSystem>.instance.AddParticle(parPath, 1.5f, base.get_gameObject(), this.formScript);
    ...
    public void Reset()
    {
        this.el_0.set_alpha(!this.bForward ? ((float) 0) : ((float) 1));
    }
