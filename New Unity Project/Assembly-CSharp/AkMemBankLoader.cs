...
public class AkMemBankLoader : MonoBehaviour
{
    ...
    {
        <LoadFile>c__Iterator3A iteratora = new <LoadFile>c__Iterator3A();
    ...
    [CompilerGenerated]
    private sealed class <LoadFile>c__Iterator3A : IDisposable, IEnumerator, IEnumerator<object>
    {
        ...
        public bool MoveNext()
        {
            ...
            switch (num)
            {
                ...
                case 1:
                    ...
                    try
                    {
                        this.<>f__this.ms_pinnedArray = GCHandle.Alloc(this.<>f__this.ms_www.get_bytes(), GCHandleType.Pinned);
                        this.<>f__this.ms_pInMemoryBankPtr = this.<>f__this.ms_pinnedArray.AddrOfPinnedObject();
                        this.<in_uInMemoryBankSize>__0 = (uint) this.<>f__this.ms_www.get_bytes().Length;
                        if ((this.<>f__this.ms_pInMemoryBankPtr.ToInt64() & 15L) != 0)
                        {
                            this.<alignedBytes>__1 = new byte[this.<>f__this.ms_www.get_bytes().Length + 0x10L];
                            ...
                            Array.Copy(this.<>f__this.ms_www.get_bytes(), 0, this.<alignedBytes>__1, this.<alignedOffset>__4, this.<>f__this.ms_www.get_bytes().Length);
