...
public class CPlayerHonorController : Singleton<CPlayerHonorController>
{
            ...
            if (honorInfo != null)
            {
                ...
                if ((dataByKey != null) && ((honorInfo.iHonorLevel >= 0) && (honorInfo.iHonorLevel <= dataByKey.astHonorLevel.Length)))
                {
                    ...
                    if (srcWidgetScript != null)
                    {
                        ...
                        if (widget != null)
                        {
                            this.SetHonorImage(widget.get_transform(), honorInfo);
                        }
                        if (obj3 != null)
                        {
                            this.SetHonorAssitImage(obj3.get_transform(), honorInfo, uiEvent.m_srcFormScript);
                        }
                        if (obj4 != null)
                        {
                            this.SetHonorPoint(obj4.get_transform(), honorInfo);
                        }
    ...
    private void OnPlayerInfoTabChange()
    {
        ...
        Singleton<CUIEventManager>.GetInstance().RemoveUIEventListener(enUIEventID.Player_Info_Honor_Select_Change, new CUIEventManager.OnUIEventHandler(this.OnHonorSelectChange));
        Singleton<CUIEventManager>.GetInstance().RemoveUIEventListener(enUIEventID.Player_Info_Honor_Chosen, new CUIEventManager.OnUIEventHandler(this.OnHonorChosen));
        Singleton<EventRouter>.GetInstance().RemoveEventHandler(EventID.PlayerInfoSystem_Tab_Change, new Action(this, (IntPtr) this.OnPlayerInfoTabChange));
    }
    ...
    public static void ReceiveHonorChosenRsp(CSPkg msg)
    {
        ...
        if (msg.stPkgData.stUseHonorRsp.iErrorCode != 0)
        {
            ...
            if (form != null)
            {
                ...
                honorInfo.iHonorID = msg.stPkgData.stUseHonorRsp.iHonorID;
    ...
    private void SetHonorImage(Transform imgTransform, COMDT_HONORINFO honorInfo)
    {
        ...
        imgTransform.GetComponent<Image>().SetSprite(CUIUtility.GetSpritePrefeb(honorImagePath, false, false), false);
    }
    ...
    public void UpdateSelectedHonor(CUIFormScript form, COMDT_HONORINFO honorInfo)
    {
        ...
        if (widget != null)
        {
            Transform transform = form.get_transform().Find("pnlBg/pnlBody/pnlHonorInfo/pnlContainer/SelectedHonor/Button");
            ...
            if (honorInfo == null)
            {
                ...
                if (btn != null)
                {
                    ...
                    CUICommonSystem.SetButtonName(transform.get_gameObject(), strName);
                }
                if (obj3 != null)
                {
                    this.SetHonorImage(obj3.get_transform(), honorInfo);
                }
                if (obj4 != null)
                {
                    this.SetHonorName(obj4.get_transform(), honorInfo);
                }
                if (obj5 != null)
                {
                    this.SetHonorPoint(obj5.get_transform(), honorInfo);
                }
                if (obj6 != null)
                {
                    this.SetHonorDesc(obj6.get_transform(), honorInfo);
                }
                if (!flag && (btn != null))
                {
                    CUICommonSystem.SetButtonEnable(btn, false, false, true);
                    CUICommonSystem.SetButtonName(transform.get_gameObject(), str3);
                }
            }
            else
            {
                ...
                else if ((honorInfo.iHonorID == selectedHonorId) && (honorInfo.iHonorID != 0))
                {
                    ...
                    {
                        CUICommonSystem.SetButtonEnable(btn, false, false, true);
                        CUICommonSystem.SetButtonName(transform.get_gameObject(), text);
                    }
                }
