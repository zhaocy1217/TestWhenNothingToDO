...
public class SMaterialEffect_Translucent : SMaterialEffect_Base
{
    ...
    private void SetTranslucent(ListView<Material> mats, bool b)
    {
        if (mats != null)
        {
            for (int i = 0; i < mats.Count; i++)
            {
                ...
                if (str2 != material.get_shader().get_name())
                {
                    material.set_shader(Shader.Find(str2));
                }
            }
        }
    }
    ...
    public override bool Update()
    {
        ...
        {
            float factor = 1f;
            if (SMaterialEffect_Base.UpdateFadeState(out factor, ref this.fadeState, ref this.fadeTimer, this.fadeInterval, this.forceUpdateFactor))
            {
                this.SetAlpha(factor);
            }
