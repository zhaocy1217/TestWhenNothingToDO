...
public class InBattleInputChat
{
    ...
    public void Add(InBatChatEntity ent)
    {
        if (ent != null)
        {
            if (ent.type == InBatChatEntity.EType.ParseAllColor)
            {
    ...
    public InBatChatEntity ConstructColorFlagEnt(string content)
    {
        InBatChatEntity validChatEntity = this.GetValidChatEntity();
        validChatEntity.ullUid = 0L;
        string str = "[系统]";
        validChatEntity.type = InBatChatEntity.EType.ParseAllColor;
        object[] args = new object[] { InBatChatEntity.COLOR_SYSTEM, str, "</color>", content };
        validChatEntity.rawText = string.Format("{0}{1}{2} {3}", args);
