            ...
            if (((active != null) && (active.astarData != null)) && (active.astarData.recastGraph != null))
            {
                ...
                if (flag)
                {
                    if (active.astarDataArray == null)
                    {
                        ...
                        for (int k = 0; k < 3; k++)
                        {
                            AstarData owner = new AstarData();
                            owner.DataGroupIndex = k + 1;
                            owner.recastGraph = active.astarData.recastGraph.Clone(owner);
                            owner.userConnections = new UserConnection[0];
                            owner.graphs = new NavGraph[] { owner.recastGraph };
                            active.astarDataArray[k] = owner;
                        }
                    }
        ...
        if (this.handlers != null)
        {
            ...
            for (int j = 0; j < this.handlers.Length; j++)
            {
                ...
                for (int m = 0; m < all.Count; m++)
                {
                    ...
                    if ((item.campIndex != j) && item.get_enabled())
                    {
