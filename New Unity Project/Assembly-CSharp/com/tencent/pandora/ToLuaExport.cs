namespace com.tencent.pandora
{
    ...
    public static class ToLuaExport
    {
        ...
        public static string _C(string str)
        {
            ...
            if (str.Contains("."))
            {
                ...
                if ((str.Length > 12) && (str.Substring(0, 12) == "UnityEngine."))
                {
                    ...
                    else if (str2 == "UnityEngine.UI")
                    {
                        usingList.Add("UnityEngine.UI");
                    }
                    else if (str2 == "UnityEngine.EventSystems")
                    {
                ...
                {
                    if (str2 == "System.Collections")
                    {
                        usingList.Add(str2);
                    }
                    else if (str2 == "System.Collections.Generic")
                    {
                        usingList.Add(str2);
                    }
                    else if (str2 == "System")
        ...
        {
            for (Type type = ToLuaExport.type.BaseType; type != null; type = type.BaseType)
            {
        ...
        {
            <GenConstruct>c__AnonStorey8C storeyc = new <GenConstruct>c__AnonStorey8C();
            ...
            else
            {
                ...
                {
                    storeyc.list = new List<ConstructorInfo>();
                    for (int i = 0; i < constructors.Length; i++)
                    {
                        <GenConstruct>c__AnonStorey8B storeyb = new <GenConstruct>c__AnonStorey8B();
                        if (!HasDecimal(constructors[i].GetParameters()) && !IsObsolete(constructors[i]))
                        {
                            storeyb.r = constructors[i];
                            int index = storeyc.list.FindIndex(new Predicate<ConstructorInfo>(storeyb.<>m__A3));
                            if (index >= 0)
                            {
                                if (CompareMethod(storeyc.list[index], storeyb.r) == 2)
                                {
                                    storeyc.list.RemoveAt(index);
                                    storeyc.list.Add(storeyb.r);
                                }
                            }
                            else
                            {
                    ...
                    {
                        storeyc.list.Sort(new Comparison<ConstructorInfo>(ToLuaExport.Compare));
                        sb.AppendLine("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
                        sb.AppendFormat("\tstatic int _Create{0}(IntPtr L)\r\n", wrapClassName);
                        sb.AppendLine("\t{");
                        ...
                        <GenConstruct>c__AnonStorey8D storeyd = new <GenConstruct>c__AnonStorey8D();
                        storeyd.<>f__ref$140 = storeyc;
                        storeyd.i = 0;
                        while (storeyd.i < storeyc.list.Count)
                        {
                            if ((storeyc.list.FindIndex(new Predicate<ConstructorInfo>(storeyd.<>m__A4)) >= 0) || (HasOptionalParam(storeyc.list[storeyd.i].GetParameters()) && (storeyc.list[storeyd.i].GetParameters().Length > 1)))
                            {
                                list.Add(storeyc.list[storeyd.i]);
                            }
                            storeyd.i++;
                        }
                        MethodBase md = storeyc.list[0];
                        bool flag = storeyc.list[0].GetParameters().Length == 0;
                        ...
                        else
                        {
                            ...
                            if ((storeyc.list.Count == 1) || (md.GetParameters().Length != storeyc.list[1].GetParameters().Length))
                            {
                        ...
                        sb.AppendLine("\t\t{");
                        int num4 = ProcessParams(md, 3, true, storeyc.list.Count > 1, false);
                        sb.AppendFormat("\t\t\treturn {0};\r\n", num4);
                        sb.AppendLine("\t\t}");
                        for (int j = 1; j < storeyc.list.Count; j++)
                        {
                            flag = (storeyc.list[j].GetParameters().Length != 0) ? flag : true;
                            md = storeyc.list[j];
                            ParameterInfo[] infoArray5 = md.GetParameters();
                            if (!HasOptionalParam(md.GetParameters()))
                            {
                                if (list.Contains(storeyc.list[j]))
                                {
        ...
        public static void GenDelegates(DelegateType[] list)
        {
            ...
            for (int i = 0; i < list.Length; i++)
            {
                ...
                {
                    Debug.LogError(c.FullName + " not a delegate type");
                    return;
                }
            }
            ...
            sb.AppendLine("\t\t\t{");
            sb.AppendLine("\t\t\t\tUnityEngine.Debug.LogError(\"Delegate \" + t.FullName + \" not register\");");
            ...
            sb.AppendLine("\t}");
            sb.AppendLine("}");
            SaveFile(Application.get_dataPath() + "/Scripts/Pandora/uLua/Source/Base/DelegateFactory.cs");
            Clear();
        }
        ...
        {
            <Generate>c__AnonStorey89 storey = new <Generate>c__AnonStorey89();
            ...
            if (type.IsEnum)
            {
                ...
                GenEnumTranslator();
                sb.AppendLine("}\r\n");
                SaveFile(Application.get_dataPath() + "/Scripts/Pandora/uLua/Source/LuaWrap/" + wrapClassName + "Wrap.cs");
            }
            else
            {
                ...
                storey.ps = type.GetProperties();
                <Generate>c__AnonStorey8A storeya = new <Generate>c__AnonStorey8A();
                storeya.<>f__ref$137 = storey;
                storeya.i = 0;
                while (storeya.i < storey.ps.Length)
                {
                    int index = list.FindIndex(new Predicate<MethodInfo>(storeya.<>m__A0));
                    if ((index >= 0) && (list[index].Name != "get_Item"))
                    {
                        list.RemoveAt(index);
                    }
                    index = list.FindIndex(new Predicate<MethodInfo>(storeya.<>m__A1));
                ...
                GenFunction();
                sb.AppendLine("}\r\n");
                string path = Application.get_dataPath() + "/Scripts/Pandora/uLua/Source/LuaWrap/";
        ...
        private static void GenFunction()
        {
            ...
            for (int i = 0; i < methods.Length; i++)
            {
                ...
                {
                    Debug.Log("Generic Method " + mb.Name + " cannot be export to lua");
                }
                else
                {
        ...
        private static void GenLuaMethods()
        {
            ...
            {
                <>f__am$cache15 = delegate (MethodInfo p) {
        ...
        {
            <GenOverrideFunc>c__AnonStorey8F storeyf = new <GenOverrideFunc>c__AnonStorey8F();
            storeyf.list = new List<MethodInfo>();
            for (int i = 0; i < methods.Length; i++)
            {
                if (((methods[i].Name == name) && !methods[i].IsGenericMethod) && !HasDecimal(methods[i].GetParameters()))
                {
                    Push(storeyf.list, methods[i]);
                }
            }
            if (storeyf.list.Count == 1)
            {
                return storeyf.list[0];
            }
            storeyf.list.Sort(new Comparison<MethodInfo>(ToLuaExport.Compare));
            sb.AppendLine("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
            sb.AppendFormat("\tstatic int {0}(IntPtr L)\r\n", GetFuncName(name));
            sb.AppendLine("\t{");
            sb.AppendLine("\t\tint count = LuaDLL.lua_gettop(L);");
            List<MethodInfo> list = new List<MethodInfo>();
            <GenOverrideFunc>c__AnonStorey90 storey = new <GenOverrideFunc>c__AnonStorey90();
            storey.<>f__ref$143 = storeyf;
            storey.i = 0;
            while (storey.i < storeyf.list.Count)
            {
                if ((storeyf.list.FindIndex(new Predicate<MethodInfo>(storey.<>m__A6)) >= 0) || (HasOptionalParam(storeyf.list[storey.i].GetParameters()) && (storeyf.list[storey.i].GetParameters().Length > 1)))
                {
                    list.Add(storeyf.list[storey.i]);
                }
                storey.i++;
            }
            sb.AppendLine();
            MethodInfo md = storeyf.list[0];
            int num3 = (md.ReturnType != typeof(void)) ? 1 : 0;
            int num4 = !md.IsStatic ? 1 : 0;
            int num5 = num4 + 1;
            int num6 = md.GetParameters().Length + num4;
            int num7 = storeyf.list[1].GetParameters().Length + (!storeyf.list[1].IsStatic ? 1 : 0);
            ...
            sb.AppendLine("\t\t{");
            int num8 = ProcessParams(md, 3, false, (storeyf.list.Count > 1) && beLuaString, beCheckTypes);
            sb.AppendFormat("\t\t\treturn {0};\r\n", num3 + num8);
            sb.AppendLine("\t\t}");
            for (int j = 1; j < storeyf.list.Count; j++)
            {
                beLuaString = true;
                beCheckTypes = true;
                md = storeyf.list[j];
                ...
                if (!HasOptionalParam(md.GetParameters()))
                {
                    ...
                    if (list.Contains(storeyf.list[j]))
                    {
        ...
        private static void GenToStringFunc()
        {
            ...
            {
                <>f__am$cache16 = delegate (MethodInfo p) {
                    return p.Name == "ToString";
                };
            }
        ...
        public static string GetGenericLibName(Type type)
        {
            ...
            return (_C(str) + "_" + string.Join("_", GetGenericLibName(genericArguments)));
        }
        ...
        private static void ProcessExtends(List<MethodInfo> list)
        {
            ...
            if (extendType != null)
            {
                <ProcessExtends>c__AnonStorey91 storey = new <ProcessExtends>c__AnonStorey91();
                storey.list2 = new List<MethodInfo>();
                storey.list2.AddRange(extendType.GetMethods((BindingFlags.Instance | binding) | BindingFlags.DeclaredOnly));
                <ProcessExtends>c__AnonStorey92 storey2 = new <ProcessExtends>c__AnonStorey92();
                storey2.<>f__ref$145 = storey;
                storey2.i = storey.list2.Count - 1;
                while (storey2.i >= 0)
                {
                    ...
                    {
                        list.RemoveAll(new Predicate<MethodInfo>(storey2.<>m__A8));
        ...
        {
            <Push>c__AnonStorey8E storeye = new <Push>c__AnonStorey8E();
            storeye.r = r;
            int index = list.FindIndex(new Predicate<MethodInfo>(storeye.<>m__A5));
            if (index >= 0)
            {
                if (CompareMethod(list[index], storeye.r) == 2)
                {
                    list.RemoveAt(index);
                    list.Add(storeye.r);
                }
        ...
        [CompilerGenerated]
        private sealed class <GenConstruct>c__AnonStorey8B
        {
            internal ConstructorInfo r;

            internal bool <>m__A3(ConstructorInfo p)
            {
        ...
        [CompilerGenerated]
        private sealed class <GenConstruct>c__AnonStorey8C
        {
            internal List<ConstructorInfo> list;
        }

        [CompilerGenerated]
        private sealed class <GenConstruct>c__AnonStorey8D
        {
            internal ToLuaExport.<GenConstruct>c__AnonStorey8C <>f__ref$140;
            internal int i;

            internal bool <>m__A4(ConstructorInfo p)
            {
                return ((p != this.<>f__ref$140.list[this.i]) && (p.GetParameters().Length == this.<>f__ref$140.list[this.i].GetParameters().Length));
            }
        }

        [CompilerGenerated]
        private sealed class <Generate>c__AnonStorey89
        {
            internal PropertyInfo[] ps;
        }

        [CompilerGenerated]
        private sealed class <Generate>c__AnonStorey8A
        {
            internal ToLuaExport.<Generate>c__AnonStorey89 <>f__ref$137;
            internal int i;

            internal bool <>m__A0(MethodInfo m)
            {
                return (m.Name == ("get_" + this.<>f__ref$137.ps[this.i].Name));
            }

            internal bool <>m__A1(MethodInfo m)
            {
                return (m.Name == ("set_" + this.<>f__ref$137.ps[this.i].Name));
            }
        }

        [CompilerGenerated]
        private sealed class <GenOverrideFunc>c__AnonStorey8F
        {
            internal List<MethodInfo> list;
        }

        [CompilerGenerated]
        private sealed class <GenOverrideFunc>c__AnonStorey90
        {
            internal ToLuaExport.<GenOverrideFunc>c__AnonStorey8F <>f__ref$143;
            internal int i;

            internal bool <>m__A6(MethodInfo p)
            {
                return ToLuaExport.CompareParmsCount(p, this.<>f__ref$143.list[this.i]);
            }
        }

        [CompilerGenerated]
        private sealed class <ProcessExtends>c__AnonStorey91
        {
            internal List<MethodInfo> list2;
        }

        [CompilerGenerated]
        private sealed class <ProcessExtends>c__AnonStorey92
        {
            internal ToLuaExport.<ProcessExtends>c__AnonStorey91 <>f__ref$145;
            internal int i;

            internal bool <>m__A8(MethodInfo md)
            {
                return (md.Name == this.<>f__ref$145.list2[this.i].Name);
            }
        }

        [CompilerGenerated]
        private sealed class <Push>c__AnonStorey8E
        {
            internal MethodInfo r;

            internal bool <>m__A5(MethodInfo p)
            {
