namespace com.tencent.pandora
{
    ...
    public class NotificationCenter : MonoBehaviour
    {
        ...
        public void AddObserver(Component observer, string name, Component sender)
        {
            ...
            {
                Logger.d("在AddObserver函数中指定的是空名称!.");
            }
            else
            {
        ...
        public void PostNotification(Notification aNotification)
        {
            ...
            {
                Logger.d("Null name sent to PostNotification.");
            }
            else
            {
                ...
                if (list == null)
                {
                    Logger.d("在PostNotification的通知列表中未找到: " + aNotification.name);
                }
                else
                {
                    ...
                    for (int i = 0; i < list.Count; i++)
                    {
                        ...
                        {
                            item.SendMessage(aNotification.name, aNotification, 1);
                        }
                        if (count > list.Count)
                        {
                            i--;
