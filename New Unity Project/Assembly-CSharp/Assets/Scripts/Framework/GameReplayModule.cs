    ...
    public class GameReplayModule : Singleton<GameReplayModule>, IGameModule
    {
        ...
        public bool FlushRecord()
        {
            ...
            try
            {
                ...
                stream.Write(this.recordStream.GetBuffer(), 0, (int) this.recordStream.Position);
                stream.Flush();
                stream.Close();
                flag = true;
            }
                ...
                for (int i = 0; i < strArray.Length; i++)
                {
                    try
                    {
                        ...
                        if (CVersion.IsSynchronizedVersion(appVersion, usedResourceVersion))
                        {
                            ...
                            item.path = path;
                            item.heroId = reader.ReadUInt32();
                            item.startTime = reader.ReadInt64();
                            item.mapType = reader.ReadByte();
                            item.mapId = reader.ReadUInt32();
                            item.userName = reader.ReadString();
                            item.userHead = reader.ReadString();
                            item.userRankGrade = reader.ReadByte();
                            item.userRankClass = reader.ReadUInt32();
                            view.Add(item);
                            flag = true;
                        }
                        reader.Close();
        ...
        private void ObjToStr(StringBuilder sb, object obj, object prtObj = new object())
        {
            if (obj == null)
            {
                sb.Append("null");
