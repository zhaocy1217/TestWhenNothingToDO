namespace Assets.Scripts.Framework
{
    ...
    public class LobbyConnector : BaseConnector
    {
        ...
        {
            return (base.connected || ((((msg.stPkgHead.dwMsgID != 0x519) && (msg.stPkgHead.dwMsgID != 0x4ed)) && (msg.stPkgHead.dwMsgID != 0x515)) && (msg.stPkgHead.dwMsgID != 0x8f1)));
        }
        ...
        protected override void DealConnectError(ApolloResult result)
        {
            ...
            events.Add(new KeyValuePair<string, string>("type", "platform"));
        ...
        public void HandleSending()
        {
            if (base.connected)
            {
                ...
                while (base.connected && (index < this.lobbySendQueue.Count))
                {
                    ...
                    {
                        if (msg.stPkgHead.dwMsgID != 0x4ed)
                        {
        ...
        public CSPkg RecvPackage()
        {
            ...
            if ((base.connected && (base.connector != null)) && (base.connector.ReadData(out buffer, out num) == ApolloResult.Success))
            {
                ...
                if ((type == TdrError.ErrorType.TDR_NO_ERROR) && (usedSize > 0))
                {
                    ...
                    if ((pkg.stPkgHead.dwSvrPkgSeq > this.curSvrPkgSeq) || (pkg.stPkgHead.dwSvrPkgSeq == 0))
                    {
                        if ((pkg.stPkgHead.dwSvrPkgSeq > this.curSvrPkgSeq) && (pkg.stPkgHead.dwMsgID != 0x410))
                        {
