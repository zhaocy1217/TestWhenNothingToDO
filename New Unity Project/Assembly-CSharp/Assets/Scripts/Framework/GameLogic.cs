namespace Assets.Scripts.Framework
{
    ...
    public class GameLogic : Singleton<GameLogic>, IGameModule
    {
        ...
        {
            if ((Singleton<FrameSynchr>.instance.bActive && ((Singleton<FrameSynchr>.instance.CurFrameNum % this.HashCheckFreq) == 0)) && (Singleton<BattleLogic>.instance.isFighting && !Singleton<WatchController>.GetInstance().IsWatching))
            {
                ...
                for (int i = 0; i < heroActors.Count; i++)
                {
                    ...
                    src[num2++] = root.ValueComponent.actorHpTotal;
                    src[num2++] = (((int) root.ActorControl.myBehavior) | (((root.HorizonMarker.IsVisibleFor(1) == null) ? 0 : 1) << 5)) | (((root.HorizonMarker.IsVisibleFor(2) == null) ? 0 : 1) << 6);
                }
                src[num2++] = Singleton<GameObjMgr>.GetInstance().GameActors.Count;
        ...
        public bool UpdateTails()
        {
            ...
            catch (Exception exception)
            {
                ...
                object[] inParameters = new object[] { exception.Message, stackTrace };
