namespace Assets.Scripts.Framework
{
    ...
    public class NetworkModule : Singleton<NetworkModule>
    {
        ...
        [CompilerGenerated]
        private uint <lobbyPing>k__BackingField;
        ...
        private void HandleLobbyMsgRecv()
        {
            if (this.lobbySvr != null)
            {
                for (CSPkg pkg = this.lobbySvr.RecvPackage(); pkg != null; pkg = this.lobbySvr.RecvPackage())
                {
                    ...
                    {
                        uint num = ((uint) (Time.get_realtimeSinceStartup() * 1000f)) - pkg.stPkgData.stGameSvrPing.dwTime;
        ...
        public static void InitRelayConnnecting(COMDT_TGWINFO inRelayTgw)
        {
            if (inRelayTgw.dwVipCnt > 0)
            {
                ...
                para.bIsUDP = inRelayTgw.bIsUDP > 0;
                para.ip = host;
                ...
                MonoSingleton<GSDKsys>.GetInstance().StartSpeed(host, inRelayTgw.wVPort);
            }
        }
        ...
        private void UpdateGameConnection()
        {
            ...
            if (this.gameSvr.connected && ((Time.get_unscaledTime() - this._preGameHeartTime) > 3f))
            {
                this._preGameHeartTime = Time.get_unscaledTime();
                CSPkg msg = CreateDefaultCSPKG(0x4ec);
                msg.stPkgData.stRelaySvrPing.dwTime = (uint) (Time.get_realtimeSinceStartup() * 1000f);
        ...
        {
            if (this.lobbySvr.CanSendPing() && ((Time.get_unscaledTime() - this._preLobbyHeartTime) > 5f))
            {
                this._preLobbyHeartTime = Time.get_unscaledTime();
                CSPkg msg = CreateDefaultCSPKG(0x4ed);
                msg.stPkgData.stGameSvrPing.dwTime = (uint) (Time.get_realtimeSinceStartup() * 1000f);
                this.lobbySvr.PushSendMsg(msg);
            }
        }
        ...
        public uint lobbyPing
        {
            ...
            get
            {
                return this.<lobbyPing>k__BackingField;
            }
            [CompilerGenerated]
            private set
            {
                this.<lobbyPing>k__BackingField = value;
            }
        }
