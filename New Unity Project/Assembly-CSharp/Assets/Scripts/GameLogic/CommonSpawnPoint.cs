namespace Assets.Scripts.GameLogic
{
    ...
    public class CommonSpawnPoint : FuncRegion
    {
        ...
        {
            outPos = base.get_gameObject().get_transform().get_position();
            outDir = base.get_gameObject().get_transform().get_forward();
            if (this.RandomPos)
            {
                int num = 0;
        ...
        protected virtual void DoSpawnSelf()
        {
            ...
            foreach (SpawnerWrapper wrapper in this.SpawnerList)
            {
                if (wrapper != null)
                {
                    this.m_spawnedList.Add(wrapper.DoSpawn(zero, forward, base.get_gameObject()));
                }
            }
        }
