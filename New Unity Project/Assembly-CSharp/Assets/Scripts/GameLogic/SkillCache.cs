    ...
    public class SkillCache
    {
        ...
        public void UseSkillCache(PoolObjHandle<ActorRoot> _actorRoot)
        {
            if (_actorRoot != 0)
            {
                ...
                {
                    if (this.cacheMoveCommand.cmdType == 0x83)
                    {
                        ...
                        if (!_actorRoot.handle.ActorControl.GetNoAbilityFlag(ObjAbilityType.ObjAbility_MoveRotate))
                        {
                            ...
                            quaternion = Quaternion.LookRotation((Vector3) inDirection);
                            _actorRoot.handle.rotation = quaternion;
                        }
                    }
                    this.cacheRotateExpire = true;
                }
        ...
        {
            if ((_actorRoot != 0) && (((this.cacheMoveCommand != null) && !this.cacheMoveExpire) && (this.cacheMoveCommand.cmdType == 0x83)))
            {
                FrameCommand<MoveDirectionCommand> cacheMoveCommand = (FrameCommand<MoveDirectionCommand>) this.cacheMoveCommand;
                if (!_actorRoot.handle.ActorControl.GetNoAbilityFlag(ObjAbilityType.ObjAbility_Move))
                {
                    ...
                    Vector3 vector = _actorRoot.handle.myTransform.get_position();
                    VInt3 delta = num.NormalizeTo((_moveSpeed * _deltaTime) / 0x3e8);
                    VInt groundY = 0;
                    delta = PathfindingUtility.MoveLerp((ActorRoot) _actorRoot, (VInt3) vector, delta, out groundY);
                    if (_actorRoot.handle.MovementComponent.isFlying)
                    {
                        float y = vector.y;
                        _actorRoot.handle.myTransform.set_position(_actorRoot.handle.myTransform.get_position() + ((Vector3) delta));
                        Vector3 vector2 = _actorRoot.handle.myTransform.get_position();
                        vector2.y = y;
                        _actorRoot.handle.myTransform.set_position(vector2);
                    }
                    else
                    {
                        _actorRoot.handle.myTransform.set_position(_actorRoot.handle.myTransform.get_position() + ((Vector3) delta));
