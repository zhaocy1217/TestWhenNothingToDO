namespace Assets.Scripts.GameLogic
{
    ...
    public class LobbyLogic : Singleton<LobbyLogic>, IUpdateLogic
    {
        ...
        [CompilerGenerated]
        private bool <CanShowRolling>k__BackingField;
        ...
        private int CheckWifi()
        {
            ...
            if (Application.get_internetReachability() == 2)
            {
                return 2;
            }
            return 0;
        ...
        public bool ConnectServer()
        {
            if (Singleton<NetworkModule>.GetInstance().isOnlineMode && !this.isLogin)
            {
                ...
                para.url = ApolloConfig.loginUrl;
                para.ip = ApolloConfig.loginOnlyIpOrUrl;
                para.vPort = ApolloConfig.loginOnlyVPort;
        ...
        private void CreateBattleStatisticInfoData(HeroKDA HeroInfo, ref COMDT_HERO_BATTLE_STATISTIC_INFO stBattleStatisticInfo)
        {
            ...
            {
                stBattleStatisticInfo.bEyeNum = (HeroInfo.actorHero.handle.SkillControl.stSkillStat.m_uiSpawnEyeTimes >= 15) ? ((byte) 15) : ((byte) HeroInfo.actorHero.handle.SkillControl.stSkillStat.m_uiSpawnEyeTimes);
                for (int i = 0; i < stBattleStatisticInfo.bEyeNum; i++)
                {
                    stBattleStatisticInfo.astEyePos[i].iXPos = HeroInfo.actorHero.handle.SkillControl.stSkillStat.stEyePostion[i].x;
                    stBattleStatisticInfo.astEyePos[i].iZPos = HeroInfo.actorHero.handle.SkillControl.stSkillStat.stEyePostion[i].z;
                }
                ...
                {
                    if (HeroInfo.actorHero.handle.SkillControl.stSkillStat.SkillStatistictInfo[7].uiUsedTimes > stBattleStatisticInfo.bFakeWardNum)
                    {
                        stBattleStatisticInfo.bVisionWardNum = (byte) (HeroInfo.actorHero.handle.SkillControl.stSkillStat.SkillStatistictInfo[7].uiUsedTimes - stBattleStatisticInfo.bFakeWardNum);
                    }
        ...
        private void CreateGeneralData(uint playerId, ref COMDT_SETTLE_COMMON_DATA CommonData, bool bGMWin)
        {
            ...
            if (num8 > 0)
            {
                ...
                for (int num11 = 0; num11 < num9; num11++)
                {
                    ...
                    stGeneralData.astGame30SecDetail[num11].dwCoinCnt = playerKDA.GetPlayerCoinAtTimeWithType(num11 + 1, KDAStat.GET_COIN_CHANNEL_TYPE.GET_COIN_CHANNEL_TYPE_ALL);
                    stGeneralData.astGame30SecDetail[num11].dwSoliderKillCoin = playerKDA.GetPlayerCoinAtTimeWithType(num11 + 1, KDAStat.GET_COIN_CHANNEL_TYPE.GET_COIN_CHANNEL_TYPE_KILLSOLIDER);
                    stGeneralData.astGame30SecDetail[num11].dwMonsterKillCoin = playerKDA.GetPlayerCoinAtTimeWithType(num11 + 1, KDAStat.GET_COIN_CHANNEL_TYPE.GET_COIN_CHANNEL_TYPE_KILLMONSTER);
            ...
            stGeneralData.iPauseTimeTotal = (int) ((Time.get_timeSinceLevelLoad() - stGeneralData.iTimeUse) - 15f);
            ...
            stGeneralData.OperateType[6] = !GameSettings.ShowEnemyHeroHeadBtnMode ? 0 : 1;
            stGeneralData.OperateType[7] = !GameSettings.LunPanLockEnemyHeroMode ? 0 : 1;
        ...
        private void CreateHeroData(uint playerId, ref COMDT_SETTLE_COMMON_DATA CommonData)
        {
            ...
            else
            {
                ...
                while (enumerator.MoveNext())
                {
                    ...
                    CommonData.stHeroData.astHeroList[index].stHeroBattleInfo.dwSoliderKillCoin = (uint) enumerator.Current.CoinFromKillSolider;
                    CommonData.stHeroData.astHeroList[index].stHeroBattleInfo.dwMonsterKillCoin = (uint) enumerator.Current.CoinFromKillMonster;
                    CommonData.stHeroData.astHeroList[index].stHeroBattleInfo.dwTowerKillCoin = (uint) enumerator.Current.CoinFromKillTower;
                    CommonData.stHeroData.astHeroList[index].stHeroBattleInfo.dwDragonKillCoin = (uint) enumerator.Current.CoinFromKillDragon;
                    CommonData.stHeroData.astHeroList[index].stHeroBattleInfo.dwHeroKillCoin = (uint) enumerator.Current.CoinFromKillHero;
                    CommonData.stHeroData.astHeroList[index].stHeroBattleInfo.dwSystemAddCoin = (uint) enumerator.Current.CoinFromSystem;
                    CommonData.stHeroData.astHeroList[index].stHeroBattleInfo.bDeathNum = (byte) enumerator.Current.numDead;
                    for (int j = 0; (j < enumerator.Current.numDead) && (j < 50); j++)
                    {
                        CommonData.stHeroData.astHeroList[index].stHeroBattleInfo.astDeathPos[j].iXPos = enumerator.Current.m_arrDeadPosition[j].iXPos;
                        CommonData.stHeroData.astHeroList[index].stHeroBattleInfo.astDeathPos[j].iZPos = enumerator.Current.m_arrDeadPosition[j].iZPos;
                    }
        ...
        private COMDT_SETTLE_COMMON_DATA CreateReportData(uint playerId, bool bGMWin = false, bool bWin = false)
        {
            ...
            commonData.stOtherData.iLatitude = MonoSingleton<GPSSys>.GetInstance().iLatitude;
            commonData.stOtherData.iLongitude = MonoSingleton<GPSSys>.GetInstance().iLongitude;
            return commonData;
        }
        ...
        private void CreateSettleReportData(uint playerId, ref COMDT_SETTLE_COMMON_DATA CommonData, bool bGMWin, bool bWin)
        {
            ...
            index++;
            astDetail[index].stKeyInfo.bKeyNum = 1;
            astDetail[index].stKeyInfo.KeyDetail[0] = 0x23;
            astDetail[index].stValueInfo.bValueNum = 1;
            astDetail[index].stValueInfo.ValueDetail[0] = Singleton<BattleStatistic>.instance.GetCampScore(playerKDA.PlayerCamp);
            index++;
            CommonData.stStatisticData.astDetail[CommonData.stStatisticData.bNum].wNum = (ushort) index;
            CommonData.stStatisticData.bNum = (byte) (CommonData.stStatisticData.bNum + 1);
        }
            ...
            if (((HeroInfo.actorHero != 0) && (HeroInfo.actorHero.handle.SkillControl != null)) && ((HeroInfo.actorHero.handle.SkillControl.stSkillStat != null) && (HeroInfo.actorHero.handle.SkillControl.stSkillStat.SkillStatistictInfo != null)))
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    if (skillSlotArray[i] != null)
                    {
                        ...
                        if (skillStatistictInfo[i].uiUsedTimes != 0)
                        {
                            stSkillDetailInfo[i].iHitHeroRate = (int) ((skillStatistictInfo[i].iUseSkillHitHeroTimes * 100) / skillStatistictInfo[i].uiUsedTimes);
                        }
        ...
        private void FillVDStat(ref COMDT_SETTLE_GAME_GENERAL_INFO stGeneralData, uint playerID)
        {
            if (Singleton<BattleStatistic>.instance.m_vdStat != null)
            {
                ...
                if (player != null)
                {
                    ...
                    stGeneralData.bAdvantageNum = (((byte) Singleton<BattleStatistic>.instance.m_vdStat.count) >= 0x10) ? ((byte) 0x10) : ((byte) Singleton<BattleStatistic>.instance.m_vdStat.count);
        ...
        public void LoginGame()
        {
            if (!this.isLogin)
            {
                ...
                Utility.StringToByteArray(SystemInfo.get_deviceName() + " " + SystemInfo.get_deviceModel(), ref msg.stPkgData.stGameLoginReq.stClientInfo.szSystemHardware);
                Utility.StringToByteArray(SystemInfo.get_operatingSystem(), ref msg.stPkgData.stGameLoginReq.stClientInfo.szSystemSoftware);
                ...
                msg.stPkgData.stGameLoginReq.stClientInfo.iMemorySize = SystemInfo.get_systemMemorySize();
                ...
                byte[] bytes = Encoding.ASCII.GetBytes(SystemInfo.get_deviceUniqueIdentifier());
                if (bytes.Length > 0)
                {
                ...
                {
                    msg.stPkgData.stGameLoginReq.stRecruitSource = MonoSingleton<ShareSys>.GetInstance().m_RecruitSource;
                    MonoSingleton<ShareSys>.GetInstance().m_RecruitSource = null;
                }
                else
                {
                    msg.stPkgData.stGameLoginReq.stRecruitSource.iPlatId = 0;
                    msg.stPkgData.stGameLoginReq.stRecruitSource.stUin.dwLogicWorldId = 0;
                    msg.stPkgData.stGameLoginReq.stRecruitSource.stUin.ullUid = 0L;
                }
        ...
        private void OnUiFormUpdate(ListView<CUIFormScript> inForms)
        {
            if (inForms != null)
            {
                ...
                CUIFormScript script9 = Singleton<CUIManager>.GetInstance().GetForm(NewbieGuideScriptControl.FormGuideMaskPath);
        ...
        public static void ReqStartGuideLevel11(bool bReEntry, uint heroType = 0)
        {
            if (heroType == 0)
            {
            ...
            uint heroId = 0;
            int levelId = 0;
            if (CBattleGuideManager.GetGuide1v1HeroIDAndLevelID(heroType, out heroId, out levelId))
            {
                Singleton<LobbyLogic>.GetInstance().ReqStartGuideLevel(levelId, heroId);
        ...
        public static void ReqStartGuideLevel55(bool bReEntry, uint heroType = 0)
        {
            if (heroType == 0)
            {
                heroType = GameDataMgr.GetGlobeValue(RES_GLOBAL_CONF_TYPE.RES_GLOBAL_CONF_TYPE_NEWBIE_RECOMMEND_HEROTYPE1);
            ...
            uint heroId = 0;
            int levelId = 0;
            if (CBattleGuideManager.GetGuide5v5HeroIDAndLevelID(heroType, out heroId, out levelId))
            {
                Singleton<LobbyLogic>.GetInstance().ReqStartGuideLevel(levelId, heroId);
                CLobbySystem.AutoPopAllow = false;
                Singleton<CBattleGuideManager>.instance.bReEntry = bReEntry;
        ...
        public static void ReqStartGuideLevelSelHero(bool bReEntry, int levelId)
        {
            ...
            else
            {
                ...
                reportInfo.iLevelID = dataByKey.iCfgID;
                reportInfo.bChapterNo = (byte) dataByKey.iChapterId;
                reportInfo.bLevelNo = dataByKey.bLevelNo;
        ...
        public bool CanShowRolling
        {
            ...
            get
            {
                return this.<CanShowRolling>k__BackingField;
            }
            [CompilerGenerated]
            private set
            {
                this.<CanShowRolling>k__BackingField = value;
            }
        }
