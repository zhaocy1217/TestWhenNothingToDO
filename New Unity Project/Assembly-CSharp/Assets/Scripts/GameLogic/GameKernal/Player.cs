namespace Assets.Scripts.GameLogic.GameKernal
{
    ...
    public class Player
    {
        ...
        {
            <SetCaptain>c__AnonStorey47 storey = new <SetCaptain>c__AnonStorey47();
            storey.configId = configId;
            this.CaptainId = storey.configId;
            this.Captain = this._heroes.Find(new Predicate<PoolObjHandle<ActorRoot>>(storey.<>m__22));
        ...
        public void SetOperateMode(OperateMode _mode)
        {
            ...
            for (int i = 0; i < heroActors.Count; i++)
            {
                ...
                if ((handle != 0) && (handle.handle.TheActorMeta.PlayerId == this.PlayerId))
                {
                    HeroWrapper actorControl = handle.handle.ActorControl as HeroWrapper;
                    if (actorControl != null)
                    {
                        actorControl.CurOpMode = this.useOperateMode;
                    }
                }
            }
            if (ActorHelper.IsHostCtrlActor(ref this.Captain))
        ...
        [CompilerGenerated]
        private sealed class <SetCaptain>c__AnonStorey47
        {
            internal uint configId;

            internal bool <>m__22(PoolObjHandle<ActorRoot> hero)
            {
