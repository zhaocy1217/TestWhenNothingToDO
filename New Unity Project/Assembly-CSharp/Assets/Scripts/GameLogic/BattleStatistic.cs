    ...
    public class BattleStatistic : Singleton<BattleStatistic>
    {
        ...
        public uint GetBestPlayer()
        {
            ...
            while (enumerator.MoveNext())
            {
                ...
                rkda = current.Value;
                float num7 = ((rkda.numKill * num) + (rkda.numAssist * num2)) - (rkda.numDead * num3);
        ...
        public uint GetMvpPlayer(COM_PLAYERCAMP camp, bool bWin)
        {
            ...
            while (enumerator.MoveNext())
            {
                ...
                {
                    float mvpValue = rkda.MvpValue;
                    float kDAValue = rkda.KDAValue;
                    if ((rkda.PlayerCamp == camp) && (mvpValue >= num3))
                    {
                        if (mvpValue == num3)
                        {
                            KeyValuePair<uint, PlayerKDA> pair2 = enumerator.Current;
                        ...
                        if ((bWin || (mvpValue >= num5)) && (bWin || (kDAValue >= 3f)))
                        {
                            KeyValuePair<uint, PlayerKDA> pair3 = enumerator.Current;
        ...
        public string GetPlayerName(ulong playerUid, uint playerLogicWorldId)
        {
            if (this.m_playerKDAStat != null)
            {
                ...
                while (enumerator.MoveNext())
                {
                    ...
                    {
                        KeyValuePair<uint, PlayerKDA> pair2 = enumerator.Current;
                        if (playerLogicWorldId == pair2.Value.WorldId)
                        {
                            KeyValuePair<uint, PlayerKDA> pair3 = enumerator.Current;
                            return pair3.Value.PlayerName;
                        }
                    }
                }
            }
        ...
        private void OnActorBattleCoinChanged(PoolObjHandle<ActorRoot> actor, int changeValue, bool isIncome, PoolObjHandle<ActorRoot> target)
        {
            if (isIncome)
            {
                CampInfo info = this.campStat[(uint) actor.handle.TheActorMeta.ActorCamp];
                info.coinTotal += changeValue;
            }
        }
        ...
        {
            CampInfo info = this.campStat[(uint) act.handle.TheActorMeta.ActorCamp];
            info.soulExpTotal += changeValue;
        }
        ...
        public void RecordMvp(COMDT_GAME_INFO gameInfo)
        {
            ...
            for (int i = 0; i < stMvpScoreDetail.bAcntNum; i++)
            {
        ...
        public void StartStatistic()
        {
            ...
            for (uint i = 0; i < 3; i++)
            {
                this.campStat.Add(i, new CampInfo((COM_PLAYERCAMP) i));
            }
            this.initEvent();
