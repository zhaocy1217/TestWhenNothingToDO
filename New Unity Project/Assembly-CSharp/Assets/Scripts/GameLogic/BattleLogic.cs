namespace Assets.Scripts.GameLogic
{
    ...
    {
        [CompilerGenerated]
        private GlobalTrigger <m_globalTrigger>k__BackingField;
        ...
        public void DoBattleStart()
        {
            ...
            this.m_FpsTimeBegin = Time.get_time();
        ...
        private void onActorDead(ref GameDeadEventParam prm)
        {
            ...
            {
                if (((this.m_LevelContext != null) && this.m_LevelContext.IsMobaMode()) && ((prm.src != 0) && ActorHelper.IsHostCtrlActor(ref prm.src)))
                {
        ...
        private void OnActorHemophagia(ref HemophagiaEventResultInfo hri)
        {
            if (((hri.src == 0) || (hri.src.handle.Visible && hri.src.handle.InCamera)) && ActorHelper.IsHostActor(ref hri.src))
            {
                ...
                if (hri.hpChanged != 0)
                {
                    Vector3 vector = hri.src.handle.myTransform.get_position();
                ...
                else
                {
                    if (Singleton<CBattleSystem>.GetInstance().FightForm != null)
                    {
                        ...
                        if (this.m_LevelContext.IsMobaMode())
                        {
                            ...
                            param.isWin = Singleton<WinLose>.instance.LastSingleGameWin;
        ...
        private void OnVisit(ResSkillCombineCfgInfo InCfg)
        {
            byte num = 1;
            byte num2 = 5;
            if ((InCfg.bSrcType >= num) && (InCfg.bSrcType < num2))
            {
                if (this.s_dragonBuffIds.ContainsKey(InCfg.bSrcType))
                {
                    this.s_dragonBuffIds[InCfg.bSrcType] = InCfg.iCfgID;
                }
                else
                {
                    this.s_dragonBuffIds.Add(InCfg.bSrcType, InCfg.iCfgID);
                }
            }
        }
        ...
        public void UpdateLogic(int delta)
        {
            if (this.isRuning)
            {
                ...
                if ((Time.get_time() - this.m_FpsTimeBegin) > 5f)
                {
                    this.m_FpsTimeBegin = Time.get_time();
        ...
        public GlobalTrigger m_globalTrigger
        {
            ...
            get
            {
                return this.<m_globalTrigger>k__BackingField;
            }
            [CompilerGenerated]
            private set
            {
                this.<m_globalTrigger>k__BackingField = value;
            }
        }
    }
}
