namespace Assets.Scripts.GameLogic
{
    ...
    public class GameObjMgr : Singleton<GameObjMgr>, IUpdateLogic
    {
        ...
        public void AddActor(PoolObjHandle<ActorRoot> actor)
        {
            ...
            actor.handle.gameObject.set_name(actor.handle.name);
        ...
        public bool AddToCache(PoolObjHandle<ActorRoot> actor)
        {
            ...
            if (this.cachedRoot == null)
            {
                ...
                this.cachedRoot.set_name("CachedActorRoot");
            }
            handle.isRecycled = true;
            handle.myTransform.set_parent(this.cachedRoot.get_gameObject().get_transform());
            return true;
        }
        ...
        {
            <DestroyActor>c__AnonStorey45 storey = new <DestroyActor>c__AnonStorey45();
            storey.ObjID = ObjID;
            int index = 0;
            while (index < this.GameActors.Count)
            {
                ...
                if (handle.ObjID == storey.ObjID)
                {
                    ...
                    int num2 = this.HeroActors.FindIndex(new Predicate<PoolObjHandle<ActorRoot>>(storey.<>m__17));
                    if (num2 >= 0)
                    {
                        this.HeroActors.RemoveAt(num2);
                    }
                    num2 = this.OrganActors.FindIndex(new Predicate<PoolObjHandle<ActorRoot>>(storey.<>m__18));
                    if (num2 >= 0)
                    {
                        this.OrganActors.RemoveAt(num2);
                    }
                    num2 = this.TowerActors.FindIndex(new Predicate<PoolObjHandle<ActorRoot>>(storey.<>m__19));
                    if (num2 >= 0)
                    {
                        this.TowerActors.RemoveAt(num2);
                    }
                    num2 = this.SoldierActors.FindIndex(new Predicate<PoolObjHandle<ActorRoot>>(storey.<>m__1A));
                    if (num2 >= 0)
                    {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        num2 = this.CampsActors[i].FindIndex(new Predicate<PoolObjHandle<ActorRoot>>(storey.<>m__1B));
                        if (num2 >= 0)
                        {
                            this.CampsActors[i].RemoveAt(num2);
                        }
                    }
                    num2 = this.FakeTrueEyes.FindIndex(new Predicate<PoolObjHandle<ActorRoot>>(storey.<>m__1C));
                    if (num2 >= 0)
                    {
        ...
        public PoolObjHandle<ActorRoot> SpawnActorEx(GameObject rootObj, ref ActorMeta actorMeta, VInt3 pos, VInt3 dir, bool useLobbyModel, bool addComponent)
        {
            ...
            else
            {
                ...
                {
                    obj2 = componentInChildren.get_gameObject();
                }
                rootObj.get_transform().set_parent(MonoSingleton<SceneMgr>.GetInstance().GetRoot((SceneObjType) actorData.TheActorMeta.ActorType).get_transform());
            }
            component.ConfigID = actorMeta.ConfigId;
            CActorInfo actorInfo = null;
            if (!string.IsNullOrEmpty(actorData.TheResInfo.ResPath))
            {
                CActorInfo info2 = CActorInfo.GetActorInfo(actorData.TheResInfo.ResPath, enResourceType.BattleScene);
                if (info2 != null)
                {
                    actorInfo = Object.Instantiate(info2);
                    ...
                    {
                        bool isInit = false;
                        obj2 = MonoSingleton<SceneMgr>.GetInstance().GetPooledGameObjLOD(str, false, SceneObjType.ActionRes, Vector3.get_zero(), Quaternion.get_identity(), out isInit);
                        if (obj2 != null)
                        {
                            Transform transform = obj2.GetComponent<Transform>();
                            transform.SetParent(rootObj.get_transform());
                            ...
                            {
                                transform.set_localPosition(transform.get_localPosition() + transformConfigIfHaveOne.Offset);
                                transform.set_localScale((Vector3) (transform.get_localScale() * transformConfigIfHaveOne.Scale));
                            }
                        }
                    }
                }
        ...
        [CompilerGenerated]
        private sealed class <DestroyActor>c__AnonStorey45
        {
            internal uint ObjID;
            ...
            internal bool <>m__18(PoolObjHandle<ActorRoot> item)
            {
                return (item.handle.ObjID == this.ObjID);
            }

            internal bool <>m__19(PoolObjHandle<ActorRoot> item)
            {
                return (item.handle.ObjID == this.ObjID);
            }

            internal bool <>m__1A(PoolObjHandle<ActorRoot> item)
            {
                return (item.handle.ObjID == this.ObjID);
            }

            internal bool <>m__1B(PoolObjHandle<ActorRoot> item)
            {
                return (item.handle.ObjID == this.ObjID);
            }

            internal bool <>m__1C(PoolObjHandle<ActorRoot> item)
            {
                return (item.handle.ObjID == this.ObjID);
            }
        }
