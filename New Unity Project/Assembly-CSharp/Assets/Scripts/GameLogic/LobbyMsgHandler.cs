namespace Assets.Scripts.GameLogic
{
    ...
    public class LobbyMsgHandler
    {
        ...
        [CompilerGenerated]
        private static bool <isHostGMAcnt>k__BackingField;
        ...
        public static void onCSError(CSPkg msg)
        {
            ...
            else if (msg.stPkgData.stNtfErr.iErrCode != 0x29)
            {
                ...
                else if (msg.stPkgData.stNtfErr.iErrCode == 0xb2)
                {
                    Singleton<CUIManager>.GetInstance().OpenMessageBox(Singleton<CTextManager>.GetInstance().GetText("SecurePwd_Invalid_Pwd"), false);
                }
                else if (msg.stPkgData.stNtfErr.iErrCode == 0xb8)
                {
                    Singleton<CUIManager>.GetInstance().OpenMessageBox(Singleton<CTextManager>.GetInstance().GetText("SecurePwd_Invalid_Pwd"), false);
                }
                else if (msg.stPkgData.stNtfErr.iErrCode == 0xb6)
                {
                ...
                else if (msg.stPkgData.stNtfErr.iErrCode == 0xb7)
                {
        ...
        public static void onGameLoginEvent(CSPkg msg)
        {
            if (msg.stPkgData.stGameLoginRsp.bIsSucc != 0)
            {
                ...
                Singleton<CMallSystem>.GetInstance().m_PlayerRegisterTime = Utility.ToUtcTime2Local((long) msg.stPkgData.stGameLoginRsp.dwFirstLoginTime);
            }
        }
        ...
        public static void onMultiGameSettle(CSPkg msg)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    if ((msg.stPkgData.stMultGameSettleGain.stDetail.dwTipsMask & (((int) 1) << i)) != 0)
                    {
                        switch (i)
                        {
                            case 1:
                            {
                                uint dwConfValue = GameDataMgr.globalInfoDatabin.GetDataByKey((uint) 0x116).dwConfValue;
                                string[] args = new string[] { dwConfValue.ToString() };
                                Singleton<CUIManager>.GetInstance().OpenTips(Singleton<CTextManager>.GetInstance().GetText("Settlement_WEEK_Gold_Tips_FirstWin", args), false, 1.5f, null, new object[0]);
                            }
                        }
                    }
                }
                ...
                try
                {
                    ...
                    events.Add(new KeyValuePair<string, string>("error", "0"));
        ...
        public static void OnReciveOldAcntType(CSPkg msg)
        {
            ...
            {
                switch (stAcntOldTypeNtf.bAcntOldType)
                {
                    case 2:
                        Singleton<CUIManager>.GetInstance().CloseAllForm(null, true, true);
                        NewbieGuideManager.ShowCompleteNewbieGuidePanel();
            ...
            else if (stAcntOldTypeNtf.iErrCode == 0xb3)
            {
        ...
        public static void SendMidasToken(ApolloAccountInfo accountInfo)
        {
            ...
            if (accountInfo != null)
            {
                ...
                foreach (ApolloToken token in accountInfo.TokenList)
                {
                    ...
                    if ((ApolloConfig.platform == ApolloPlatform.Guest) && (token.Type == ApolloTokenType.Access))
                    {
                        str = token.Value;
                    }
                }
