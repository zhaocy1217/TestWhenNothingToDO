    ...
    public class ValueProperty : LogicComponent
    {
        ...
        public void AddSoulExp(int addVal, bool bFloatDigit, AddSoulType type)
        {
            if (Singleton<BattleLogic>.instance.m_LevelContext.IsSoulGrow())
            {
                ...
                {
                    Singleton<CBattleSystem>.GetInstance().CreateBattleFloatDigit(addVal, DIGIT_TYPE.ReceiveSpirit, base.actor.myTransform.get_position());
                }
        ...
        public void ChangeGoldCoinInBattle(int changeValue, bool isIncome, bool floatDigit = false, [Optional] Vector3 position, bool isLastHit = false, [Optional] PoolObjHandle<ActorRoot> target)
        {
            ...
            if (((floatDigit && (changeValue > 0)) && (isIncome && base.actor.Visible)) && ActorHelper.IsHostCtrlActor(ref this.actorPtr))
            {
                ...
                {
                    position = base.actor.myTransform.get_position();
                }
            ...
            Singleton<EventRouter>.GetInstance().BroadCastEvent<PoolObjHandle<ActorRoot>, int, bool, PoolObjHandle<ActorRoot>>("HeroGoldCoinInBattleChange", base.actorPtr, changeValue, isIncome, target);
        }

        public void ChangePhyAtkByPhyDefence()
        {
            ValueDataInfo info = this.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_PHYATKPT];
            info.addValue -= info.totalAddValueByDefence;
            long num = (this.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_PHYDEFPT].totalValue * info.convertRatioByDefence) / 0x2710L;
            info.totalAddValueByDefence = (int) num;
            info.addValue += info.totalAddValueByDefence;
        }
        ...
        public override void Init()
        {
            ...
            this.mEnergy.Init(base.actorPtr);
            this.actorHp = 0;
            this.actorEp = 0;
        ...
        public void OnValuePropertyChangeByMgcEffect()
        {
            ...
            base.actor.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_MAXHP].totalEftValueByMgc = (totalEftRatioByMgc * base.actor.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_MGCATKPT].totalValue) / 0x2710;
            ...
            totalEftRatioByMgc = base.actor.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_PHYATKPT].totalEftRatioByMgc;
            base.actor.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_PHYATKPT].totalEftValueByMgc = (totalEftRatioByMgc * base.actor.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_MGCATKPT].totalValue) / 0x2710;
        }
