namespace Assets.Scripts.GameLogic
{
    ...
    public sealed class JoystickMode : GameInputMode
    {
        ...
        private void HandleMoveInput(Vector2 axis, enInputMode inputMode)
        {
            if (axis != Vector2.get_zero())
            {
                if (((inputMode == enInputMode.UI) || (inputMode == this.m_leftAxisInputMode)) || (this.m_leftAxisInputMode == enInputMode.None))
                {
                    ...
                    Singleton<GameInput>.GetInstance().SendMoveDirection(Vector2.get_zero(), axis);
                }
            }
        ...
        {
            Vector2 inMovement = new Vector2(uiEvent.m_pointerEventData.get_delta().x, uiEvent.m_pointerEventData.get_delta().y);
            MonoSingleton<CameraSystem>.instance.UpdatePanelCameraMovement(ref inMovement);
            Vector3 vector2 = MonoSingleton<CameraSystem>.instance.MobaCamera.requirements.pivot.get_position();
            ...
            if ((theMinimapSys != null) && (theMinimapSys.MMiniMapCameraFrame_3Dui != null))
            {
                ...
                theMinimapSys.MMiniMapCameraFrame_3Dui.SetPos(vector2.x * Singleton<CBattleSystem>.instance.world_UI_Factor_Small.x, vector2.z * Singleton<CBattleSystem>.instance.world_UI_Factor_Small.y);
            }
        }
