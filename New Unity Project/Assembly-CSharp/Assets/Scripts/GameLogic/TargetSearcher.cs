    ...
    public class TargetSearcher : Singleton<TargetSearcher>
    {
        ...
        public void CommanderNotifyToAttack(COM_PLAYERCAMP camp, int srchR, PoolObjHandle<ActorRoot> target, bool ignoreSearchRange)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if ((root.ObjID != target.handle.ObjID) && (root.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero))
                {
                    VInt3 num5 = root.location - target.handle.location;
                    if (num5.sqrMagnitudeLong2D < num)
                    {
                        root.ActorControl.SetToAttackTarget(target, ignoreSearchRange);
                    }
                }
            }
        }
        ...
        public int GetActorsInCircle(VInt3 center, int radius, PoolObjHandle<ActorRoot>[] outResults, AreaEventTrigger InTriggerRef)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if ((null == InTriggerRef) || InTriggerRef.ActorFilter(ref act))
                {
                    VInt3 num6 = act.handle.location - center;
                    if (num6.sqrMagnitudeLong2D < num2)
                    {
                        outResults[num++] = act;
                    }
                }
        ...
        public int GetAliveHeroNum(COM_PLAYERCAMP camp)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                ActorRoot root = handle.handle;
                if ((root.TheActorMeta.ActorCamp == camp) && !root.ActorControl.IsDeadState)
                {
                    num++;
                }
            }
        ...
        public PoolObjHandle<ActorRoot> GetCommanderHeroTarget(COM_PLAYERCAMP camp, int range)
        {
            ...
            {
                PoolObjHandle<ActorRoot> handle = heroActors[i];
                ActorRoot inActor = handle.handle;
                if (((inActor.TheActorMeta.ActorCamp != camp) && ((inActor.ValueComponent.actorHp * 100) <= (inActor.ValueComponent.actorHpTotal * 70))) && !inActor.ActorControl.IsDeadState)
                {
                    num2 = this.GetHeroNumInRange(inActor, range, inActor.TheActorMeta.ActorCamp);
                    num3 = this.GetHeroNumInRange(inActor, range, camp);
                    if ((num2 <= 2) && (num3 > num2))
                    {
                        return heroActors[i];
                    }
        ...
        public PoolObjHandle<ActorRoot> GetCommanderTowerTarget(COM_PLAYERCAMP camp, int friendRange, int enemyRange)
        {
            ...
            int num2 = 0;
            OrganPos theOrganPos = OrganPos.Base;
            for (int i = 0; i < count; i++)
            {
                PoolObjHandle<ActorRoot> handle = towerActors[i];
                ActorRoot root = handle.handle;
                if (((root.TheActorMeta.ActorCamp != camp) && !root.ActorControl.IsDeadState) && (root.ObjLinker.theOrganPos < theOrganPos))
                {
                    theOrganPos = root.ObjLinker.theOrganPos;
                }
            }
            for (int j = 0; j < count; j++)
            {
                PoolObjHandle<ActorRoot> handle2 = towerActors[j];
                ActorRoot inActor = handle2.handle;
                if (((inActor.TheActorMeta.ActorCamp != camp) && !inActor.ActorControl.IsDeadState) && (inActor.ObjLinker.theOrganPos == theOrganPos))
                {
                    num2 = this.GetHeroNumInRange(inActor, enemyRange, inActor.TheActorMeta.ActorCamp);
                    if (this.GetHeroNumInRange(inActor, friendRange, camp) >= num2)
                    {
        ...
        public uint GetEnemyBuilding(ActorRoot InActor, int srchR)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    VInt3 num6 = actor.location - InActor.location;
                    if (((num6.sqrMagnitudeLong2D < num2) && !actor.ActorControl.IsDeadState) && !InActor.IsSelfCamp(actor))
                    {
                        return actor.ObjID;
                    }
                }
        ...
        public int GetEnemyCountInRange(ActorRoot InActor, int srchR)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                PoolObjHandle<ActorRoot> handle = campActors[i];
                ActorRoot root = handle.handle;
                if (!root.ActorControl.IsDeadState)
                {
        ...
        public int GetEnemyHeroCountInRange(ActorRoot InActor, int srchR)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                PoolObjHandle<ActorRoot> handle = campActors[i];
                ActorRoot root = handle.handle;
                if ((root.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero) && !root.ActorControl.IsDeadState)
                {
                    VInt3 num6 = root.location - InActor.location;
                    if (num6.sqrMagnitudeLong2D < num)
                    {
        ...
        public int GetHeroNumInRange(ActorRoot InActor, int range, Relationship rship)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                PoolObjHandle<ActorRoot> handle = heroActors[i];
                ActorRoot root = handle.handle;
                if ((((rship != Relationship.Enemy) || (InActor.TheActorMeta.ActorCamp != root.TheActorMeta.ActorCamp)) && ((rship != Relationship.FriendAndMe) || (InActor.TheActorMeta.ActorCamp == root.TheActorMeta.ActorCamp))) && !root.ActorControl.IsDeadState)
                {
                    VInt3 num6 = root.location - InActor.location;
                    if (num6.sqrMagnitudeLong2D < num)
                    {
            ...
            for (int i = 0; i < 3; i++)
            {
                if (i != InActor.TheActorMeta.ActorCamp)
                {
                    ...
                    for (int j = 0; j < count; j++)
                    {
                        PoolObjHandle<ActorRoot> handle = campActors[j];
                        ActorRoot target = handle.handle;
                        ActorTypeDef actorType = target.TheActorMeta.ActorType;
                        if (actorType == ActorTypeDef.Actor_Type_Monster)
                        {
                            ...
                            {
                                actorType = actorControl.hostActor.handle.TheActorMeta.ActorType;
                            }
                        }
                        if (((filter & (((int) 1) << actorType)) <= 0L) && target.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp))
                        {
                            if (!bWithMonsterNotInBattle)
                            {
                                MonsterWrapper wrapper2 = target.ActorControl as MonsterWrapper;
                                if (wrapper2 != null)
                                {
                                    ResMonsterCfgInfo cfgInfo = wrapper2.cfgInfo;
                        ...
                        if (target.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp))
                        {
                            ...
                            else
                            {
                                ...
                                if (wrapper != null)
                                {
                                    ...
                                    else if (wrapper.cfgInfo.bMonsterType != 2)
                                    {
            ...
            for (int i = 0; i < 3; i++)
            {
                if (i != InActor.TheActorMeta.ActorCamp)
                {
                    ...
                    for (int j = 0; j < count; j++)
                    {
                        ...
                        {
                            MonsterWrapper wrapper = target.AsMonster();
                            if ((wrapper == null) || (wrapper.cfgInfo.bMonsterType != 2))
                            {
                                VInt3 num6 = target.location - InActor.location;
                                ulong num5 = (ulong) num6.sqrMagnitudeLong2D;
                                if ((num5 < num) && InActor.CanAttack(target))
        ...
        public ActorRoot GetNearestEnemyWithTwoPriorityWithoutJungleMonster(ActorRoot InActor, int srchR, TargetPriority priotity_1, TargetPriority priotity_2)
        {
                ...
                if (i != InActor.TheActorMeta.ActorCamp)
                {
                    ...
                    for (int j = 0; j < count; j++)
                    {
                        ...
                        if (target.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp))
                        {
                            ...
                            if ((((priotity_1 == TargetPriority.TargetPriority_Hero) && (target.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)) || ((priotity_1 == TargetPriority.TargetPriority_Monster) && (target.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Monster))) || ((priotity_1 == TargetPriority.TargetPriority_Organ) && (target.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ)))
                            {
                                VInt3 num10 = target.location - InActor.location;
                                ulong num7 = (ulong) num10.sqrMagnitudeLong2D;
                                if ((num7 < num) && InActor.CanAttack(target))
                                {
                                    root = target;
                                    num = num7;
                                }
                            }
                            else if ((((priotity_2 == TargetPriority.TargetPriority_Hero) && (target.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)) || ((priotity_2 == TargetPriority.TargetPriority_Monster) && (target.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Monster))) || ((priotity_2 == TargetPriority.TargetPriority_Organ) && (target.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ)))
                            {
                                VInt3 num11 = target.location - InActor.location;
                                ulong num8 = (ulong) num11.sqrMagnitudeLong2D;
                                if ((num8 < num2) && InActor.CanAttack(target))
                                {
                                    root2 = target;
                                    num2 = num8;
                                }
                            ...
                            {
                                VInt3 num12 = target.location - InActor.location;
                                ulong num9 = (ulong) num12.sqrMagnitudeLong2D;
                                if ((num9 < num3) && InActor.CanAttack(target))
                                {
                                    root3 = target;
                                    num3 = num9;
                                }
            ...
            return ((root == null) ? ((root2 == null) ? root3 : root2) : root);
        }
        ...
        public List<ActorRoot> GetOurCampActors(ActorRoot InActor, int srchR)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if ((item.ObjID != InActor.ObjID) && !item.ActorControl.IsDeadState)
                {
        ...
        public bool HasSelfSoldierCountRoundEnemyBuilding(ActorRoot InActor, uint buildId)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                PoolObjHandle<ActorRoot> handle = organActors[i];
                ActorRoot root = handle.handle;
                if ((root.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ) || (root.ObjID == buildId))
                {
                    long num3 = root.ActorControl.AttackRange * root.ActorControl.AttackRange;
                    List<PoolObjHandle<ActorRoot>> soldierActors = Singleton<GameObjMgr>.GetInstance().SoldierActors;
                    int num4 = soldierActors.Count;
                    for (int j = 0; j < num4; j++)
                    {
                        ...
                        if (root2.IsSelfCamp(InActor))
                        {
                            VInt3 num7 = root2.location - root.location;
                            if (num7.sqrMagnitudeLong2D < num3)
                            {
                                return true;
                            }
                        }
