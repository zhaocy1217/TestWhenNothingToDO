namespace Assets.Scripts.GameLogic.SkillFunc
{
    ...
    internal class SkillFuncSpecialDelegator
    {
        ...
        [SkillFuncHandler(0x55, new int[] {  })]
        public static bool OnSkillFuncDecreaseReviveTime(ref SSkillFuncContext inContext)
        {
            PoolObjHandle<ActorRoot> inTargetObj = inContext.inTargetObj;
            if ((inTargetObj != 0) && (inContext.inStage == ESkillFuncStage.Enter))
            {
                int skillFuncParam = inContext.GetSkillFuncParam(0, false);
                ...
                {
                    case 0:
                        inTargetObj.handle.ActorControl.ReviveCooldown -= num2;
                        break;

                    case 1:
                        inTargetObj.handle.ActorControl.ReviveCooldown -= (int) (((long) (num2 * inTargetObj.handle.ActorControl.CfgReviveCD)) / 0x2710L);
                        break;
                }
            }
            return true;
        ...
        public static bool OnSkillFuncHpCondition(ref SSkillFuncContext inContext)
        {
            ...
            if ((skillFuncParam < 0) || (skillFuncParam >= 0x25))
            {
                return false;
            }
        ...
        private static void SkillFuncChangeSkillCDImpl(ref SSkillFuncContext inContext, int changeType, int slotMask, int value)
        {
            ...
            if (inTargetObj != 0)
            {
                ...
                if (skillControl != null)
                {
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        if (((((slotMask == 0) && (i != 0)) && (((i != 4) && (i != 5)) && (i != 7))) || ((slotMask & (((int) 1) << i)) > 0)) && (skillControl.TryGetSkillSlot((SkillSlotType) i, out slot) && (slot != null)))
                        {
                            if (changeType == 0)
                            {
