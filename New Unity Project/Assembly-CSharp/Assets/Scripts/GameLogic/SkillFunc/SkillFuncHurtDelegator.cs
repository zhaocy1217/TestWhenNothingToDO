namespace Assets.Scripts.GameLogic.SkillFunc
{
    ...
    public class SkillFuncHurtDelegator
    {
        ...
        private static int GetSkillFuncProtectValue(ref SSkillFuncContext inContext)
        {
            ...
            if (inContext.inStage == ESkillFuncStage.Enter)
            {
                ...
                if ((inTargetObj != 0) && (inOriginator != 0))
                {
                    ...
                    info.iConditionType = inContext.GetSkillFuncParam(6, false);
                    info.iConditionParam = inContext.GetSkillFuncParam(7, true);
                    info.loseHpValue = 0;
                    info.hurtCount = inContext.inDoCount;
                    info.firstHemoFadeRate = 0x2710;
                    info.followUpHemoFadeRate = 0x2710;
                    info.iEffectCountInSingleTrigger = 1;
                    ...
                    info.iCanSkillCrit = inContext.inBuffSkill.handle.cfgData.bCanSkillCrit;
                    ...
                    info.iEffectiveTargetType = inContext.inBuffSkill.handle.cfgData.bEffectiveTargetType;
        ...
        private static bool HandleSkillFuncHurt(ref SSkillFuncContext inContext, HurtTypeDef hurtType)
        {
            ...
            if (inContext.inStage == ESkillFuncStage.Enter)
            {
                ...
                if ((inTargetObj != 0) && !inTargetObj.handle.ActorControl.IsDeadState)
                {
                    ...
                    info.iConditionParam = inContext.GetSkillFuncParam(7, true);
                    info.hurtCount = inContext.inDoCount;
                    info.firstHemoFadeRate = inContext.inBuffSkill.handle.cfgData.iFirstLifeStealAttenuation;
                    info.followUpHemoFadeRate = inContext.inBuffSkill.handle.cfgData.iFollowUpLifeStealAttenuation;
                    info.iEffectCountInSingleTrigger = inContext.inEffectCountInSingleTrigger;
                    info.bExtraBuff = inContext.inBuffSkill.handle.bExtraBuff;
                    info.gatherTime = inContext.inUseContext.GatherTime;
                    flag = inContext.inBuffSkill.handle.cfgData.bEffectSubType == 9;
            ...
            else if (inContext.inStage == ESkillFuncStage.Update)
            {
                ...
                if ((handle3 != 0) && !handle3.handle.ActorControl.IsDeadState)
                {
                    ...
                    info2.iConditionType = inContext.GetSkillFuncParam(6, false);
                    info2.iConditionParam = inContext.GetSkillFuncParam(7, true);
                    info2.hurtCount = inContext.inDoCount;
                    info2.firstHemoFadeRate = inContext.inBuffSkill.handle.cfgData.iFirstLifeStealAttenuation;
                    info2.followUpHemoFadeRate = inContext.inBuffSkill.handle.cfgData.iFollowUpLifeStealAttenuation;
                    info2.iEffectCountInSingleTrigger = inContext.inEffectCountInSingleTrigger;
                    info2.bExtraBuff = inContext.inBuffSkill.handle.bExtraBuff;
                    info2.gatherTime = inContext.inUseContext.GatherTime;
                    flag2 = inContext.inBuffSkill.handle.cfgData.bEffectSubType == 9;
                    info2.bBounceHurt = flag2;
                    info2.bLastHurt = inContext.inLastEffect;
                    info2.iAddTotalHurtValueRate = 0;
                    info2.iAddTotalHurtValue = 0;
                    info2.iCanSkillCrit = inContext.inBuffSkill.handle.cfgData.bCanSkillCrit;
                    ...
                    info2.iEffectiveTargetType = inContext.inBuffSkill.handle.cfgData.bEffectiveTargetType;
        ...
        public static bool OnSkillFuncProtect(ref SSkillFuncContext inContext)
        {
            ...
            int num4 = inContext.GetSkillFuncParam(6, false);
            switch (skillFuncParam)
            {
                ...
                case 4:
                    ...
                    SendProtectEvent(ref inContext, 3, skillFuncProtectValue);
                    break;
            }
            inContext.inBuffSkill.handle.CustomParams[4] = num3;
            inContext.inBuffSkill.handle.CustomParams[5] = num4;
        ...
        public static bool OnSkillFuncSuckBloodSpecialSkill(ref SSkillFuncContext inContext)
        {
            ...
            {
                int num = 0;
                if ((inContext.inSkillFunc.astSkillFuncParam[1].iParam & (((int) 1) << inContext.inUseContext.SlotType)) == 0)
                {
                    return false;
                }
                if (inContext.inAction.handle.refParams.GetRefParam("HurtValue", ref num))
