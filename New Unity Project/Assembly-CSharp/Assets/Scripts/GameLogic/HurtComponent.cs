    ...
    public class HurtComponent : LogicComponent
    {
        ...
        private int Hemophagia(ref HurtDataInfo hurt, int hurtValue)
        {
            ...
            if (hurt.atker != 0)
            {
                ...
                {
                    nAddHp = (nAddHp * hurt.followUpHemoFadeRate) / 0x2710;
                }
                else
                {
                    nAddHp = (nAddHp * hurt.firstHemoFadeRate) / 0x2710;
                }
                if (nAddHp > 0)
                {
        ...
        public int TakeBouncesDamage(ref HurtDataInfo hurt)
        {
            ...
            int actorHp = base.actor.ValueComponent.actorHp;
            num = ((((this.CommonDamagePart(ref hurt) * (0x2710 - this.ReduceDamagePart(ref hurt, hurt.hurtType))) / 0x2710) * (0x2710 + num3)) / 0x2710) - base.actor.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_REALHURTLESS].totalValue;
            num = (num * (0x2710 - base.actor.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_PROPERTY_HURTREDUCERATE].totalValue)) / 0x2710;
            if (num < 0)
            {
                num = 0;
            }
            if (hurt.atker != 0)
            {
                num = (num * Singleton<BattleLogic>.GetInstance().clashAddition.CalcDamageAddition(hurt.atker.handle.TheStaticData.TheBaseAttribute.ClashMark, base.actor.TheStaticData.TheBaseAttribute.ClashMark)) / 0x2710;
            }
            num = base.actor.BuffHolderComp.OnDamage(ref hurt, num);
            num2 = base.actor.ValueComponent.actorHp;
            base.actor.ValueComponent.actorHp -= num;
            num2 = base.actor.ValueComponent.actorHp - num2;
            if (flag)
            {
            ...
            HurtEventResultInfo prm = new HurtEventResultInfo(base.GetActor(), hurt.atker, hurt, num, num2, num3);
        ...
        public int TakeDamage(ref HurtDataInfo hurt)
        {
            ...
            if (hurt.hurtType == HurtTypeDef.Therapic)
            {
                ...
                {
                    nAddHp += (nAddHp * hurt.iAddTotalHurtValueRate) / 0x2710;
                }
                nAddHp += hurt.iAddTotalHurtValue;
                if (((hurt.atker != 0) && (hurt.atker != base.actorPtr)) && Singleton<BattleLogic>.GetInstance().GetCurLvelContext().IsLuanDouPlayMode())
                {
                    nAddHp = nAddHp >> 1;
                }
            ...
            else
            {
                ...
                nAddHp = (this.CommonDamagePart(ref hurt) * num4) / 0x2710;
                nAddHp = (((((base.actor.BuffHolderComp.OnHurtBounceDamage(ref hurt, nAddHp) * (0x2710 - this.ReduceDamagePart(ref hurt, hurt.hurtType))) / 0x2710) * (0x2710 + num3)) / 0x2710) + hurt.attackInfo.iFinalHurt) - base.actor.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_REALHURTLESS].totalValue;
