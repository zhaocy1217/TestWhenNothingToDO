    ...
    public class BurnExpeditionView
    {
            ...
            if (((this.boxNodeList != null) && (index >= 0)) && (index < this.boxNodeList.Count))
            {
                ...
                if (obj2 != null)
                {
                    ...
                    if (component != null)
                    {
                        ...
                        {
                            component.set_enabled(state == COM_LEVEL_STATUS.COM_LEVEL_STATUS_UNLOCKED);
                        }
        ...
        private void _Show_Buff(GameObject node, int buffid, bool bSelected)
        {
            ...
            else
            {
                ...
                {
                    image.SetSprite(str, this.map_fromScript, true, false, false, false);
                }
            }
        }
        ...
        private void _Show_Enemy_Heros(int index, uint cfgID, string icon, int level, int startCount, uint dwBloodTTH, uint heroID)
        {
            ...
            if (obj2 != null)
            {
                ...
                Utility.GetComponetInChild<Image>(obj2, "blood_bar").set_fillAmount(num);
        ...
        private void _show_LevelNode(int index, string name, string icon, COM_LEVEL_STATUS state)
        {
            ...
            {
                obj2.CustomSetActive(true);
                obj2.GetComponent<CUIEventScript>().set_enabled(state == COM_LEVEL_STATUS.COM_LEVEL_STATUS_UNLOCKED);
            }
        ...
        Label_005E:
            ...
            for (int i = 0; i < node.get_transform().get_childCount(); i++)
            {
                node.get_transform().GetChild(i).get_gameObject().CustomSetActive(false);
            }
            Utility.FindChild(node, path).CustomSetActive(true);
        }

        private void _Show_PlayerInfo(COMDT_PLAYERINFO info, uint force, int levelIndex, string headurl = "", COMDT_GAME_VIP_CLIENT nobeVip = new COMDT_GAME_VIP_CLIENT())
        {
            if ((info != null) && (info.szName != null))
            {
                ...
                Image component = this.enemyNode.get_transform().FindChild("PlayerIcon/NobeIcon").GetComponent<Image>();
                Image image = this.enemyNode.get_transform().FindChild("PlayerIcon/pnlSnsHead/NobeImag").GetComponent<Image>();
                if (nobeVip != null)
                {
                    if (component != null)
                    {
        ...
        public void Check_Box_Info(uint goldNum, uint burn_num)
        {
            ...
            Utility.GetComponetInChild<Text>(formScript.get_gameObject(), "bg/Title").set_text(UT.GetText("Burn_Box_Award"));
            GameObject container = Utility.FindChild(formScript.get_gameObject(), "IconContainer");
        ...
        public void Init()
        {
            ...
            this.mapNode = Utility.FindChild(this.map_fromScript.get_gameObject(), "mapNode/map");
            this.animationScript = Utility.GetComponetInChild<CUIAnimatorScript>(this.mapNode, "Panel_Pointer");
            for (int i = 0; i < this.mapNode.get_transform().get_childCount(); i++)
            {
                if (this.mapNode.get_transform().GetChild(i).get_name().IndexOf("level") != -1)
                {
                    this.levelNodeList.Add(null);
                }
                if (this.mapNode.get_transform().GetChild(i).get_name().IndexOf("box") != -1)
                {
                    this.boxNodeList.Add(null);
                }
            }
            for (int j = 0; j < this.mapNode.get_transform().get_childCount(); j++)
            {
                GameObject obj2 = this.mapNode.get_transform().GetChild(j).get_gameObject();
                if (obj2.get_name().IndexOf("Panel") == -1)
                {
                    obj2.CustomSetActive(false);
                    int index = BurnExpeditionUT.GetIndex(obj2.get_name());
                    if (obj2.get_name().IndexOf("level") != -1)
                    {
                        this.levelNodeList[index - 1] = obj2;
                    }
                    else if (obj2.get_name().IndexOf("box") != -1)
                    {
                        this.boxNodeList[index - 1] = obj2;
                    }
                }
            }
            this.resetNumText = Utility.GetComponetInChild<Text>(this.map_fromScript.get_gameObject(), "mapNode/toolbar/Info");
            this.coinText = Utility.GetComponetInChild<Text>(this.map_fromScript.get_gameObject(), "mapNode/toolbar/Coin/num");
            this.enemyNode = Utility.FindChild(this.map_fromScript.get_gameObject(), "enemyNode");
            ...
            this.buffNode = Utility.FindChild(this.map_fromScript.get_gameObject(), "enemyNode/Buffs");
            ...
            this.mapNode.get_transform().get_parent().get_gameObject().CustomSetActive(true);
        ...
        public bool IsShow()
        {
            return ((this.map_fromScript != null) && this.map_fromScript.get_gameObject().get_activeInHierarchy());
        }
        ...
        private void Set_Award(GameObject container, int index, string icon, string count, string desc, CUIFormScript formScript)
        {
            ...
            {
                GameObject obj2 = container.GetComponent<CUIListScript>().GetElemenet(index).get_gameObject();
                obj2.CustomSetActive(true);
                Utility.GetComponetInChild<Image>(obj2, "imgIcon").SetSprite(icon, formScript, true, false, false, false);
                Utility.GetComponetInChild<Text>(obj2, "lblIconCount").get_gameObject().CustomSetActive(!string.IsNullOrEmpty(count));
                Utility.GetComponetInChild<Text>(obj2, "lblIconCount").set_text(count);
                if (!string.IsNullOrEmpty(desc))
                {
                    Utility.GetComponetInChild<Text>(obj2, "ItemName").set_text(desc);
                }
            }
        }
        ...
        public void Show_ResetNum(int num)
        {
            ...
            {
                this.resetNumText.get_gameObject().CustomSetActive(true);
                string str = string.Format("<color=#be7d15ff>{0}</color>", num);
                this.resetNumText.set_text(string.Format(UT.GetText("Burn_Valid_Count"), str));
            }
        }
    }
}
