namespace Assets.Scripts.GameLogic
{
    ...
    public class BuffSkill : BaseSkill
    {
        ...
        {
            if (context.TargetActor.handle.ActorControl.GetNoAbilityFlag(ObjAbilityType.ObjAbility_ImmuneNegative) && ((this.cfgData.bEffectType == 1) || (this.cfgData.bEffectType == 2)))
            {
                return false;
            }
            if (context.TargetActor.handle.ActorControl.GetNoAbilityFlag(ObjAbilityType.ObjAbility_ImmuneControl) && (this.cfgData.bEffectType == 2))
            {
                return false;
            }
            return true;
        ...
        public ResDT_SkillFunc FindSkillFunc(int inSkillFuncType)
        {
            if (this.cfgData != null)
            {
                for (int i = 0; i < 4; i++)
                {
                    if (this.cfgData.astSkillFuncInfo[i].bSkillFuncType == inSkillFuncType)
                    {
        ...
        public int GetSkillFuncParam(ResDT_SkillFunc skillFunc, int _index, bool _bGrow)
        {
            if (skillFunc != null)
            {
                ...
                if (_bGrow)
                {
                    int iParam = skillFunc.astSkillFuncParam[_index].iParam;
                    int num3 = skillFunc.astSkillFuncGroup[_index].iParam * (this.buffLevel - 1);
                    iParam += num3;
        ...
        private bool UseImpl(PoolObjHandle<ActorRoot> user)
        {
            ...
            switch (base.skillContext.AppointType)
            {
                ...
                case SkillRangeAppointType.Directional:
                    ...
                    if (base.skillContext.TargetID != 0)
                    {
                        PoolObjHandle<ActorRoot> actor = Singleton<GameObjMgr>.GetInstance().GetActor(base.skillContext.TargetID);
                        if (actor != 0)
                        {
            ...
            base.curAction = new PoolObjHandle<Action>(ActionManager.Instance.PlayAction(base.ActionName, true, false, objArray1));
            if (base.curAction == 0)
            {
                return false;
            }
            ...
            if ((this.cfgData.bShowType != 0) || (this.cfgData.bFloatTextID > 0))
            {
                ...
                if (flag3)
                {
                    SpawnBuffEventParam param = new SpawnBuffEventParam(this.cfgData.bShowType, this.cfgData.bFloatTextID, base.skillContext.TargetActor);
