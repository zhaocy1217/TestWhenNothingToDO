        ...
        protected void UpdateRotateDir(VInt3 targetDir, int dt)
        {
            ...
            if (targetDir != VInt3.zero)
            {
                ...
                if (!actor.ActorControl.GetNoAbilityFlag(ObjAbilityType.ObjAbility_MoveRotate))
                {
                    ...
                    if (actor.InCamera)
                    {
                        Vector3 vector = this.Parent.actor.myTransform.get_forward();
                        ...
                        Quaternion quaternion = Quaternion.AngleAxis(factor.single * 57.29578f, Vector3.get_up());
                        actor.rotation = Quaternion.RotateTowards(actor.rotation, quaternion, (this.Parent.RotateSpeed * dt) * 0.001f);
                    }
                }
            }
        }
