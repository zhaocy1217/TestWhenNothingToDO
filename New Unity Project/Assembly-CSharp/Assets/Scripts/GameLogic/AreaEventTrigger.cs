    ...
    public class AreaEventTrigger : FuncRegion, ITrigger
    {
        ...
        protected DictionaryView<TriggerActionBase, RefParamOperator> DoActorEnterShared(ref PoolObjHandle<ActorRoot> inActor)
        {
            ...
            foreach (TriggerActionWrapper wrapper in this.m_internalActList)
            {
                if (wrapper != null)
                {
                    RefParamOperator @operator = wrapper.TriggerEnter(inActor, this.thisActor, this);
                    if (@operator != null)
                    {
        ...
        private int GetCollidingActors()
        {
            ...
            VInt3 center = base.get_transform().get_position();
            return Singleton<TargetSearcher>.GetInstance().GetActorsInCircle(center, this.Radius, this._actorTestCache, this);
        }

        private VCollisionShape GetCollisionShape()
        {
            SCollisionComponent component = base.get_gameObject().GetComponent<SCollisionComponent>();
            if (component != null)
            {
                return component.CreateShape();
            }
            return VCollisionShape.createFromCollider(base.get_gameObject());
        }
        ...
        public override void Startup()
        {
            ...
            this.m_collidePolygon = base.get_gameObject().GetComponent<GeoPolygon>();
            Singleton<GameEventSys>.instance.RmvEventHandler<DefaultGameEventParam>(GameEventDef.Event_ActorDestroy, new RefAction<DefaultGameEventParam>(this.onActorDestroy));
        ...
        public override void Stop()
        {
            Singleton<GameEventSys>.instance.RmvEventHandler<DefaultGameEventParam>(GameEventDef.Event_ActorDestroy, new RefAction<DefaultGameEventParam>(this.onActorDestroy));
            base.Stop();
            int length = this.m_internalActList.Length;
            for (int i = 0; i < length; i++)
            {
                TriggerActionWrapper wrapper = this.m_internalActList[i];
                if (wrapper != null)
                {
                    wrapper.Stop();
                }
        ...
        private void UpdateCollisionShape()
        {
            if ((this.m_shape == null) && !this.m_bShaped)
            {
                ...
                if (this.m_shape != null)
                {
                    VInt3 num = base.get_transform().get_forward();
                    this.m_shape.UpdateShape((VInt3) base.get_transform().get_position(), num.NormalizeTo(0x3e8));
        ...
        {
            if (base.isStartup && this.CheckDifficulty())
            {
                if (this.CoolMode)
                {
