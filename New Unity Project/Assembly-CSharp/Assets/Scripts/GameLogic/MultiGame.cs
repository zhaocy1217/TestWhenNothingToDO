                ...
                for (int i = 0; i < 2; i++)
                {
                    for (int j = 0; j < gameContext.MessageRef.astCampInfo[i].dwPlayerNum; j++)
                    {
                        ...
                        if (player == null)
                        {
                            ...
                            player = Singleton<GamePlayerCenter>.GetInstance().AddPlayer(dwObjId, (COM_PLAYERCAMP) (i + 1), gameContext.MessageRef.astCampInfo[i].astCampPlayerInfo[j].stPlayerInfo.bPosOfCamp, gameContext.MessageRef.astCampInfo[i].astCampPlayerInfo[j].stPlayerInfo.dwLevel, isComputer, Utility.UTF8Convert(gameContext.MessageRef.astCampInfo[i].astCampPlayerInfo[j].stPlayerInfo.szName), 0, (int) dwFakeLogicWorldID, uid, vipLv, openId, gameContext.MessageRef.astCampInfo[i].astCampPlayerInfo[j].dwGradeOfRank, gameContext.MessageRef.astCampInfo[i].astCampPlayerInfo[j].dwClassOfRank, honorId, honorLevel);
