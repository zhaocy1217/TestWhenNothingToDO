    ...
    internal struct BufferMarkParticle
    {
        ...
        private void CreateParticle(string _resName, ref PoolObjHandle<ActorRoot> _origin, ref PoolObjHandle<ActorRoot> _target)
        {
            if (_target != 0)
            {
                ...
                this.particleObj = MonoSingleton<SceneMgr>.GetInstance().GetPooledGameObjLOD(prefabName, true, SceneObjType.ActionRes, _target.handle.myTransform.get_position());
                if (this.particleObj == null)
                {
                    this.particleObj = MonoSingleton<SceneMgr>.GetInstance().GetPooledGameObjLOD(_resName, true, SceneObjType.ActionRes, _target.handle.myTransform.get_position());
                }
                if (this.particleObj != null)
                {
                    string layerName = "Particles";
                    if (_target.handle.gameObject.get_layer() == LayerMask.NameToLayer("Hide"))
                    {
                        layerName = "Hide";
                    }
                    this.particleObj.SetLayer(layerName, false);
                    this.particleObj.get_transform().SetParent(_target.handle.myTransform);
        ...
        private void RemoveParticle()
        {
            if (this.particleObj != null)
            {
                this.particleObj.get_transform().set_position(new Vector3(10000f, 10000f, 10000f));
