    ...
    public class CActorInfo : ScriptableObject
    {
        ...
        public VCollisionShape CreateCollisionShape()
        {
            ...
            if (this.collisionType == CollisionShapeType.Box)
            {
                ...
                box.Pos = this.iCollisionCenter;
                box.Size = this.iCollisionSize;
                return box;
            }
            if (this.collisionType == CollisionShapeType.Sphere)
            {
                VCollisionSphere sphere = new VCollisionSphere();
                sphere.Pos = this.iCollisionCenter;
                sphere.Radius = this.iCollisionSize.x;
                return sphere;
            }
            ...
            if ((skinId >= 1) && (skinId <= this.SkinPrefab.Length))
            {
                ...
                if (element != null)
                {
                    for (int i = 0; i < element.AdvanceSkin.Length; i++)
                    {
                        ...
                        {
                            prefabPath = element.AdvanceSkin[i].ArtSkinPrefabLOD[index];
                            return true;
                        }
                    }
                }
        ...
        public string GetArtPrefabName(int skinId = 0, int InLOD = -1)
        {
            ...
            if ((skinId >= 1) && (skinId <= this.SkinPrefab.Length))
            {
                return this.SkinPrefab[skinId - 1].ArtSkinPrefabLOD[index];
            }
            return this.ArtPrefabLOD[index];
        }
            ...
            if (mesPrefabs != null)
            {
                ...
                if ((skinId >= 1) && (skinId <= this.SkinPrefab.Length))
                {
                    ...
                    if (element != null)
                    {
                        for (int i = 0; i < element.AdvanceSkin.Length; i++)
                        {
                            if ((element.AdvanceSkin[i] != null) && !string.IsNullOrEmpty(element.AdvanceSkin[i].ArtSkinPrefabLOD[index]))
                            {
                                AssetLoadBase item = new AssetLoadBase();
                                item.assetPath = element.AdvanceSkin[i].ArtSkinPrefabLOD[index];
                                mesPrefabs.Add(item);
                            }
                        }
                    }
