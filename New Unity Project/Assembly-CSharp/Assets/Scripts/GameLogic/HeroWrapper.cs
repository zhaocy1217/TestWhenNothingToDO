    ...
    public class HeroWrapper : ObjWrapper
    {
        ...
        public override void AddDisableSkillFlag(SkillSlotType _type, bool bForce = false)
        {
            ...
            if (_type == SkillSlotType.SLOT_SKILL_COUNT)
            {
                for (int i = 0; i < 10; i++)
                {
                    if ((base.actor.SkillControl.DisableSkill[i] == 1) && (!base.actor.SkillControl.IsIngnoreDisableSkill((SkillSlotType) i) || bForce))
                    {
            ...
            else if ((base.actor.SkillControl.DisableSkill[(int) _type] == 1) && (!base.actor.SkillControl.IsIngnoreDisableSkill(_type) || bForce))
            {
            ...
            if (bForce)
            {
                base.actor.SkillControl.SetForceDisableSkillSlot(_type, true);
            }
        }
        ...
        public override void Born(ActorRoot owner)
        {
            ...
            base.actor.PetControl = base.actor.CreateLogicComponent<PetComponent>(base.actor);
        ...
        public override void Fight()
        {
            ...
            if ((base.actor.HorizonMarker != null) && FogOfWar.enable)
            {
                base.actor.HorizonMarker.SightRadius = Horizon.QueryGlobalSight();
            }
        }
        ...
        private void OnAdvanceSkin()
        {
            if ((this.m_skinId > 0) && (base.actor.ValueComponent.actorSoulLevel > 1))
            {
                ...
                {
                    bool isInit = false;
                    GameObject inActorMesh = MonoSingleton<SceneMgr>.GetInstance().GetPooledGameObjLOD(prefabPath, false, SceneObjType.ActionRes, Vector3.get_zero(), Quaternion.get_identity(), out isInit);
                    if (inActorMesh != null)
                    {
                        inActorMesh.get_transform().SetParent(base.actor.myTransform);
                        inActorMesh.get_transform().set_localPosition(Vector3.get_zero());
                        inActorMesh.get_transform().set_localRotation(Quaternion.get_identity());
        ...
        {
            base.OnDead();
            PoolObjHandle<ActorRoot> handle = (base.myLastAtker == 0) ? base.myLastAtker : this.myLastAtker.handle.ActorControl.GetOrignalActor();
            if (((handle != 0) && (handle.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)) || base.IsKilledByHero())
            {
                this.contiKillNum = 0;
            }
            this.contiDeadNum++;
        ...
        public override void RmvDisableSkillFlag(SkillSlotType _type, bool bForce = false)
        {
            ...
            if (_type == SkillSlotType.SLOT_SKILL_COUNT)
            {
                for (int i = 0; i < 10; i++)
                {
                    if ((base.actor.SkillControl.DisableSkill[i] == 0) && (!base.actor.SkillControl.IsIngnoreDisableSkill((SkillSlotType) i) || bForce))
                    {
                        DefaultSkillEventParam param = new DefaultSkillEventParam((SkillSlotType) i, 0, base.actorPtr);
            ...
            else if ((base.actor.SkillControl.DisableSkill[(int) _type] == 0) && (!base.actor.SkillControl.IsIngnoreDisableSkill(_type) || bForce))
            {
                DefaultSkillEventParam param2 = new DefaultSkillEventParam(_type, 0, base.actorPtr);
            ...
            if (bForce)
            {
                base.actor.SkillControl.SetForceDisableSkillSlot(_type, false);
            }
        }
        ...
        public override void SetHelpToAttackTarget(PoolObjHandle<ActorRoot> helpActor, PoolObjHandle<ActorRoot> enemyActor)
        {
            ...
            base.m_needToHelpOtherWait = 0x3e8 + (FrameRandom.Random(8) * 500);
        }
        ...
        public override void SetToAttackTarget(PoolObjHandle<ActorRoot> enemyActor, bool ignoreSrchRange = false)
        {
            ...
            base.m_needSwithTargetWait = 0x3e8 + (FrameRandom.Random(8) * 500);
        }
