namespace Assets.Scripts.GameLogic
{
    ...
    public class PathIndicator : MonoBehaviour
    {
        ...
        private void GetParFromCache(int inNeedNum)
        {
            ...
            else
            {
                while (num >= this.m_parCachePool.Count)
                {
                    ...
                    {
                        bool isInit = false;
                        GameObject obj3 = MonoSingleton<SceneMgr>.GetInstance().GetPooledGameObjLOD("Prefab_Skill_Effects/tongyong_effects/UI_fx/yidong_ui_blue01.prefab", true, SceneObjType.Temp, Vector3.get_zero(), Quaternion.get_identity(), out isInit);
                        if (obj3 != null)
                        {
        ...
        private void ReturnToParCache(int inReturnNum)
        {
            ...
            if (num3 >= 50)
            {
                ...
                for (int j = num4; j > num5; j--)
                {
                    ...
                    if (pooledGameObject != null)
                    {
                        DebugHelper.Assert(!pooledGameObject.get_activeInHierarchy());
                        Singleton<CGameObjectPool>.GetInstance().RecycleGameObject(pooledGameObject);
                    }
        ...
        private void Update()
        {
            ...
            {
                Vector3 location = Vector3.get_zero();
                if (this.SrcActorHandle != 0)
                {
                    location = (Vector3) this.SrcActorHandle.handle.location;
                }
                else if (this.SrcObj != null)
                {
                    location = this.SrcObj.get_transform().get_position();
                }
                ...
                {
                    destPosition = this.DestObj.get_transform().get_position();
                }
                Vector3 vector5 = destPosition - location;
                int num = (int) (Mathf.Sqrt(vector5.get_sqrMagnitude()) / 1.15f);
                ...
                if (num > 0)
                {
                    Vector3 vector3 = (destPosition - location).get_normalized();
                    bool flag = true;
                    for (int i = 0; i <= this.m_parCacheIndex; i++)
                    {
                        ...
                        if (obj2 != null)
                        {
                            obj2.get_transform().set_forward(vector3);
                            Vector3 vector4 = Vector3.get_one();
                            if (flag)
                            {
                                vector4 = destPosition + ((Vector3) (((i + 1) * 1.15f) * -vector3));
                            }
                            else
                            {
                                vector4 = location + ((Vector3) (((i + 1) * 1.15f) * vector3));
                            }
                            vector4.y = 0.1f;
                            obj2.get_transform().set_position(vector4);
                        }
                    }
                }
            }
