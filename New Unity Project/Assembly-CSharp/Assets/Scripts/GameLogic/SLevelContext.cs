namespace Assets.Scripts.GameLogic
{
    ...
    public class SLevelContext
    {
        ...
        {
            CSDT_CAMP_EXT_GUILDMATCH[] csdt_camp_ext_guildmatchArray = new CSDT_CAMP_EXT_GUILDMATCH[2];
            for (int i = 0; i < 2; i++)
            {
                ...
                csdt_camp_ext_guildmatchArray[i] = this.m_campExtInfos[i].stExtData.stGuildMatch;
            }
            return csdt_camp_ext_guildmatchArray;
        }
        ...
        {
            if ((!this.IsGameTypeLadder() && !this.IsGameTypePvpMatch()) && ((!this.IsGameTypeRewardMatch() && !this.IsGameTypeEntertainment()) && !this.IsGameTypeGuildMatch()))
            {
                return false;
            }
            return true;
        ...
        public bool IsRedBagMode()
        {
            ...
            if (!this.m_isWarmBattle && Singleton<GamePlayerCenter>.GetInstance().IsHostPlayerHasCpuEnemy())
            {
                return false;
            }
            return true;
