namespace Assets.Scripts.GameLogic
{
    ...
    public class BuffProtectRule
    {
        ...
        {
            int num = _buffSkill.GetSkillFuncParam(30, 1, false);
            int num2 = _buffSkill.GetSkillFuncParam(30, 2, false);
            if (num == 0)
            {
                return true;
            }
        ...
        private bool NoDamageImpl(ref HurtDataInfo _hurt)
        {
            ...
            for (int i = 0; i < this.NoHurtBuffList.Count; i++)
            {
                ...
                if (skill != null)
                {
                    if (skill.cfgData.bEffectSubType == 6)
                    {
                    ...
                    {
                        if ((skill.cfgData.bEffectSubType == 4) && this.CheckTargetNoDamage(ref _hurt, skill))
                        {
                            return true;
                        }
                    }
                    else if (((_hurt.hurtType == HurtTypeDef.MagicHurt) && (skill.cfgData.bEffectSubType == 5)) && this.CheckTargetNoDamage(ref _hurt, skill))
                    {
                        return true;
                    }
                }
        ...
        public int ResistDamage(ref HurtDataInfo _hurt, int _hurtValue)
        {
            ...
            if (_hurtValue > 0)
            {
                ...
                if (_hurtValue > 0)
                {
                    ...
                    changeValue -= _hurtValue;
                    this.SendProtectEvent(3, -changeValue);
                }
            ...
            if ((this.buffHolder != null) && (this.buffHolder.actor != null))
            {
                ...
                for (int i = 0; i < actor.BuffHolderComp.SpawnedBuffList.Count; i++)
                {
                    ...
                    if ((inBuff != null) && inBuff.FindSkillFunc(0x36, out outSkillFunc))
                    {
                        ...
                        if (inBuff.GetSkillFuncParam(outSkillFunc, 1, false) == 0)
                        {
                        ...
                        else
                        {
                            ...
                            param3.bExposing = inBuff.skillContext.bExposing;
                    ...
                    {
                        int num4 = inBuff.GetSkillFuncParam(outSkillFunc, 0, false);
                        int num5 = inBuff.GetSkillFuncParam(outSkillFunc, 4, false);
                        switch (num4)
                        {
                            case 1:
