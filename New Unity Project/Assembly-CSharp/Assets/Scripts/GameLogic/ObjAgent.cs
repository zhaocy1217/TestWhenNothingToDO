namespace Assets.Scripts.GameLogic
{
    ...
    public class ObjAgent : BTBaseAgent, IPooledMonoBehaviour, IActorComponent
    {
        ...
        private PoolObjHandle<Action> m_currentAction = new PoolObjHandle<Action>();
        ...
        public int GerPathIndexBuTowerId(uint TowerID)
        {
            ...
            {
                PoolObjHandle<ActorRoot> handle = towerActors[i];
                if (handle.handle.ObjID == TowerID)
                {
                    return handle.handle.ObjLinker.PathIndex;
                }
            }
            return 0;
        }
        ...
        public int GetActorHPPercent(uint objID)
        {
            ...
            if (actor != 0)
            {
                return ((actor.handle.ValueComponent.actorHp * 0x2710) / actor.handle.ValueComponent.actorHpTotal);
            }
            return 0;
        }
        ...
        public uint GetBigDragon()
        {
            ...
            {
                PoolObjHandle<ActorRoot> handle = Singleton<GameObjMgr>.instance.GameActors[i];
                if ((handle != 0) && !handle.handle.ActorControl.IsDeadState)
                {
                    MonsterWrapper actorControl = handle.handle.ActorControl as MonsterWrapper;
                    if ((actorControl != null) && (actorControl.cfgInfo.bSoldierType == 8))
                    {
        ...
        public int GetCampIndex()
        {
            ...
            {
                num = Singleton<ActorDataCenter>.instance.GetActorDataProvider(GameActorDataProviderType.ServerDataProvider).Fast_GetActorServerDataBornIndex(ref this.m_wrapper.actor.TheActorMeta);
            }
            return num;
        }
        ...
        {
            PoolObjHandle<ActorRoot> handle = Singleton<TargetSearcher>.instance.GetCommanderTowerTarget(this.m_wrapper.actor.TheActorMeta.ActorCamp, friendrange, enemyRange);
            if (handle != 0)
            {
                return handle.handle.ObjID;
            }
        ...
        public Vector3 GetCurRouteLastForward()
        {
            ...
            return (endPoint.get_transform().get_position() - waypoint2.get_transform().get_position());
        }
        ...
        {
            ActorRoot root = Singleton<TargetSearcher>.GetInstance().GetNearestEnemy(this.m_wrapper.actor, srchR, filter, true);
            if (root != null)
            {
                return root.ObjID;
            }
        ...
        {
            ActorRoot root = Singleton<TargetSearcher>.GetInstance().GetNearestEnemyWithoutJungleMonsterWithoutActor((ActorRoot) this.m_wrapper.actorPtr, srchR, withOutActor);
            if (root != null)
            {
                return root.ObjID;
            }
        ...
        {
            ActorRoot root = Singleton<TargetSearcher>.GetInstance().GetNearestEnemyWithPriorityWithoutJungleMonster(this.m_wrapper.actor, srchR, priotity);
            if (root != null)
            {
                return root.ObjID;
            }
        ...
        public Vector3 GetNearestShenfuInRange(int range)
        {
            ...
            {
                if (pair.Value.ShenFu.get_activeSelf())
                {
                    Vector3 position = pair.Value.ShenFu.get_transform().get_position();
        ...
        public uint GetNewTargetByPriority(uint objID, int srchR)
        {
            ...
            if (actor != 0)
            {
                ...
                ActorRoot root = Singleton<TargetSearcher>.GetInstance().GetNearestEnemyWithTwoPriorityWithoutJungleMonster(this.m_wrapper.actor, srchR, TargetPriority.TargetPriority_Monster, TargetPriority.TargetPriority_Organ);
                if ((root != null) && (root.ObjID != objID))
                {
                    return root.ObjID;
                }
            }
        ...
        public uint GetNewTargetByPriorityForSiege(uint objID, int srchR)
        {
            ...
            if (actor != 0)
            {
                ...
                ActorRoot root = Singleton<TargetSearcher>.GetInstance().GetNearestEnemyWithTwoPriorityWithoutJungleMonster(this.m_wrapper.actor, srchR, TargetPriority.TargetPriority_Organ, TargetPriority.TargetPriority_Monster);
                if ((root != null) && (root.ObjID != objID))
                {
                    return root.ObjID;
                }
        ...
        public Vector3 GetRandomPointByGivenPoint(Vector3 aimPos, int range, int distance)
        {
            ...
            Vector3 vector2 = this.m_wrapper.actor.location.vec3 + ((Vector3) (vector.get_normalized() * (distance * 0.001f)));
        ...
        public uint GetSmallDragon()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    MonsterWrapper actorControl = handle.handle.ActorControl as MonsterWrapper;
                    if ((actorControl != null) && (actorControl.cfgInfo.bSoldierType == 9))
                    {
                        return handle.handle.ObjID;
                    }
                }
        ...
        public int GetWaitFrame(int frame)
        {
            ...
            if (this.m_wrapper.actor.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ)
            {
                return (((num - 1) / 5) + 1);
            }
            return (((num - 1) / 6) + 1);
        ...
        public EBTStatus IsAroundTeamThanStrongThanEnemise(int srchR, int strengthRate)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                ActorRoot actor = handle.handle;
                VInt3 num11 = actor.location - this.m_wrapper.actor.location;
                if (num11.sqrMagnitudeLong2D < num)
                {
                    if (this.m_wrapper.actor.IsSelfCamp(actor))
                    {
            ...
            ulong num9 = (ulong) ((num3 * num6) * 0x2710L);
            ulong num10 = (ulong) ((num5 * num4) * strengthRate);
            if (num9 > num10)
            {
                return EBTStatus.BT_SUCCESS;
            }
        ...
        public EBTStatus IsDangerUnderEnemyBuilding(uint targetHeroId, int srchR)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                PoolObjHandle<ActorRoot> handle = heroActors[i];
                ActorRoot actor = handle.handle;
                if (actor.ObjID == targetHeroId)
                {
                    root = actor;
                }
                VInt3 num8 = actor.location - this.m_wrapper.actor.location;
            ...
            {
                int num7 = (root.ValueComponent.actorHp * 0x2710) / root.ValueComponent.actorHpTotal;
                if (((num7 >= 0xbb8) && (num7 < 0x1388)) && (num3 > num4))
                {
                    return EBTStatus.BT_FAILURE;
                }
                if ((num7 < 0xbb8) && (num3 >= num4))
                {
                    return EBTStatus.BT_FAILURE;
                }
                if ((num7 >= 0x1388) && (num3 > (num4 + 1)))
                {
                    return EBTStatus.BT_FAILURE;
                }
                return EBTStatus.BT_SUCCESS;
        ...
        {
            if (!this.m_wrapper.IsNeedToHelpOther)
            {
                return false;
            }
            if (this.m_wrapper.m_needToHelpTarget == 0)
        ...
        public bool IsOutOfPursuitRange(Vector3 aimPos)
        {
            ...
            if (wrapper != null)
            {
                PursuitInfo pursuit = wrapper.Pursuit;
                if ((pursuit != null) && pursuit.IsVaild())
                {
                    VInt3 lhs = this.m_wrapper.actorLocation - pursuit.PursuitOrigin;
                    long num3 = lhs.sqrMagnitudeLong2D;
                    lhs.Normalize();
                    int num4 = pursuit.PursuitAngle / 2;
                    if ((num3 >= (pursuit.PursuitRadius * pursuit.PursuitRadius)) || ((num4 < 180) && (VInt3.DotXZLong(ref lhs, ref pursuit.PursuitDir) <= pursuit.CosHalfAngle)))
                    {
                        return true;
                    }
                }
                else if (wrapper.cfgInfo != null)
                {
                    int iPursuitR = wrapper.cfgInfo.iPursuitR;
                    VInt3 num6 = this.m_wrapper.actorLocation - num;
                    return (num6.sqrMagnitudeLong2D >= (iPursuitR * iPursuitR));
                }
            }
            return false;
        }
        ...
        {
            if (Singleton<BattleLogic>.GetInstance().GetCurLvelContext().m_isWarmBattle)
            {
                return EBTStatus.BT_SUCCESS;
            }
            return EBTStatus.BT_FAILURE;
        ...
        public virtual EBTStatus MoveToCommonAttackTargetWithRange(int range)
        {
            if (this.m_wrapper != null)
            {
                ...
                if ((nextSkill != null) && (curSkillUseInfo.AppointType == SkillRangeAppointType.Target))
                {
                    ...
                    Player ownerPlayer = ActorHelper.GetOwnerPlayer(ref this.m_wrapper.actorPtr);
                    HeroWrapper wrapper = this.m_wrapper as HeroWrapper;
                    if ((ownerPlayer != null) && (wrapper != null))
                    {
                        if ((wrapper.CurOpMode == OperateMode.LockMode) || (ownerPlayer.useLastHitMode == LastHitMode.LastHit))
                        {
                            range = wrapper.ChaseRange;
                        }
                        else
                        {
                            range = wrapper.SearchRange;
                        }
                    }
                    long num2 = range * range;
                    BaseAttackMode currentAttackMode = this.m_wrapper.GetCurrentAttackMode();
                    if ((currentAttackMode == null) || (currentAttackMode.GetEnemyHeroAttackTargetID() <= 0))
                    {
        ...
        public void NotifyTeamSelectRandomRoute(int range, uint centerObjId)
        {
            ...
            for (int i = 0; i < heroActors.Count; i++)
            {
                ...
                if (handle2.handle.TheActorMeta.ActorCamp == this.m_wrapper.actor.TheActorMeta.ActorCamp)
                {
                    VInt3 num4 = handle2.handle.location - actor.handle.location;
                    if (num4.sqrMagnitudeLong2D < num)
                    {
                        handle2.handle.ActorAgent.SelectRoute();
                    }
                }
            }
        }
        ...
        public void NotifyTeamSelectRoute(int index, int range, uint centerObjId)
        {
            ...
            if (((waypointsList != null) && (waypointsList.Count != 0)) && (index >= 0))
            {
                List<PoolObjHandle<ActorRoot>> heroActors = Singleton<GameObjMgr>.GetInstance().HeroActors;
                for (int i = 0; i < waypointsList.Count; i++)
                {
                    if ((waypointsList[i] != null) && (waypointsList[i].m_index == index))
                    {
                        ...
                        {
                            PoolObjHandle<ActorRoot> handle2 = heroActors[j];
                            if (handle2.handle.TheActorMeta.ActorCamp == this.m_wrapper.actor.TheActorMeta.ActorCamp)
                            {
                                VInt3 num5 = handle2.handle.location - actor.handle.location;
                                if (num5.sqrMagnitudeLong2D < num2)
                                {
                                    handle2.handle.ActorControl.m_curWaypointsHolder = waypointsList[i];
                                    handle2.handle.ActorControl.m_curWaypointTarget = handle2.handle.ActorControl.m_curWaypointsHolder.startPoint;
                                    handle2.handle.ActorControl.m_curWaypointTargetPosition = new VInt3(handle2.handle.ActorControl.m_curWaypointTarget.get_transform().get_position());
                                }
                            }
                        }
                        break;
        ...
        public EBTStatus PlayAgeAction(string actionName)
        {
            ...
            GameObject[] objArray1 = new GameObject[] { base.get_gameObject() };
            this.m_currentAction = new PoolObjHandle<Action>(ActionManager.Instance.PlayAction(actionName, true, false, objArray1));
            SkillUseContext context = new SkillUseContext();
            context.Originator = this.m_wrapper.actorPtr;
        ...
        public EBTStatus PlayDeadAgeAction()
        {
            ...
            GameObject[] objArray1 = new GameObject[] { base.get_gameObject() };
            this.m_currentAction = new PoolObjHandle<Action>(ActionManager.Instance.PlayAction(deadAgePath, true, false, objArray1));
            if (this.m_currentAction == 0)
            {
                return EBTStatus.BT_FAILURE;
            }
        ...
        public EBTStatus PlayDeadMoveToPositionAgeAction()
        {
            ...
            GameObject[] objArray1 = new GameObject[] { base.get_gameObject(), this.m_wrapper.m_deadPointGo };
            this.m_currentAction = new PoolObjHandle<Action>(ActionManager.Instance.PlayAction(deadAgePath, true, false, objArray1));
            if (this.m_currentAction == 0)
            {
                return EBTStatus.BT_FAILURE;
        ...
        {
            ActionHelper component = base.get_gameObject().GetComponent<ActionHelper>();
            if (component == null)
            {
                return EBTStatus.BT_FAILURE;
            }
            ...
            this.m_currentAction = new PoolObjHandle<Action>(component.PlayAction(actionName));
            if (this.m_currentAction == 0)
            {
                return EBTStatus.BT_FAILURE;
            }
        ...
        public virtual void RealMoveInMoveAttack(uint objID, int range)
        {
            ...
            if (actor != 0)
            {
                ...
                {
                    int num = (this.m_wrapper.actor.ValueComponent.actorHp * 0x2710) / this.m_wrapper.actor.ValueComponent.actorHpTotal;
                    int num2 = (actor.handle.ValueComponent.actorHp * 0x2710) / this.m_wrapper.actor.ValueComponent.actorHpTotal;
                    if ((num > 0xbb8) && (num2 < 0x3e8))
                    {
                        flag = true;
                    }
                }
                ...
                {
                    VInt3 num3 = this.m_wrapper.actor.location - actor.handle.location;
                    long num4 = range - num3.magnitude2D;
                    num3.y = 0;
                    num3 = num3.NormalizeTo(0x3e8);
                    VInt3 rhs = !(num3 == VInt3.zero) ? num3 : VInt3.forward;
                    if ((actor.handle.ActorControl.GetActorSubType() != 2) && (Math.Abs(num4) >= 0x3e8L))
                    {
                        VInt3 dest = this.m_wrapper.actor.location + ((VInt3) ((rhs * num4) / 1000f));
                        this.m_wrapper.RealMovePosition(dest, 0);
                    }
                    else
                    {
                        int num7 = FrameRandom.Random(2) * 0x2710;
                        VInt3 num8 = VInt3.Cross(VInt3.up, rhs);
                        int num9 = (num7 - 0x1388) / 0x1388;
                        num8 *= num9;
                        if (actor.handle.ActorControl.GetActorSubType() == 2)
                        {
                            num8 -= rhs;
                            num8.NormalizeTo(0x3e8);
                        }
                        int num10 = 0x7d0 + FrameRandom.Random(500);
                        VInt3 num11 = this.m_wrapper.actor.location + ((VInt3) ((num8 * num10) / 1000f));
                        this.m_wrapper.RealMovePosition(num11, 0);
                    }
                }
            }
        }
        ...
        public void ResetEndurance()
        {
            ...
            if ((wrapper != null) && (wrapper.cfgInfo != null))
            {
                wrapper.Endurance = wrapper.cfgInfo.iPursuitE;
            }
        }
        ...
        {
            if (((this.m_wrapper.m_curWaypointsHolder != null) && (this.m_wrapper.m_curWaypointsHolder.startPoint != null)) && (this.m_wrapper.m_curWaypointTarget.get_transform() != null))
            {
                this.m_wrapper.m_curWaypointTarget = this.m_wrapper.m_curWaypointsHolder.startPoint;
                this.m_wrapper.m_curWaypointTargetPosition = new VInt3(this.m_wrapper.m_curWaypointTarget.get_transform().get_position());
            }
            return EBTStatus.BT_SUCCESS;
        }
        ...
        public EBTStatus SelectNearestRoute()
        {
            ...
            for (int i = 0; i < waypointsList.Count; i++)
            {
                VInt3 num3 = new VInt3(waypointsList[i].startPoint.get_transform().get_position());
        ...
        {
            int num = Singleton<ActorDataCenter>.instance.GetActorDataProvider(GameActorDataProviderType.ServerDataProvider).Fast_GetActorServerDataBornIndex(ref this.m_wrapper.actor.TheActorMeta);
            ListView<WaypointsHolder> waypointsList = Singleton<BattleLogic>.GetInstance().mapLogic.GetWaypointsList(this.m_wrapper.actor.TheActorMeta.ActorCamp);
            if ((waypointsList == null) || (waypointsList.Count == 0))
            {
                return EBTStatus.BT_INVALID;
        ...
        public void StopCurAgeAction()
        {
            if (this.m_isActionPlaying)
            {
                if (this.m_currentAction != 0)
                {
                    ActionManager.Instance.StopAction((Action) this.m_currentAction);
                }
                this.m_isActionPlaying = false;
            }
        }
            ...
            if (!base.m_isPaused)
            {
                ...
                if (this.m_wrapper != null)
                {
                    ...
                    if (actorType == ActorTypeDef.Actor_Type_Hero)
                    {
                        if (this.m_wrapper.myBehavior == ObjBehaviMode.State_AutoAI)
                        {
                            if (((this.m_frame + this.m_wrapper.actor.ObjID) % (3 * num)) == 0)
                            {
                    ...
                    else
                    {
                        ...
                        if (((wrapper != null) && (wrapper.cfgInfo != null)) && (wrapper.cfgInfo.bIsBoss > 0))
                        {
                            if (((this.m_frame + this.m_wrapper.actor.ObjID) % (4 * num)) == 0)
                            {
                                base.UpdateLogic(delta);
                            }
                        }
                        ...
                        {
                            if (((this.m_frame + this.m_wrapper.actor.ObjID) % (5 * num)) == 0)
                            {
                                base.UpdateLogic(delta);
                            }
                        }
                        else if (((this.m_frame + this.m_wrapper.actor.ObjID) % (6 * num)) == 0)
                        {
                            base.UpdateLogic(delta);
                        }
                    }
