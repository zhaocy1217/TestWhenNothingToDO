namespace Assets.Scripts.UI
{
    ...
    public class CUIHttpTextScript : CUIComponent
    {
        ...
        {
            <DownloadText>c__Iterator38 iterator = new <DownloadText>c__Iterator38();
            iterator.url = url;
            iterator.<$>url = url;
        ...
        public override void Initialize(CUIFormScript formScript)
        {
            if (!base.m_isInitialized)
            {
                ...
                this.m_scrollRectScript = CUIUtility.GetComponentInChildren<ScrollRect>(base.get_gameObject());
                this.m_textScript = (this.m_scrollRectScript == null) ? null : CUIUtility.GetComponentInChildren<Text>(this.m_scrollRectScript.get_gameObject());
                Transform transform = base.get_gameObject().get_transform().FindChild("Title");
                this.m_titleTextScript = (transform == null) ? null : CUIUtility.GetComponentInChildren<Text>(transform.get_gameObject());
                this.m_httpTextState = enHttpTextState.Unload;
                if (this.m_loadingCover != null)
                {
                    this.m_loadingCover.CustomSetActive(true);
                }
                if (base.get_gameObject().get_activeInHierarchy() && !string.IsNullOrEmpty(this.m_textUrl))
                {
        ...
        public void SetTextUrl(string url, bool forceReset = false)
        {
            if (!string.IsNullOrEmpty(url) && (!string.Equals(url, this.m_textUrl) || forceReset))
            {
                ...
                if (base.get_gameObject().get_activeInHierarchy() && (this.m_httpTextState == enHttpTextState.Loading))
                {
                    base.StopAllCoroutines();
                }
                this.m_httpTextState = enHttpTextState.Unload;
        ...
        [CompilerGenerated]
        private sealed class <DownloadText>c__Iterator38 : IDisposable, IEnumerator, IEnumerator<object>
        {
                ...
                switch (num)
                {
                    ...
                    case 1:
                        ...
                        if (string.IsNullOrEmpty(this.<www>__0.get_error()))
                        {
                            ...
                            this.<text>__1 = this.<www>__0.get_text();
                            ...
                            {
                                this.<>f__this.m_titleTextScript.set_text(this.<title>__2);
                            }
                            if (this.<>f__this.m_textScript != null)
                            {
                                this.<>f__this.m_textScript.set_text(this.<content>__3);
                                this.<>f__this.m_textScript.get_rectTransform().set_anchoredPosition(new Vector2(this.<>f__this.m_textScript.get_rectTransform().get_anchoredPosition().x, 0f));
                            }
                        }
                        this.$PC = -1;
                        break;
