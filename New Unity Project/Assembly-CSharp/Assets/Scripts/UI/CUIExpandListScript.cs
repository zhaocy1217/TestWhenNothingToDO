    ...
    public class CUIExpandListScript : CUIListScript
    {
        ...
        public override void SelectElement(int index, bool isDispatchSelectedChangeEvent = true)
        {
            if (this.m_selectingState == enExpandListSelectingState.None)
            {
                ...
                this.m_contentAnchoredPosition = base.m_contentRectTransform.get_anchoredPosition();
                this.m_timeSlice = 0f;
                if (base.m_lastSelectedElementIndex >= 0)
                {
        ...
        public void SelectElementImmediately(int index)
        {
            ...
            for (int i = 0; i < base.m_elementAmount; i++)
            {
                stRect item = this.LayoutExpandElement(i, (i != index) ? ((float) 0) : ((float) 1), ref this.m_contentSize, ref offset);
                if (i < base.m_elementsRect.Count)
                {
                    base.m_elementsRect[i] = item;
                }
            ...
            {
                base.m_contentRectTransform.set_anchoredPosition(this.GetTargetContentAnchoredPosition(index));
            }
        ...
        protected override void Update()
        {
            if ((base.m_belongedFormScript == null) || !base.m_belongedFormScript.IsClosed())
            {
                if (this.m_selectingState != enExpandListSelectingState.None)
                {
                    if (base.m_scrollRect.get_enabled())
                    {
                ...
                else if (!base.m_scrollRect.get_enabled())
                {
                    base.m_scrollRect.StopMovement();
        ...
        private void UpdateSelectedElement(enExpandListSelectingState selectingState)
        {
            ...
            switch (selectingState)
            {
                case enExpandListSelectingState.Retract:
                    ...
                    this.m_timeSlice += Time.get_deltaTime();
                    base.m_contentSize = Vector2.get_zero();
                    ...
                    if (base.m_listType != enUIListType.Horizontal)
                    {
                        if (base.m_listType == enUIListType.Vertical)
                        {
                            vector.y += base.m_elementLayoutOffset;
                        }
                        break;
                    }
                    vector.x += base.m_elementLayoutOffset;
                    break;
                ...
                case enExpandListSelectingState.Move:
                    ...
                    else if (base.m_listType != enUIListType.Horizontal)
                    {
                        if (base.m_listType == enUIListType.Vertical)
                        {
                            ...
                            this.m_contentAnchoredPosition.y += (Time.get_deltaTime() * this.m_contentFixingSpeed) * num4;
                    ...
                    else
                    {
                        ...
                        this.m_contentAnchoredPosition.x += (Time.get_deltaTime() * this.m_contentFixingSpeed) * num3;
                ...
                case enExpandListSelectingState.Expand:
                    ...
                    else
                    {
                        ...
                        for (int j = 0; j < base.m_elementAmount; j++)
                        {
                            float expandedRate = 0f;
                            if (j == base.m_selectedElementIndex)
                            {
                                expandedRate = this.m_timeSlice / this.m_expandedTime;
                                expandedRate = Mathf.Clamp(expandedRate, 0f, 1f);
                            }
                            stRect item = this.LayoutExpandElement(j, expandedRate, ref this.m_contentSize, ref offset);
            ...
            for (int i = 0; i < base.m_elementAmount; i++)
            {
                ...
                stRect rect = this.LayoutExpandElement(i, num2, ref this.m_contentSize, ref vector);
                if (i < base.m_elementsRect.Count)
                {
        ...
        Label_055A:
            ...
            base.m_contentRectTransform.set_anchoredPosition(this.m_contentAnchoredPosition);
        }
    }
}
