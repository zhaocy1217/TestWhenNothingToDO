                ...
                if (num >= 6)
                {
                    ...
                    if (((num2 >= 6) && (num2 <= num)) && (CMemoryManager.ReadShort(data, ref offset) == 0x2713))
                    {
                        ...
                        for (int i = 0; i < num4; i++)
                        {
                            ...
                            info.m_key = CMemoryManager.ReadString(data, ref offset);
                            info.m_width = CMemoryManager.ReadShort(data, ref offset);
                            info.m_height = CMemoryManager.ReadShort(data, ref offset);
                            info.m_lastModifyTime = CMemoryManager.ReadDateTime(data, ref offset);
                            info.m_isGif = CMemoryManager.ReadByte(data, ref offset) > 0;
