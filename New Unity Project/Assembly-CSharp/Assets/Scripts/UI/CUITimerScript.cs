    ...
    public class CUITimerScript : CUIComponent
    {
        ...
        public override void Initialize(CUIFormScript formScript)
        {
            if (!base.m_isInitialized)
            {
                ...
                this.m_timerText = base.GetComponentInChildren<Text>(base.get_gameObject());
                if ((this.m_timerDisplayType == enTimerDisplayType.None) && (this.m_timerText != null))
                {
                    this.m_timerText.get_gameObject().CustomSetActive(false);
                }
        ...
        public void PauseTimer()
        {
            if (!this.m_isPaused)
            {
                this.m_pauseTime = Time.get_realtimeSinceStartup();
            ...
            if ((this.m_timerText != null) && (this.m_timerDisplayType != enTimerDisplayType.None))
            {
                ...
                switch (this.m_timerDisplayType)
                {
                    ...
                    case enTimerDisplayType.S:
                        this.m_timerText.set_text(string.Format("{0:D}", currentTime));
                        break;
                    ...
                    case enTimerDisplayType.D_H_M_S:
                    {
                        ...
                        this.m_timerText.set_text(string.Format("{0}天{1:D2}:{2:D2}:{3:D2}", args));
                        break;
                    }
        ...
        public void ResumeTimer()
        {
            if (this.m_isPaused)
            {
                this.m_pauseElastTime += Time.get_realtimeSinceStartup() - this.m_pauseTime;
        ...
        private void UpdateTimer()
        {
            ...
            switch (this.m_timerType)
            {
                case enTimerType.CountUp:
                    this.m_currentTime = (Time.get_realtimeSinceStartup() - this.m_startTime) - this.m_pauseElastTime;
                    flag = this.m_currentTime >= this.m_totalTime;
                    break;

                case enTimerType.CountDown:
                    this.m_currentTime = this.m_totalTime - ((Time.get_realtimeSinceStartup() - this.m_startTime) - this.m_pauseElastTime);
