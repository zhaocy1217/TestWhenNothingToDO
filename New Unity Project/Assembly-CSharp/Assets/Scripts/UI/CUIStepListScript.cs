namespace Assets.Scripts.UI
{
    ...
    public class CUIStepListScript : CUIListScript
    {
        ...
        private float GetElementScale(int index)
        {
            ...
            int num = (int) (rect.m_center.x + base.m_contentRectTransform.get_anchoredPosition().x);
            int num2 = (int) Mathf.Abs(num - this.m_stepListCenter);
        ...
        public override void Initialize(CUIFormScript formScript)
        {
            if (!base.m_isInitialized)
            {
                ...
                base.m_elementSpacing = Vector2.get_zero();
                ...
                {
                    this.m_scrollRectLastScrollSpeed = base.m_scrollRect.get_velocity().x;
                }
        ...
        {
            stRect rect = base.m_elementsRect[index];
            float num = rect.m_center.x + base.m_contentRectTransform.get_anchoredPosition().x;
            return ((num > this.m_selectAreaMin) && (num < this.m_selectAreaMax));
        }

        protected override void ProcessElements()
        {
            base.m_contentSize = Vector2.get_zero();
            Vector2 offset = Vector2.get_zero();
            this.m_contentSize.x += this.m_contentExtendSize;
            offset.x += this.m_contentExtendSize;
            for (int i = 0; i < base.m_elementAmount; i++)
            {
                stRect item = base.LayoutElement(i, ref this.m_contentSize, ref offset);
        ...
        protected override void ResizeContent(ref Vector2 size, bool resetPosition)
        {
            ...
            {
                base.m_contentRectTransform.set_sizeDelta(size);
                base.m_contentRectTransform.set_pivot(new Vector2(0f, 0.5f));
                base.m_contentRectTransform.set_anchorMin(new Vector2(0f, 0.5f));
                base.m_contentRectTransform.set_anchorMax(new Vector2(0f, 0.5f));
                base.m_contentRectTransform.set_anchoredPosition(Vector2.get_zero());
                if (resetPosition)
                {
                    base.m_contentRectTransform.set_anchoredPosition(Vector2.get_zero());
                }
        ...
        public override void SelectElement(int index, bool isDispatchSelectedChangeEvent = true)
        {
            ...
            if ((index >= 0) && (index < base.m_elementAmount))
            {
                ...
                if (base.m_contentRectTransform.get_anchoredPosition().x != this.m_contentFixScrollTargetPosition)
                {
                    if (base.m_scrollRect != null)
                    {
                        base.m_scrollRect.StopMovement();
                    }
                    this.m_fixScrollSpeed = (this.m_contentFixScrollTargetPosition - base.m_contentRectTransform.get_anchoredPosition().x) / (this.m_fixScrollTime * GameFramework.c_renderFPS);
                    if (Mathf.Abs(this.m_fixScrollSpeed) < 0.001f)
                    {
                        base.m_contentRectTransform.set_anchoredPosition(new Vector2(this.m_contentFixScrollTargetPosition, base.m_contentRectTransform.get_anchoredPosition().y));
                    }
                    else
                    {
            ...
            if ((base.m_belongedFormScript == null) || !base.m_belongedFormScript.IsClosed())
            {
                ...
                if (!this.m_bDontUpdate && (base.m_scrollRect != null))
                {
                    for (int i = 0; i < base.m_elementAmount; i++)
                    {
                        ...
                        if (elemenet != null)
                        {
                            elemenet.get_gameObject().get_transform().set_localScale((Vector3) (Vector3.get_one() * this.GetElementScale(i)));
                        }
                    }
                    if ((this.m_fixingScroll && (base.m_selectedElementIndex >= 0)) && (base.m_selectedElementIndex < base.m_elementAmount))
                    {
                        base.m_scrollRect.set_enabled(false);
                        float num2 = base.m_contentRectTransform.get_anchoredPosition().x + this.m_fixScrollSpeed;
                        if (((num2 > this.m_contentFixScrollTargetPosition) && (this.m_fixScrollSpeed > 0f)) || ((num2 < this.m_contentFixScrollTargetPosition) && (this.m_fixScrollSpeed < 0f)))
                        {
                            base.m_contentRectTransform.set_anchoredPosition(new Vector2(this.m_contentFixScrollTargetPosition, base.m_contentRectTransform.get_anchoredPosition().y));
                        ...
                        {
                            base.m_contentRectTransform.set_anchoredPosition(new Vector2(num2, base.m_contentRectTransform.get_anchoredPosition().y));
                        }
                    }
                    else
                    {
                        base.m_scrollRect.set_enabled(true);
                        ...
                        {
                            if (((base.m_selectedElementIndex >= 0) && (base.m_selectedElementIndex < base.m_elementAmount)) && (base.m_contentRectTransform.get_anchoredPosition().x != this.m_contentFixScrollTargetPosition))
                            {
                                base.m_contentRectTransform.set_anchoredPosition(new Vector2(this.m_contentFixScrollTargetPosition, base.m_contentRectTransform.get_anchoredPosition().y));
                                for (int j = 0; j < base.m_elementAmount; j++)
                                {
                                    ...
                                    if (script2 != null)
                                    {
                                        script2.get_gameObject().get_transform().set_localScale((Vector3) (Vector3.get_one() * this.GetElementScale(j)));
                                    }
                                }
                            }
                        }
                        ...
                        {
                            float num3 = Mathf.Abs(base.m_scrollRect.get_velocity().x);
                            int index = -1;
                            if (base.m_contentRectTransform.get_anchoredPosition().x > 0f)
                            {
                                index = 0;
                            }
                            else if ((base.m_contentRectTransform.get_anchoredPosition().x + this.m_contentSize.x) < this.m_scrollAreaSize.x)
                            {
                        ...
                        this.m_scrollRectLastScrollSpeed = Mathf.Abs(base.m_scrollRect.get_velocity().x);
                    }
                    base.DetectScroll();
                }
            }
