namespace Assets.Scripts.UI
{
    ...
    public class CUIEventScript : CUIComponent, IPointerDownHandler, IEventSystemHandler, IPointerClickHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler, IPointerUpHandler
    {
        ...
        public void ExecuteClearInputStatus()
        {
            ...
            this.m_downPosition = Vector2.get_zero();
        ...
        {
            if ((this.m_canClick && (base.m_belongedFormScript != null)) && (base.m_belongedFormScript.ChangeScreenValueToForm(Vector2.Distance(eventData.get_position(), this.m_downPosition)) > 40f))
            {
                this.m_canClick = false;
            }
        ...
        public void OnPointerDown(PointerEventData eventData)
        {
            ...
            this.m_downTimestamp = Time.get_realtimeSinceStartup();
        ...
        private void Update()
        {
            if (!this.m_needClearInputStatus && this.m_isDown)
            {
                if (!this.m_isHold)
                {
                    if ((Time.get_realtimeSinceStartup() - this.m_downTimestamp) >= 1f)
                    {
                        this.m_isHold = true;
                        this.m_canClick = false;
