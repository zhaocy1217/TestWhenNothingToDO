namespace Assets.Scripts.GameSystem
{
    ...
    public class CFunctionUnlockSys : Singleton<CFunctionUnlockSys>
    {
        ...
        public void OpenUnlockTip(string tips, string icon)
        {
            ...
            if (formScript != null)
            {
                Text component = formScript.get_transform().FindChild("TipContentTxt").GetComponent<Text>();
                Image image = formScript.get_transform().FindChild("TipIconImg").GetComponent<Image>();
                formScript.GetComponent<CUIEventScript>().set_enabled(false);
                component.set_text(tips);
                image.SetSprite(CUIUtility.s_Sprite_Dynamic_FucUnlock_Dir + icon, formScript, true, false, false, false);
                image.SetNativeSize();
