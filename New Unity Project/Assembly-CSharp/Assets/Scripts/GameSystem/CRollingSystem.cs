    ...
    public class CRollingSystem : Singleton<CRollingSystem>
    {
        ...
        public static void ReceiveRollingMsgNtf(CSPkg msg)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                {
                    item.content = "<color=#b5e9ff>" + item.content + "</color>";
                }
        ...
        private void StartRolling(CUIAutoScroller autoScroller)
        {
            ...
            {
                RectTransform transform = autoScroller.get_transform() as RectTransform;
                if (transform != null)
                {
                    if (Singleton<CLoudSpeakerSys>.instance.IsLoudSpeakerShowing())
                    {
                        transform.set_anchorMin(new Vector2(0.661f, 0.5f));
                    }
                    else
                    {
                        transform.set_anchorMin(new Vector2(0.3f, 0.5f));
                    }
                }
