    ...
    public class CSkillData
    {
        ...
        public static int CalcEscapeValue(string escapeString, CHeroInfo heroInfo, int skillLevel, int heroSoulLevel, uint heroId)
        {
            ...
            for (int i = 0; i < strArray.Length; i++)
            {
                ...
                foreach (string str2 in str.Split(chArray2))
                {
                    ...
                    else if (ParseSkillProperty(str2, out num3, heroId))
                    {
                        list2.Add(num3);
                    }
        ...
        public static int CalcEscapeValue(string escapeString, ValueDataInfo[] valueData, int skillLevel, int heroSoulLevel, uint heroId)
        {
            ...
            for (int i = 0; i < strArray.Length; i++)
            {
                ...
                float item = (float) list2[0];
                for (int n = 0; n < list4.Count; n++)
                {
                    switch (list4[n])
                    {
                        case '*':
                            item *= (float) list2[n + 1];
                            break;
        ...
        public string GetSkillDesc(int slotId, uint heroId)
        {
            ...
            if (escapeString != null)
            {
                ...
                {
                    skillDesc = skillDesc.Replace("[" + escapeString[i] + "]", CalcEscapeValue(escapeString[i], info2, this.GetSkillLevel(slotId), 1, heroId).ToString());
                }
            }
            return skillDesc;
        }
        ...
        public string GetSkillUpTip(int slotId, uint heroId)
        {
            ...
            if (escapeString != null)
            {
                for (int i = 0; i < escapeString.Length; i++)
                {
                    int num2 = CalcEscapeValue(escapeString[i], info2, this.GetSkillLevel(slotId), 1, heroId);
        ...
        private static bool ParseSkillProperty(string s, out int value, uint heroId)
        {
            ...
            if (index != -1)
            {
                if (num2 != -1)
                {
                    ...
                    object[] inParameters = new object[] { num7, s, heroId.ToString() };
                    DebugHelper.Assert(dataByKey != null, "ResSkillCombineCfgInfo[{0}] can not be found! from string:\"{1}\" heroId[{2}]", inParameters);
                    if (dataByKey != null)
                    {
