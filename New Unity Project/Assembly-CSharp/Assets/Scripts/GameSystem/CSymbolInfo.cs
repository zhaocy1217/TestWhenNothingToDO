        ...
        public bool CheckAnyWearSymbol(out int posId, out uint symbolId, byte colorType = 0)
        {
            ...
            for (int i = 0; i < curUseableCount; i++)
            {
                ...
                if (useableByIndex.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMSYMBOL)
                {
                    ...
                    if (((item != null) && (item.m_stackCount > item.GetPageWearCnt(0))) && ((item.GetPageWearCnt(0) < CSymbolWearController.s_maxSameIDSymbolEquipNum) && this.GetWearPos(item, 0, out pos, out code)))
                    {
                        if (colorType != 0)
                        {
                            if ((item.m_SymbolData.bColor == colorType) && ((item2 == null) || (item.m_SymbolData.wLevel < item2.m_SymbolData.wLevel)))
                            {
                                item2 = item;
                                posId = pos;
                        ...
                        else if (((item2 == null) || (item.m_SymbolData.bColor < item2.m_SymbolData.bColor)) || ((item.m_SymbolData.bColor == item2.m_SymbolData.bColor) && (item.m_SymbolData.wLevel < item2.m_SymbolData.wLevel)))
                        {
                            item2 = item;
                            posId = pos;
                            symbolId = item.m_baseID;
