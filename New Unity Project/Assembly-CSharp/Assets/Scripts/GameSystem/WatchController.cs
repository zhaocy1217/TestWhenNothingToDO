namespace Assets.Scripts.GameSystem
{
    ...
    public class WatchController : Singleton<WatchController>
    {
        ...
        {
            if ((this._workMode == WorkMode.Observe) && (pkg.dwOffset == this._totalRecvSize))
            {
                if (pkg.bAllOver == 1)
                {
        ...
        public void UpdateFrame()
        {
            if (this.IsWatching)
            {
                ...
                {
                    if (Time.get_time() > (this._overTime + 10f))
                    {
                        this.Stop();
                        return;
                    }
        ...
        public bool IsObserving
        {
            get
            {
                return (this._workMode == WorkMode.Observe);
            }
        }
        ...
        public bool IsRelayCast
        {
            get
            {
                return ((this._workMode == WorkMode.Observe) || (this._workMode == WorkMode.Replay));
            }
        }
        ...
        public byte SpeedRate
        {
            ...
            set
            {
                this._speedRate = (byte) Mathf.Clamp(value, this.SpeedRateMin, this.SpeedRateMax);
                if (this.IsWatching && this._isRunning)
                {
                    Singleton<FrameSynchr>.GetInstance().FrameSpeed = this._speedRate;
                    Time.set_timeScale((float) this._speedRate);
                }
            }
        }
