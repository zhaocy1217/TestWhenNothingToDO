namespace Assets.Scripts.GameSystem
{
    ...
    public class CHeroSelectBaseSystem : Singleton<CHeroSelectBaseSystem>
    {
        ...
        public void Clear()
        {
            ...
            s_propArr = new int[0x25];
            s_propPctArr = new int[0x25];
        ...
        public Transform GetTeamPlayerElement(ulong playerUID, COM_PLAYERCAMP tarCamp)
        {
            ...
            if (this.uiType == enUIType.enNormal)
            {
                ...
                Transform transform2 = form.get_transform().Find("PanelRight/ListTeamHeroInfo");
                if (transform2 == null)
                {
                    return null;
                }
                CUIListScript component = transform2.get_gameObject().GetComponent<CUIListScript>();
                if (component == null)
                {
                    return null;
                }
                ...
                {
                    transform = component.GetElemenet(teamPlayerIndex).get_transform();
                }
                return transform;
            }
            if (this.uiType == enUIType.enBanPick)
            {
                ...
                {
                    transform3 = script3.get_transform().Find("PanelLeft/TeamHeroInfo");
                }
                else
                {
                    transform3 = script3.get_transform().Find("PanelRight/TeamHeroInfo");
                }
                if (transform3 == null)
                {
                    return null;
                }
                CUIListScript script4 = transform3.get_gameObject().GetComponent<CUIListScript>();
                if (script4 == null)
                {
                    return null;
                }
                ...
                {
                    transform = script4.GetElemenet(index).get_transform();
                }
            }
            return transform;
        }
        ...
        {
            for (int i = 0; i < stateInfo.bPosNum; i++)
            {
        ...
        {
            return (((this.gameType == enSelectGameType.enGuide) && (this.m_stGameOfAdventure != null)) && (this.m_stGameOfAdventure.iLevelID == GameDataMgr.globalInfoDatabin.GetDataByKey((uint) 120).dwConfValue));
        }
        ...
        {
            this.m_fOpenHeroSelectForm = Time.get_time();
        ...
        {
            <PostCombatSingleGame>c__AnonStorey54 storey = new <PostCombatSingleGame>c__AnonStorey54();
            ...
            GameDataMgr.heroDatabin.Accept(new Action<ResHeroCfgInfo>(storey.<>m__3E));
            ...
            for (COM_PLAYERCAMP com_playercamp3 = COM_PLAYERCAMP.COM_PLAYERCAMP_1; com_playercamp3 < COM_PLAYERCAMP.COM_PLAYERCAMP_COUNT; com_playercamp3 += 1)
            {
                ...
                for (int j = 0; j < view2.Count; j++)
                {
                    ...
                    if (info2.RoomMemberType == 2)
                    {
                        ...
                        {
                            <PostCombatSingleGame>c__AnonStorey55 storey2 = new <PostCombatSingleGame>c__AnonStorey55();
                            ...
                            if (Singleton<CHeroSelectBaseSystem>.instance.m_isAllowDupHero == 0)
                            {
                                while (((listArray[((int) com_playercamp3) - 1].FindIndex(new Predicate<uint>(storey2.<>m__3F)) != -1) || !flag) || (CSysDynamicBlock.bLobbyEntryBlocked && (storey2.heroCfg.bIOSHide == 1)))
                                {
                                    id = Random.Range(0, GameDataMgr.heroDatabin.Count());
                                    storey2.heroCfg = GameDataMgr.heroDatabin.GetDataByIndex(id);
                                    flag = GameDataMgr.IsHeroCanBePickByComputer(storey2.heroCfg.dwCfgID);
        ...
        public static void ReciveQuitSingleGame(CSPkg msg)
        {
            ...
            uiEvent.m_eventID = enUIEventID.ReplayKit_Pause_Recording;
        ...
        public void SavePveDefaultHeroList()
        {
            if (s_defaultBattleListInfo != null)
            {
                ...
                if (!flag)
                {
                    ...
                    item.dwBattleListID = this.m_battleListID;
                    item.stBattleList = new COMDT_BATTLEHERO();
        ...
        public void SetPveDataWithArena(uint battleListID, CSDT_SINGLE_GAME_OF_ARENA reportInfo = new CSDT_SINGLE_GAME_OF_ARENA(), string levleName = "")
        {
            this.m_battleListID = battleListID;
        ...
        public static void StartPvpHeroSelectSystem(COMDT_DESKINFO deskInfo, CSDT_CAMPINFO[] campInfo, COMDT_FREEHERO freeHero, COMDT_FREEHERO_INACNT freeHeroSymbol)
        {
            ...
            {
                enNull = enSelectGameType.enGuildMatch;
            }
        ...
        [CompilerGenerated]
        private sealed class <PostCombatSingleGame>c__AnonStorey54
        {
            internal int canPickHeroNum;

            internal void <>m__3E(ResHeroCfgInfo heroCfg)
            {
        ...
        [CompilerGenerated]
        private sealed class <PostCombatSingleGame>c__AnonStorey55
        {
            internal ResHeroCfgInfo heroCfg;

            internal bool <>m__3F(uint x)
            {
