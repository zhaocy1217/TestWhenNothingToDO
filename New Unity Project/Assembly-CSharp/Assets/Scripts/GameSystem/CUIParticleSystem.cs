namespace Assets.Scripts.GameSystem
{
    ...
    public class CUIParticleSystem : Singleton<CUIParticleSystem>
    {
        ...
        public UIParticleInfo AddParticle(string parPath, float playTime, Vector2 sreenLoc)
        {
            ...
            UIParticleInfo item = new UIParticleInfo();
            item.path = parPath;
            item.currentTime = 0f;
            item.totlalTime = playTime;
            item.parObj = this.m_particleContainerScript.AddGameObject(item.path, false, ref sreenLoc, false);
            this.m_particleList.Add(item);
            return item;
        }
        ...
        public UIParticleInfo AddParticle(string parPath, float playTime, GameObject targetLoc, CUIFormScript targetFormScript)
        {
            ...
            Vector2 sreenLoc = CUIUtility.WorldToScreenPoint(targetFormScript.GetCamera(), targetLoc.get_transform().get_position());
        ...
        public void Hide(CUIEvent evt = new CUIEvent())
        {
            if (this.m_particleContainerScript != null)
            {
        ...
        public void Open()
        {
            if (Singleton<CUIManager>.GetInstance().GetForm(s_qualifyingFormPath) == null)
            {
                ...
                if (script != null)
                {
                    Transform transform = script.get_transform().Find("3DImage");
                    if (transform != null)
                    {
                        this.m_particleContainerScript = transform.GetComponent<CUI3DImageScript>();
                    }
                    Transform transform2 = script.get_transform().Find("txtInfo");
                    if (transform2 != null)
                    {
        ...
        public void Show(CUIEvent evt = new CUIEvent())
        {
            if (this.m_particleContainerScript != null)
            {
                Camera component = this.m_particleContainerScript.GetComponent<Camera>();
                if (component != null)
                {
                    component.set_cullingMask(((int) 1) << CUI3DImageScript.s_cameraLayers[1]);
                }
            }
        }
    }
