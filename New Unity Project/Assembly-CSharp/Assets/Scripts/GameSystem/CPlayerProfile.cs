namespace Assets.Scripts.GameSystem
{
    ...
    public class CPlayerProfile
    {
        ...
        [CompilerGenerated]
        private static Comparison<COMDT_MOST_USED_HERO_INFO> <>f__am$cache44;
        [CompilerGenerated]
        private string <GuildName>k__BackingField;
        [CompilerGenerated]
        private COM_PLAYER_GUILD_STATE <GuildState>k__BackingField;
        ...
        public void ConvertRoleInfoData(CRoleInfo roleInfo)
        {
            ...
            else
            {
                ...
                CAchieveInfo2 info = CAchieveInfo2.GetAchieveInfo(roleInfo.logicWorldID, roleInfo.playerUllUID, false);
                if (info != null)
                {
                    if (info.LastDoneTrophyRewardInfo != null)
                    {
                        this._trophyRewardInfoLevel = info.LastDoneTrophyRewardInfo.Cfg.dwTrophyLvl;
                    }
                    else
                    {
                        object[] inParameters = new object[] { roleInfo.logicWorldID, roleInfo.playerUllUID };
                        DebugHelper.Assert(false, "cheieveInfo.LastDoneTrophyRewardInfo is null,wordID{0},uid{1}", inParameters);
                    }
                    this._trophyRank = info.GetWorldRank();
                    if (info.SelectedTrophies != null)
                    {
                        Array.Copy(info.SelectedTrophies, this._selectedTrophies, info.SelectedTrophies.Length);
                    }
                    else
                    {
                        object[] objArray2 = new object[] { roleInfo.logicWorldID, roleInfo.playerUllUID };
                        DebugHelper.Assert(false, "cheieveInfo.SelectedTrophies is null,wordID{0},uid{1}", objArray2);
                    }
                    this._mentorInfo = Utility.DeepCopyByReflection<COMDT_ACNT_MASTER_INFO>(roleInfo.m_mentorInfo);
                }
                else
                {
                    object[] objArray3 = new object[] { roleInfo.logicWorldID, roleInfo.playerUllUID };
                    DebugHelper.Assert(false, "cheieveInfo is null,wordID{0},uid{1}", objArray3);
                }
            }
        }
        ...
        public void ConvertServerDetailData(CSDT_ACNT_DETAIL_INFO detailInfo)
        {
            ...
            if (detailInfo != null)
            {
                ...
                CAchieveInfo2 info2 = CAchieveInfo2.GetAchieveInfo(detailInfo.iLogicWorldId, detailInfo.ullUid, false);
                info2.OnServerAchieveInfo(detailInfo.astShowAchievement, detailInfo.dwAchieveMentScore);
                this._trophyRewardInfoLevel = info2.LastDoneTrophyRewardInfo.Cfg.dwTrophyLvl;
                this._trophyRank = info2.GetWorldRank();
                Array.Copy(info2.SelectedTrophies, this._selectedTrophies, info2.SelectedTrophies.Length);
        ...
        public static float Divide(ulong a, uint b)
        {
            ...
            return (((float) ((a * ((ulong) 0x3e8L)) / ((ulong) b))) / 1000f);
        }
        ...
        {
            return Divide((uint) this._guildWinCount, (uint) this._guildTotalCount);
        }
        ...
        private void SortMostUsedHeroList()
        {
            if (this._mostUsedHeroList != null)
            {
                ...
                {
                    <>f__am$cache44 = delegate (COMDT_MOST_USED_HERO_INFO b, COMDT_MOST_USED_HERO_INFO a) {
                        if ((a == null) && (b == null))
                        {
                            return 0;
                        }
                ...
                this._mostUsedHeroList.Sort(<>f__am$cache44);
            }
        }
        ...
        public COM_PLAYER_GUILD_STATE GuildState
        {
            ...
            get
            {
                return this.<GuildState>k__BackingField;
            }
            [CompilerGenerated]
            set
            {
                this.<GuildState>k__BackingField = value;
            }
        }
    }
}
