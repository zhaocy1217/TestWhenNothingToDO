namespace Assets.Scripts.GameSystem
{
    ...
    public abstract class Activity
    {
        ...
        public string PeriodText
        {
            get
            {
                string newValue = Utility.DateTimeFormatString(Utility.ToUtcTime2Local(this.StartTime), Utility.enDTFormate.DATE);
                string str2 = Utility.DateTimeFormatString(Utility.ToUtcTime2Local(this.CloseTime), Utility.enDTFormate.DATE);
                string timeDesc = this.TimeDesc;
                if (!string.IsNullOrEmpty(timeDesc))
                {
                    timeDesc = timeDesc.Trim();
                    if (timeDesc.Length > 0)
                    {
                        return timeDesc.Replace("{S}", newValue).Replace("{C}", str2);
                    }
                }
                return (!(newValue != str2) ? newValue : (newValue + " ~ " + str2));
            }
        }
