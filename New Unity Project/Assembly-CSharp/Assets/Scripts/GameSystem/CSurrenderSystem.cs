namespace Assets.Scripts.GameSystem
{
    ...
    public class CSurrenderSystem : Singleton<CSurrenderSystem>
    {
        ...
        public void DelayCloseSurrenderForm(int delay)
        {
            ...
            if (form != null)
            {
                CUITimerScript component = form.get_transform().GetChild(0).GetComponent<CUITimerScript>();
        ...
        private void OnSurrenderCountDown(CUIEvent cuiEvent)
        {
            ...
            if ((srcWidget != null) && (srcWidgetScript != null))
            {
                ...
                Utility.GetComponetInChild<Slider>(srcWidget, "CountDownBar/Bar").set_value(num);
            }
        }
        ...
        public void OpenSurrenderForm(int maxNum, int totalNum, byte data)
        {
            ...
            if (script != null)
            {
                obj2 = script.get_transform().GetChild(0).get_gameObject();
                obj3 = obj2.get_transform().Find("SurrenderElement").get_gameObject();
                for (int i = 0; i < 5; i++)
                {
                    obj4 = obj3.get_transform().GetChild(i).get_gameObject();
                    flag2 = i < maxNum;
                    ...
                    if (flag2)
                    {
                        if (i < totalNum)
                        {
                            ...
                            obj4.get_transform().GetChild(0).get_gameObject().CustomSetActive(bActive);
                            obj4.get_transform().GetChild(1).get_gameObject().CustomSetActive(!bActive);
                        }
                        else
                        {
                            obj4.get_transform().GetChild(0).get_gameObject().CustomSetActive(false);
                            obj4.get_transform().GetChild(1).get_gameObject().CustomSetActive(false);
                        }
                    }
                }
                obj2.get_transform().Find("ButtonGroup/Button_Surrender").get_gameObject().SetActive(this.m_haveRights);
                obj2.get_transform().Find("ButtonGroup/Button_Reject").get_gameObject().SetActive(this.m_haveRights);
                if (flag)
                {
        ...
        private void SimplifySurrenderForm()
        {
            ...
            if (form != null)
            {
                Transform trans = form.get_transform().FindChild("PanelSurrender");
                if (trans != null)
                {
                    CUICommonSystem.PlayAnimation(trans, null);
                }
            }
        }
