namespace Assets.Scripts.GameSystem
{
    ...
    public class HeroSortImp : Singleton<HeroSortImp>, IMallSort<CMallSortHelper.HeroSortType>, IComparer<ResHeroCfgInfo>
    {
        ...
        private int CompareDefault(ResHeroCfgInfo l, ResHeroCfgInfo r)
        {
            ...
            uint dwSortID = (shop2 == null) ? uint.MaxValue : shop2.dwSortId;
            if (promotion != null)
            {
            ...
            if (promotion2 != null)
            {
                dwSortID = promotion2.dwSortIndex;
            }
            if (shop != null)
            {
                ResDT_RegisterSale_Info stRegisterSale = shop.stRegisterSale;
                if (CMallSystem.IsinRegisterSales(stRegisterSale))
                {
                    dwSortIndex = stRegisterSale.dwSortID;
                }
            }
            if (shop2 != null)
            {
                ResDT_RegisterSale_Info info2 = shop2.stRegisterSale;
                if (CMallSystem.IsinRegisterSales(info2))
                {
                    dwSortID = info2.dwSortID;
                }
            }
            if (dwSortIndex < dwSortID)
            {
                return 1;
            }
            if (dwSortIndex > dwSortID)
