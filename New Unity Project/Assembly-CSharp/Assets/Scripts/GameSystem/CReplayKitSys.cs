namespace Assets.Scripts.GameSystem
{
    ...
    {
        [CompilerGenerated]
        private static CTimer.OnTimeUpHandler <>f__am$cache5;
        ...
        public void ChangeReplayKitStatus(Transform container, Status status)
        {
            if (container != null)
            {
                if (!this.Cap)
                {
                    container.get_gameObject().CustomSetActive(false);
                }
                else if (!GameSettings.EnableReplayKit)
                {
                    container.get_gameObject().CustomSetActive(false);
                }
                else if (container.get_gameObject().get_activeSelf())
                {
                    Transform transform = container.get_transform().Find("Recording");
                    Transform transform2 = container.get_transform().Find("Paused");
                    Transform transform3 = container.get_transform().Find("Transition");
        ...
        public override void Init()
        {
            ...
            Singleton<EventRouter>.GetInstance().AddEventHandler(EventID.GLOBAL_SERVER_TO_CLIENT_CFG_READY, new Action(this, (IntPtr) this.SetReplayKitGlobalCfg));
        }
                ...
                else
                {
                    if (Singleton<BattleLogic>.GetInstance().isRuning)
                    {
                        ...
                        if ((curLvelContext != null) && (curLvelContext.IsGameTypeGuide() || !curLvelContext.IsMobaMode()))
                        {
                            ...
                            event3.m_eventID = enUIEventID.ReplayKit_Pause_Recording;
                    ...
                    else if (Singleton<CHeroSelectBaseSystem>.instance.m_isInHeroSelectState && (Singleton<CHeroSelectBaseSystem>.GetInstance().IsSpecTraingMode() || !Singleton<CHeroSelectBaseSystem>.GetInstance().IsMobaMode()))
                    {
                        ...
                        event4.m_eventID = enUIEventID.ReplayKit_Pause_Recording;
        ...
        public void InitReplayKitRecordBtn(Transform container)
        {
            if (container != null)
            {
                ...
                {
                    CUIEventScript componetInChild = Utility.GetComponetInChild<CUIEventScript>(container.get_gameObject(), "Record");
                    if (componetInChild != null)
                    {
                        componetInChild.m_onClickEventID = enUIEventID.ReplayKit_Preview_Record;
                    }
                    Transform transform = container.get_transform().Find("Extra/BtnGroup/BtnYes");
                    if (transform != null)
                    {
                        CUIEventScript component = transform.GetComponent<CUIEventScript>();
                        if (component != null)
                    ...
                    Transform transform2 = container.get_transform().Find("Extra/BtnGroup/BtnNo");
                    if (transform2 != null)
                    {
                    ...
                    {
                        <>f__am$cache5 = delegate (int sequence) {
                            Singleton<CUIEventManager>.GetInstance().DispatchUIEvent(enUIEventID.ReplayKit_Pause_Recording);
                        };
                    }
                    Singleton<CTimerManager>.GetInstance().AddTimer(0x3e8, 1, <>f__am$cache5);
                }
            }
        }
        ...
        private void SetReplayKitGlobalCfg()
        {
            if (GameDataMgr.svr2CltCfgDict != null)
            {
                if (GameDataMgr.svr2CltCfgDict.ContainsKey(6))
                {
                    ...
                    if (GameDataMgr.svr2CltCfgDict.TryGetValue(6, out info))
                    {
                        this.m_enable = info.dwConfValue > 0;
                    }
                }
        ...
        public override void UnInit()
        {
            ...
            Singleton<EventRouter>.GetInstance().RemoveEventHandler(EventID.GLOBAL_SERVER_TO_CLIENT_CFG_READY, new Action(this, (IntPtr) this.SetReplayKitGlobalCfg));
        }
