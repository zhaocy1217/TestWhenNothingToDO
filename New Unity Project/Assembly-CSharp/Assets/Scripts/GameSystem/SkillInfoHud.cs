    ...
    public class SkillInfoHud
    {
                ...
                for (int i = 0; (i < this._skillItems.Length) && ((i + 1) < skillSlotArray.Length); i++)
                {
                    ...
                    if (obj2 != null)
                    {
                        if (slot != null)
                        {
                            ...
                            this._skillIcons[i].SetSprite(CUIUtility.s_Sprite_Dynamic_Skill_Dir + slot.SkillObj.IconName, Singleton<CBattleSystem>.GetInstance().WatchFormScript, true, false, false, false);
        ...
        public void ValidateCD(SkillSlotType slot)
        {
            if (this._curActor != 0)
            {
                ...
                if ((index >= 0) && (index < this._skillCdBgs.Length))
                {
                    ...
                    this._skillCdBgs[index].set_fillAmount(((float) slot2.CurSkillCD) / ((float) slot2.GetSkillCDMax()));
                }
            }
        }
        ...
        public void ValidateLevel(SkillSlotType slot)
        {
            if (this._curActor != 0)
            {
                ...
                {
                    SkillSlot slot2 = this._curActor.handle.SkillControl.SkillSlotArray[(int) slot];
                    this._skillLevels[index].set_text(slot2.GetSkillLevel().ToString());
                }
            }
        }
    }
