    ...
    public class InBattleMsgNetCore
    {
        ...
        public static void OnObserveTipsCome(CSPkg msg)
        {
            ...
            if (inputChat != null)
            {
                ...
                if (playerByUid != null)
                {
                    name = playerByUid.Name;
                }
                InBattleInputChat.InBatChatEntity ent = inputChat.ConstructColorFlagEnt(string.Format(Singleton<CTextManager>.GetInstance().GetText(bTipsType.ToString()), stOBTipsNtf.dwOBNum, name));
                inputChat.Add(ent);
            }
        }
