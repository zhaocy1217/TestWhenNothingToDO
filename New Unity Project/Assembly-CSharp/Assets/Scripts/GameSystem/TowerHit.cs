    ...
    public class TowerHit
    {
        ...
        public static void _play_effect(string effect_name, float playTime, GameObject target)
        {
            if (target != null)
            {
                ...
                {
                    Vector2 sreenLoc = currentCamera.WorldToScreenPoint(target.get_transform().get_position());
        ...
        public static void _play_effect_UGUI_Obj(string effect_name, float playTime, GameObject target)
        {
            if (target != null)
            {
                Vector2 sreenLoc = CUIUtility.WorldToScreenPoint(Singleton<CBattleSystem>.GetInstance().FormScript.GetCamera(), target.get_transform().get_position());
                Singleton<CUIParticleSystem>.instance.AddParticle(effect_name, playTime, sreenLoc);
            }
        }
