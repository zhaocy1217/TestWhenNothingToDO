namespace Assets.Scripts.GameSystem
{
    ...
    {
        [CompilerGenerated]
        private CS_VOICESTATE_TYPE <lastSendVoiceState>k__BackingField;
        ...
        private void CloseHeroSelctVoice()
        {
            if (this.m_bClickSound)
            {
                ...
                {
                    this.m_SoundLevel_HeroSelect.get_gameObject().CustomSetActive(false);
                }
            }
        }
            ...
            if (this.ApolloVoiceMgr != null)
            {
                ...
                for (int j = 0; j < roomInfoSvr.dwRoomUserCnt; j++)
                {
                    ...
                    else
                    {
                        ...
                        item.openID = Utility.UTF8Convert(roomInfoSvr.astRoomUserList[j].szOpenID);
                        item.memID = roomInfoSvr.astRoomUserList[j].dwMemberID;
                        item.uuid = roomInfoSvr.astRoomUserList[j].ullUid;
        ...
        private void OnHold_VOCEBtn_HeroSelect(CUIEvent uiEvent)
        {
            ...
            if ((Time.get_time() - this.m_fStartTimeHeroSelect) >= this.m_TotalVoiceTime)
            {
                this.CloseHeroSelctVoice();
            }
            else if ((Time.get_time() - this.m_fCurTimeHeroSelect) >= this.m_VoiceUpdateDelta)
            {
                this.PrintLog("holding " + this.m_fCurTimeHeroSelect, null, false);
                this.m_fCurTimeHeroSelect = Time.get_time();
                this.UpdateSoundLevel_HeroSelect(Random.Range(0f, 1f), this.m_TotalVoiceTime - (Time.get_time() - this.m_fStartTimeHeroSelect));
            }
        }
        ...
        private void OnHoldStart_VOCEBtn_HeroSelect(CUIEvent uiEvent)
        {
            ...
            if (!this.m_bClickSound)
            {
                ...
                if (this.m_SoundLevel_HeroSelect != null)
                {
                    this.m_SoundLevel_HeroSelect.get_gameObject().CustomSetActive(true);
                }
                this.m_fStartTimeHeroSelect = Time.get_time();
                this.m_fCurTimeHeroSelect = Time.get_time();
        ...
        protected void PrintLog(string message, string filename = new string(), bool append = false)
        {
        }
        ...
        public void ShowVoiceBtn_HeroSelect(CUIFormScript formScript)
        {
            ...
            if (formScript != null)
            {
                this.m_SoundLevel_HeroSelect = formScript.get_transform().Find("chatTools/volume_PL");
            }
        }
        ...
        public void SyncReconnectData(CSDT_RECONN_GAMEINGINFO info)
        {
            ...
            {
                for (int i = 0; i < info.stCampVoiceState.bPlayerNum; i++)
                {
        ...
        private void Update()
        {
            if (this.m_bInRoom && ((Time.get_time() - this.m_UpdateTime) >= 1f))
            {
                this.m_UpdateTime = Time.get_time();
        ...
        {
            <UpdateEnterRoomState>c__Iterator39 iterator = new <UpdateEnterRoomState>c__Iterator39();
        ...
        private void UpdateHeroSelectVoiceBtnState(bool bEnable)
        {
            if (this.m_VoiceBtn != null)
            {
                if (!bEnable)
                {
                    this.m_VoiceBtn.GetComponent<CUIEventScript>().set_enabled(false);
                ...
                {
                    this.m_VoiceBtn.GetComponent<CUIEventScript>().set_enabled(true);
                    ...
                    text2.set_color(new Color(text2.get_color().r, text2.get_color().g, text2.get_color().b, 1f));
                }
            }
        }
                ...
                if (masterMemberInfo != null)
                {
                    ...
                    if (this.m_bInHeroSelectUI)
                    {
                        ...
                        if (teamPlayerElement != null)
                        {
                            ...
                            {
                                transform2.get_gameObject().CustomSetActive(iMemberState >= 1);
                            }
                        }
                    }
        ...
        public CS_VOICESTATE_TYPE lastSendVoiceState
        {
            ...
            get
            {
                return this.<lastSendVoiceState>k__BackingField;
            }
            [CompilerGenerated]
            set
            {
                this.<lastSendVoiceState>k__BackingField = value;
            }
        }
        ...
        [CompilerGenerated]
        private sealed class <UpdateEnterRoomState>c__Iterator39 : IDisposable, IEnumerator, IEnumerator<object>
        {
            ...
            public bool MoveNext()
            {
                ...
                if (!this.<>f__this.m_bInRoom && (this.<>f__this.ApolloVoiceMgr != null))
                {
                    ...
                    if (this.<idx>__0 < 200)
                    {
                        ...
                        this.<>f__this.m_UpdateTime = Time.get_time();
