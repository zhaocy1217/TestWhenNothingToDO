namespace Assets.Scripts.GameSystem
{
    ...
    public class CLoginSystem : Singleton<CLoginSystem>
    {
        ...
        private void ConnectLimit(bool showTips = true, ApolloResult result = 0)
        {
            ...
            Transform transform = widget.get_transform().Find("pnlStartGame/btnStartGame");
            if (transform == null)
            {
                return;
            }
            ...
            CUITimerScript componetInChild = Utility.GetComponetInChild<CUITimerScript>(transform.get_gameObject(), "CountDown/timerEnableStartBtn");
        ...
        public void Draw()
        {
            ...
            if (!m_bFirstLogin)
            {
                ...
                events.Add(new KeyValuePair<string, string>("totaltime", Time.get_time().ToString()));
        ...
        public override void Init()
        {
            ...
            Singleton<EventRouter>.GetInstance().AddEventHandler(EventID.ApolloHelper_Login_Need_Real_Name_Auth, new Action(this, (IntPtr) this.LoginNeedRealNameAuth));
        ...
        {
            Debug.Log("*Login* Cancel" + Time.get_realtimeSinceStartup());
            Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
        ...
        {
            Debug.Log("*Login* Failed" + Time.get_realtimeSinceStartup());
            Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
            if (Singleton<GameStateCtrl>.GetInstance().GetCurrentState() is LoginState)
            {
                Singleton<CTimerManager>.GetInstance().RemoveTimerSafely(ref this.m_CheckLoginTimeoutTimerSeq);
        ...
        {
            Debug.Log("*Login* NeedRealNameAuth" + Time.get_realtimeSinceStartup());
            Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
            if (Singleton<GameStateCtrl>.GetInstance().GetCurrentState() is LoginState)
            {
                Singleton<CTimerManager>.GetInstance().RemoveTimerSafely(ref this.m_CheckLoginTimeoutTimerSeq);
        ...
        {
            Debug.Log("*Login* SUCC" + Time.get_realtimeSinceStartup());
            Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
            if (Singleton<GameStateCtrl>.GetInstance().GetCurrentState() is LoginState)
            {
                ...
                events.Add(new KeyValuePair<string, string>("platform", Singleton<ApolloHelper>.GetInstance().CurPlatform.ToString()));
                events.Add(new KeyValuePair<string, string>("openid", "NULL"));
                float num = Time.get_time() - Singleton<CLoginSystem>.GetInstance().m_fLoginClickTime;
        ...
        {
            Debug.Log("*Login* LoginTimeout" + Time.get_realtimeSinceStartup());
            Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
        ...
        private void OnChangeAccountYes(CUIEvent uiEvent)
        {
            ...
            if (Singleton<GameStateCtrl>.GetInstance().GetCurrentState() is LoginState)
            {
                Debug.Log("*Login* OnChangeAccountYes LoginState " + Time.get_realtimeSinceStartup());
                Singleton<ApolloHelper>.GetInstance().SwitchUser(true);
                if ((Singleton<ApolloHelper>.GetInstance().m_LastTriedPlatform == ApolloPlatform.Wechat) || (Singleton<ApolloHelper>.GetInstance().m_LastTriedPlatform == ApolloPlatform.QRWechat))
                {
                    Singleton<LobbyLogic>.GetInstance().GotoAccLoginPage();
            ...
            {
                Debug.Log("*Login* OnChangeAccountYes Not LoginState " + Time.get_realtimeSinceStartup());
                Singleton<ApolloHelper>.GetInstance().Logout();
                Singleton<ApolloHelper>.GetInstance().SwitchUser(false);
        ...
        private void OnEnableStartButton(CUIEvent uiEvent)
        {
            ...
            if (this.m_Form != null)
            {
                ...
                if (widget != null)
                {
                    Transform transform = widget.get_transform().Find("btnStartGame");
                    if (transform != null)
                    {
                        Transform transform2 = transform.Find("CountDown");
                        Button component = transform.GetComponent<Button>();
        ...
        {
            this.m_fLoginClickTime = Time.get_time();
        ...
        private void OnLogin_None(CUIEvent uiEvent)
        {
            ...
            if (Singleton<ApolloHelper>.GetInstance().IsNoneModeSupport)
            {
                ...
                try
                {
                    path = Application.get_persistentDataPath() + "/customOpenId.txt";
        ...
        private void OnLogin_WtLogin(CUIEvent uiEvent)
        {
            ...
            if (this.m_Form != null)
            {
                ...
                if (widget != null)
                {
                    Transform transform = widget.get_transform().Find("pnlWtLogin");
                    DebugHelper.Assert(transform != null, "WtLogin Panel Is Null");
                    if (transform != null)
                    {
                        InputField component = transform.Find("btnGroup/UinInput").get_gameObject().GetComponent<InputField>();
                        InputField field2 = transform.Find("btnGroup/PswInput").get_gameObject().GetComponent<InputField>();
                        InputField field3 = transform.Find("btnGroup/OpenIdInput").get_gameObject().GetComponent<InputField>();
                        if ((field3 != null) && (field3.get_text().Length > 0))
                        {
                            stUIEventParams par = new stUIEventParams();
                            par.tagStr = field3.get_text();
                            Singleton<CUIEventManager>.GetInstance().DispatchUIEvent(enUIEventID.Login_Platform_None, par);
                        }
                        else if ((((component != null) && (field2 != null)) && ((component.get_text() != null) && (component.get_text().Length > 0))) && (field2.get_text().Length > 0))
                        {
                            PlayerPrefs.SetString("SGamePlayerUinCache", component.get_text());
                            PlayerPrefs.SetString("SGamePlayerPwdCache", field2.get_text());
                            Singleton<ApolloHelper>.GetInstance().Login(ApolloPlatform.WTLogin, Convert.ToUInt64(component.get_text()), field2.get_text());
                        }
                    }
                }
            }
            ...
            if (Singleton<GameStateCtrl>.GetInstance().GetCurrentState() is LoginState)
            {
                ...
                if (this.m_Form != null)
                {
                    ...
                    if (((widget != null) && (obj3 != null)) && (obj4 != null))
                    {
                        ...
                        {
                            case enLoadState.Login:
                                this.RefreshLoginPnl(widget.get_transform());
                            ...
                            case enLoadState.Zone:
                                this.RefreshZonePnl(obj4.get_transform());
                                widget.CustomSetActive(false);
                                obj3.CustomSetActive(false);
                            ...
                            case enLoadState.StartGame:
                                this.RefreshStartGamePnl(obj3.get_transform());
                                widget.CustomSetActive(false);
        ...
        private void QuickLoginDone(ApolloWakeupInfo wakeupInfo)
        {
            ...
            if (wakeupInfo.ExtensionInfo != null)
            {
                ...
                while (num < count)
                {
                    ...
                    if ((pair != null) && (pair.Key == ApolloHelper.QQ_USER_OPENID))
                    {
                        if (wakeupInfo.OpenId == string.Empty)
                        {
                            wakeupInfo.OpenId = pair.Value;
                        }
                        break;
                    }
                    num++;
                ...
                int num3 = 0;
                int num4 = wakeupInfo.ExtensionInfo.Count;
                while (num3 < num4)
                {
                    ApolloKVPair pair2 = wakeupInfo.ExtensionInfo[num3];
                    if (pair2 != null)
                    {
                        if (pair2.Key == ApolloHelper.QQ_LAUNCH_FROM)
                        {
                            if ((pair2.Value == ApolloHelper.QQ_LAUNCH_FROM_GAMECENTER) && !Singleton<LobbyLogic>.instance.isLogin)
                            {
                                Singleton<ApolloHelper>.GetInstance().IsQQGameCenter = true;
                            }
                        }
                        else if ((pair2.Key == ApolloHelper.QQ_SHARE_GAMEDATA) && ((accountInfo == null) || (accountInfo.OpenId == wakeupInfo.OpenId)))
                        {
                            MonoSingleton<ShareSys>.GetInstance().UnpackInviteSNSData(pair2.Value, wakeupInfo.OpenId);
                        }
                        Debug.Log("WakeUpInfo:key:" + pair2.Key + "value:" + pair2.Value);
                    }
                    num3++;
                }
            }
            if ((accountInfo != null) && !string.IsNullOrEmpty(accountInfo.OpenId))
            {
        ...
        private void RefreshLoginPnl(Transform containerTransform)
        {
            if (containerTransform != null)
            {
                ...
                containerTransform.get_gameObject().CustomSetActive(true);
            }
        }
        ...
        private void RefreshStartGamePnl(Transform containerTransform)
        {
            if (containerTransform != null)
            {
                ...
                if (transform != null)
                {
                    this.SetZone(transform.get_gameObject(), this.m_selectTdirUrl);
                }
        ...
        private void RegisterXGPush(string openID)
        {
            ...
            if (class2 != null)
            {
                ...
                if (@static != null)
                {
                    Debug.Log("*Login*RegisterXGPush in unity " + Time.get_realtimeSinceStartup());
                    object[] objArray1 = new object[] { openID };
                    @static.Call("RegisterXGPush", objArray1);
                }
            }
        }
        ...
        public void SetLastLoginZone(Transform containerTransform)
        {
            if (containerTransform != null)
            {
                ...
                if (transform != null)
                {
                    ...
                    this.SetZone(transform.get_gameObject(), lastLoginUrl);
                }
            }
        }
        ...
        private void SetNickName(ApolloRelation aRelation)
        {
            ...
            if (this.m_Form != null)
            {
                ...
                if (widget != null)
                {
                    Transform transform = widget.get_transform().Find("pnlStartGame/pnlSnsName");
                    Transform transform2 = widget.get_transform().Find("pnlStartGame/pnlSnsName/snsName");
                    if (transform != null)
                    {
                        if ((transform2 != null) && (aRelation.Result == ApolloResult.Success))
                        {
                            ...
                            {
                                component.set_text(person.NickName);
            ...
            if (containerTransform != null)
            {
                ...
                if (transform != null)
                {
                    ...
                    for (int i = 0; i < svrUrlList.Count; i++)
                    {
                        ...
                        component.GetElemenet(i).get_gameObject().get_transform().Find("Text").get_gameObject().GetComponent<Text>().set_text(group.name);
                    }
        ...
        public void SetTdirZoneList(int index)
        {
            ...
            if (widget != null)
            {
                Transform transform = widget.get_transform().Find("pnlSelectZone/ListZones");
                ...
                if (transform != null)
                {
                    ...
                    if (((svrUrlList != null) && (index >= 0)) && (index < svrUrlList.Count))
                    {
                        ...
                        {
                            GameObject zonePanel = component.GetElemenet(i).get_gameObject().get_transform().Find("Panel").get_gameObject();
                            this.SetZone(zonePanel, tdirUrls[i]);
                        }
                    }
                }
            }
        ...
        {
            zonePanel.get_transform().Find("ZoneName").get_gameObject().GetComponent<Text>().set_text(tdirUrl.name);
            Text component = zonePanel.get_transform().Find("State").get_gameObject().GetComponent<Text>();
            if (tdirUrl.flag == SvrFlag.New)
            {
                component.set_text(Singleton<CTextManager>.GetInstance().GetText("Zone_NEW"));
            }
            else if (tdirUrl.flag == SvrFlag.Recommend)
            {
                component.set_text(Singleton<CTextManager>.GetInstance().GetText("Zone_Recommend"));
            }
            else if (tdirUrl.flag == SvrFlag.Hot)
            {
                component.set_text(Singleton<CTextManager>.GetInstance().GetText("Zone_HOT"));
            }
            else
            {
                component.set_text(string.Empty);
            }
            GameObject obj2 = zonePanel.get_transform().Find("StateImage").get_gameObject();
            ...
            if (containerTransform != null)
            {
                ...
                if (transform != null)
                {
                    ...
                    if (component != null)
                    {
                        ...
                        component.set_text(string.Format("App v{0} Res v{1} Build{2} R{3}", args));
                    }
                }
            }
        }
        ...
        public bool UILoaded()
        {
            ...
            {
                case enLoadState.Login:
                    obj2 = Utility.FindChild(this.m_Form.get_gameObject(), "LoginContainer/pnlMobileLogin");
                    break;

                case enLoadState.Zone:
                    obj2 = Utility.FindChild(this.m_Form.get_gameObject(), "ZoneContainer/pnlSelectZone");
                    break;

                case enLoadState.StartGame:
                    obj2 = Utility.FindChild(this.m_Form.get_gameObject(), "StartGameContainer/pnlStartGame");
                    break;
            }
            if (obj2 == null)
            {
        ...
        public override void UnInit()
        {
            ...
            Singleton<EventRouter>.GetInstance().RemoveEventHandler(EventID.ApolloHelper_Login_Need_Real_Name_Auth, new Action(this, (IntPtr) this.LoginNeedRealNameAuth));
