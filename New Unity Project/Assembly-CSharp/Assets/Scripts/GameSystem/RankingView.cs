namespace Assets.Scripts.GameSystem
{
    ...
    {
        private static uint RANK_GOD_WIN_CNT = 0;
        ...
        public static void InitRankGodDetailTab()
        {
            ...
            if (form != null)
            {
                ...
                CUICommonSystem.InitMenuPanel(p, titleList, 0, true);
            }
        }
        ...
        public static bool IsRankWidgetActive(RankingFormWidget widget)
        {
            ...
            return form.GetWidget((int) widget).get_activeSelf();
        }
        ...
        public static void OnHeroItemEnable(CUIEvent uiEvent, ResHeroCfgInfo heroCfgInfo)
        {
            if ((heroCfgInfo != null) && (uiEvent.m_srcWidget != null))
            {
                ...
                if (item != null)
                {
                    CUICommonSystem.SetHeroItemImage(uiEvent.m_srcFormScript, item, heroCfgInfo.szImagePath, enHeroHeadType.enIcon, false, false);
                    CUIEventScript component = item.GetComponent<CUIEventScript>();
                    if (component != null)
                    {
                        stUIEventParams eventParams = new stUIEventParams();
                        eventParams.heroId = heroCfgInfo.dwCfgID;
        ...
        public static void OnRankGodDetailEquipClick(CEquipInfo equipInfo, string playerName, string heroName)
        {
            ...
            if (form != null)
            {
                ...
                if (widget != null)
                {
                    ...
                    text2.set_text((equipInfo != null) ? equipInfo.m_equipName : string.Empty);
                    text3.set_text((equipInfo != null) ? equipInfo.m_equipPropertyDesc : string.Empty);
                    string[] args = new string[] { playerName, heroName };
                    componetInChild.set_text(Singleton<CTextManager>.instance.GetText("RankGodHeroEquipDesc", args));
                }
            }
        }
                ...
                if (widget != null)
                {
                    ...
                    if (dataByKey != null)
                    {
                        ...
                        if (tabIndex == 0)
                        {
                            ...
                            ushort[] recommendEquipID = new ushort[6];
                            if (bEquipNum > 0)
                            {
                                for (int j = 0; j < bEquipNum; j++)
                                {
                                    recommendEquipID[j] = (ushort) masterHeroInfo.stEquipList.EquipID[j];
                                }
                            }
                            else
                            {
                                ResRecommendEquipInBattle defaultRecommendEquipInfo = Singleton<CEquipSystem>.instance.GetDefaultRecommendEquipInfo(heroId, 1);
                                if (defaultRecommendEquipInfo != null)
                                {
                                    recommendEquipID = defaultRecommendEquipInfo.RecommendEquipID;
                                }
                                bEquipNum = recommendEquipID.Length;
                            }
                            componetInChild.SetElementAmount(bEquipNum);
                            for (int i = 0; i < bEquipNum; i++)
                            {
                                GameObject equipIcon = componetInChild.GetElemenet(i).get_gameObject();
                                CUIEventScript component = equipIcon.GetComponent<CUIEventScript>();
                                ushort equipID = recommendEquipID[i];
                                CEquipInfo equipInfo = CEquipSystem.GetEquipInfo(equipID);
                                component.m_onClickEventParams.battleEquipPar.equipInfo = CEquipSystem.GetEquipInfo(equipID);
                                component.m_onClickEventParams.tagStr = str;
                                component.m_onClickEventParams.tagStr1 = szName;
                                CUICommonSystem.SetEquipIcon(equipID, equipIcon, form);
                            }
                            if (bEquipNum > 0)
                            {
                        ...
                        else if (tabIndex == 1)
                        {
                            ...
                            {
                                GameObject p = script5.GetElemenet(m).get_gameObject();
                                Utility.GetComponetInChild<Image>(p, "imgIcon").SetSprite(symbolList[m].GetIconPath(), form, true, false, false, false);
                                Utility.GetComponetInChild<Text>(p, "SymbolName").set_text(symbolList[m].m_name);
                                char[] trimChars = new char[] { '\n' };
                                Utility.GetComponetInChild<Text>(p, "SymbolDesc").set_text(CSymbolSystem.GetSymbolAttString(symbolList[m], true).TrimEnd(trimChars));
                                Utility.GetComponetInChild<Text>(p, "lblIconCount").set_text(string.Format("x{0}", symbolList[m].m_stackCount));
                                num7 += symbolList[m].m_SymbolData.wLevel * symbolList[m].m_stackCount;
                            }
                            Utility.GetComponetInChild<Text>(obj4, "symbolPageLvlText").set_text(num7.ToString());
                            string[] args = new string[] { str, szName };
                            Utility.GetComponetInChild<Text>(obj4, "heroSymbolText").set_text(Singleton<CTextManager>.instance.GetText("RankGodHeroSymbolDesc", args));
                        }
                    }
                }
            }
        ...
        public static void OnRankingArenaElementEnable(CUIEvent uiEvent)
        {
            ...
            if (Singleton<CArenaSystem>.instance.m_rankInfoList.astFigterDetail[srcWidgetIndexInBelongedList].stFigterData.bMemberType == 1)
            {
                ...
                else
                {
                    ...
                    obj3.CustomSetActive(false);
                    obj4.CustomSetActive(true);
                    bool isEnable = Singleton<CFriendContoller>.instance.model.HeartData.BCanSendHeart(uniq);
        ...
        public static void OpenHeroChooseForm()
        {
            ...
            if (listObj != null)
            {
                ...
                CUICommonSystem.InitMenuPanel(listObj, titleList, (int) all, true);
        ...
        public static void ResetRankListPos(RankingSystem.RankingType rankingType)
        {
            ...
            if (form != null)
            {
                ...
                if (obj2 != null)
                {
                    ...
                    CUIListScript script2 = obj2.GetComponent<CUIListScript>();
                    if (rankingType != RankingSystem.RankingType.God)
                    {
                        component.set_anchoredPosition(s_ListPos1);
                        component.set_sizeDelta(s_ListSize1);
                    }
                    else
                    {
                        component.set_anchoredPosition(s_ListPos2);
                        component.set_sizeDelta(s_ListSize2);
                    }
                    script2.m_scrollAreaSize = new Vector2(component.get_rect().get_width(), component.get_rect().get_height());
                }
            }
        }
        ...
        public static void SetGameObjChildText(GameObject parentObj, string childName, string text)
        {
            if (parentObj != null)
            {
                parentObj.get_transform().FindChild(childName).get_gameObject().GetComponent<Text>().set_text(text);
            }
        }
        ...
        public static void SetPlatChannel(GameObject parentObj, uint logicWorldId)
        {
            if (parentObj != null)
            {
                Transform transform = parentObj.get_transform().Find("NameGroup/PlatChannelIcon");
                if (transform != null)
                {
                    transform.get_gameObject().CustomSetActive(!Utility.IsSamePlatformWithSelf(logicWorldId));
        ...
        public static void UpdateArenaSelfInfo()
        {
            if (Singleton<CArenaSystem>.instance.m_fightHeroInfoList != null)
            {
                ...
                if (form != null)
                {
                    ...
                    {
                        obj4.CustomSetActive(true);
                        obj4.get_transform().set_localEulerAngles(new Vector3(0f, 0f, 0f));
                        SetGameObjChildText(widget, "ChangeNum", num4.ToString(CultureInfo.InvariantCulture));
                    }
                    else if (num4 < 0)
                    {
                        obj4.CustomSetActive(true);
                        obj4.get_transform().set_localEulerAngles(new Vector3(0f, 0f, 180f));
                        SetGameObjChildText(widget, "ChangeNum", num4.ToString(CultureInfo.InvariantCulture));
                    }
                }
            }
        ...
        public static void UpdateGodFindBtns(CUIListScript uiList, int index)
        {
            if (uiList != null)
            {
                ...
                {
                    if (((uiList.GetElemenet(i) != null) && (uiList.GetElemenet(i).get_gameObject() != null)) && uiList.IsElementInScrollArea(i))
                    {
                        Utility.FindChild(uiList.GetElemenet(i).get_gameObject(), "FindBtn/Select").CustomSetActive(index == i);
                    }
                }
            }
        }
            ...
            if ((curRankingList != null) && (objElement != null))
            {
                ...
                if (component != null)
                {
                    ...
                    if (csdt_ranking_list_item_info != null)
                    {
                        ...
                        float num7 = CPlayerProfile.Divide((uint) (csdt_ranking_list_item_info.stExtraInfo.stDetailInfo.stMasterHero.dwWinCnt * 100), csdt_ranking_list_item_info.stExtraInfo.stDetailInfo.stMasterHero.dwGameCnt);
                        if (RANK_GOD_WIN_CNT == 0)
                        {
                            RANK_GOD_WIN_CNT = GameDataMgr.globalInfoDatabin.GetDataByKey((uint) 0xe2).dwConfValue;
                        }
                        if (csdt_ranking_list_item_info.stExtraInfo.stDetailInfo.stMasterHero.dwWinCnt >= RANK_GOD_WIN_CNT)
                        {
                            string[] args = new string[] { num7.ToString("F2"), csdt_ranking_list_item_info.stExtraInfo.stDetailInfo.stMasterHero.dwWinCnt.ToString() };
                            SetGameObjChildText(objElement, "ValueType", Singleton<CTextManager>.GetInstance().GetText("ranking_ItemHeroMasterName", args));
                        }
                        else
                        {
                            string[] textArray2 = new string[] { csdt_ranking_list_item_info.stExtraInfo.stDetailInfo.stMasterHero.dwWinCnt.ToString() };
                            SetGameObjChildText(objElement, "ValueType", Singleton<CTextManager>.GetInstance().GetText("ranking_ItemHeroMasterNameLess100", textArray2));
                        }
            ...
            if (heroCfgInfo != null)
            {
                ...
                if (form != null)
                {
                    ...
                    if (p != null)
                    {
                        ...
                        Utility.GetComponetInChild<Text>(p, "PlayerName").set_text(Singleton<CTextManager>.instance.GetText("RankGodHeroName", args));
                    }
                }
            }
        }
        ...
        public static void UpdateSymbolItem(CSymbolItem symbol, GameObject element, CUIFormScript form)
        {
            ...
            {
                Image component = element.get_transform().Find("iconImage").GetComponent<Image>();
                Text text = element.get_transform().Find("countText").GetComponent<Text>();
                Text text2 = element.get_transform().Find("nameText").GetComponent<Text>();
                Text text3 = element.get_transform().Find("descText").GetComponent<Text>();
                ...
                text.set_text(string.Format("x{0}", symbol.m_stackCount.ToString()));
                text2.set_text(symbol.m_SymbolData.szName);
                text3.set_text(CSymbolSystem.GetSymbolAttString(symbol.m_baseID, true));
            }
        }
    }
}
