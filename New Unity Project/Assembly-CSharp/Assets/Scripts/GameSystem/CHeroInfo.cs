namespace Assets.Scripts.GameSystem
{
    ...
    public class CHeroInfo
    {
        ...
        public static ResHeroProficiency GetHeroProficiency(int job, int level)
        {
            ...
            {
                ResHeroProficiency proficiency = (ResHeroProficiency) enumerator.get_Current();
                if (proficiency.bJob == job)
                {
                    return proficiency;
                }
            }
        ...
        public static int GetMaxProficiency()
        {
            if (m_maxProficiency <= 0)
            {
                for (int i = 0; i < GameDataMgr.heroProficiencyDatabin.Count(); i++)
                {
                    ...
                    while (enumerator.MoveNext())
                    {
                        m_maxProficiency = Math.Max(m_maxProficiency, ((ResHeroProficiency) enumerator.get_Current()).bLevel);
                    }
                }
            }
