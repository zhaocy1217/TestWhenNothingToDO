namespace Assets.Scripts.GameSystem
{
    ...
    public class ExeTaskPhase : ActivityPhase
    {
        ...
        public override bool AchieveJump()
        {
            ...
            if (((this._config.dwConType == 14) && (this._config.ReachConParam.Length >= 2)) && (this._config.ReachConParam[0] == 3))
            {
                uint num2 = this._config.ReachConParam[1];
                if (this._config.ReachConParam[1] <= 0)
                {
                    return CUICommonSystem.JumpForm((RES_GAME_ENTRANCE_TYPE) this._config.dwJumpEntry, 0, 0);
                }
                ...
                if (masterRoleInfo.IsHaveHero(num2, false))
                {
                    MonoSingleton<ShareSys>.GetInstance().RequestShareHeroSkinForm(num2, 0, COM_REWARDS_TYPE.COM_REWARDS_TYPE_HERO);
                    return true;
                }
                if (masterRoleInfo.IsHaveHeroSkin(num2, false))
                {
                    MonoSingleton<ShareSys>.GetInstance().RequestShareHeroSkinForm(0, num2, COM_REWARDS_TYPE.COM_REWARDS_TYPE_SKIN);
                    return true;
                }
            }
            return CUICommonSystem.JumpForm((RES_GAME_ENTRANCE_TYPE) this._config.dwJumpEntry, 0, 0);
        ...
        public override int AchieveInDays
        {
            get
            {
                ...
                return (int) Math.Ceiling((double) (this.LimitDays - totalDays));
            }
        }
