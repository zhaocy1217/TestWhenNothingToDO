namespace Assets.Scripts.GameSystem
{
    ...
    internal class CAdventureSys : Singleton<CAdventureSys>
    {
        ...
        private void CloseChapterRewardPanel(CUIEvent uiEvent)
        {
            ...
            if (form != null)
            {
                CAdventureView.CloseChapterRewardPanel(form.get_gameObject());
            }
        }

        private void ConfirmCheckTeamNum(CUIEvent uiEvent = new CUIEvent())
        {
            string itemName = string.Empty;
            if (!this.CheckFullItem(out itemName))
            {
        ...
        private void ConfirmCheckTeamPower(CUIEvent uiEvent = new CUIEvent())
        {
        ...
        private void ConfirmItemFullAdv(CUIEvent uiEvent = new CUIEvent())
        {
        ...
        {
            if (uiEvent.m_srcWidget.GetComponent<Button>().get_interactable() && this.CheckChapterRewardCondition())
            {
        ...
        {
            <GetLevelCfg>c__AnonStorey63 storey = new <GetLevelCfg>c__AnonStorey63();
            storey.ChapterId = ChapterId;
            storey.Seq = Seq;
            return GameDataMgr.levelDatabin.FindIf(new Func<ResLevelCfgInfo, bool>(storey, (IntPtr) this.<>m__67));
        }

        public static int GetLevelId(int ChapterId, int Seq, int Difficulty)
        {
            <GetLevelId>c__AnonStorey64 storey = new <GetLevelId>c__AnonStorey64();
            storey.ChapterId = ChapterId;
            storey.Seq = Seq;
            storey.Difficulty = Difficulty;
            ResLevelCfgInfo info = GameDataMgr.levelDatabin.FindIf(new Func<ResLevelCfgInfo, bool>(storey, (IntPtr) this.<>m__68));
        ...
        private void OnExploreListScroll(CUIEvent uiEvent)
        {
            ...
            if (form != null)
            {
                CExploreView.OnExploreListScroll(form.get_gameObject());
            }
        }
        ...
        private void OnOpenChooseHero(CUIEvent uiEvent)
        {
            if (uiEvent.m_srcWidget.GetComponent<Button>().get_interactable())
            {
                ...
                else
                {
                    ...
                    reportInfo.bDifficultType = (byte) this.currentDifficulty;
        ...
        private void OpenChapterRewardPanel(CUIEvent uiEvent)
        {
            ...
            if (form != null)
            {
                ...
                {
                    CAdventureView.OpenChapterRewardPanel(form, form.get_gameObject(), tag, this.currentDifficulty, bCanGetReward);
                }
            }
        }
        ...
        [CompilerGenerated]
        private sealed class <GetLevelCfg>c__AnonStorey63
        {
            internal int ChapterId;
            internal int Seq;

            internal bool <>m__67(ResLevelCfgInfo x)
            {
        ...
        [CompilerGenerated]
        private sealed class <GetLevelId>c__AnonStorey64
        {
            internal int ChapterId;
            internal int Difficulty;
            internal int Seq;

            internal bool <>m__68(ResLevelCfgInfo x)
            {
