namespace Assets.Scripts.GameSystem
{
    ...
    internal class CBuySkillPt : CBuyPanelBase
    {
        ...
        {
            base.initPanel(form);
            this.m_content = base.m_FormScript.get_transform().FindChild("Content").GetComponent<Text>();
            base.m_Buttons = new GameObject[] { base.m_FormScript.get_transform().Find("ButtonGroup/Button_Confirm").get_gameObject(), base.m_FormScript.get_transform().Find("ButtonGroup/Button_Cancel").get_gameObject() };
            base.m_CheckRightButton = base.m_FormScript.get_transform().Find("ButtonGroup/Button_CheckRight").get_gameObject();
            this.RefreshUI();
        ...
        private void RefreshUI()
        {
            if (base.bOpen)
            {
                if (!base.bTimesOut)
                {
                    ...
                    this.m_content.set_text(Singleton<CTextManager>.GetInstance().GetText("BuyAct_NoSkillPoint", args));
                }
                else
                {
                    string[] textArray2 = new string[] { this.CurBuyTime.ToString(), this.m_MaxBuyTime.ToString() };
                    this.m_content.set_text(Singleton<CTextManager>.GetInstance().GetText("BuyAct_NoMore", textArray2));
                }
