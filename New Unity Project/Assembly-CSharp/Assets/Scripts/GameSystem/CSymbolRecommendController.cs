namespace Assets.Scripts.GameSystem
{
    ...
    public class CSymbolRecommendController
    {
        ...
        private void GetHeroRcmdSymbolList()
        {
            if (!this.m_cfgHeroRcmdSymbolList.TryGetValue(this.m_curHeroId, out this.m_rcmdSymbolList))
            {
                ...
                else
                {
                    ...
                    {
                        ResSymbolRcmd rcmd = (ResSymbolRcmd) enumerator.get_Current();
                        if (listArray[rcmd.wSymbolLvl - 1] == null)
                        {
                            listArray[rcmd.wSymbolLvl - 1] = new List<uint>();
                        }
                        for (int i = 0; i < rcmd.SymbolID.Length; i++)
                        {
                            if (rcmd.SymbolID[i] > 0)
                            {
                                listArray[rcmd.wSymbolLvl - 1].Add(rcmd.SymbolID[i]);
                            }
                        }
                    }
        ...
        {
            if (Utility.FindChild(form.GetWidget(3), CSymbolSystem.s_symbolRecommendPanel) == null)
            {
                return false;
            }
            return true;
                ...
                if ((masterRoleInfo != null) && (masterRoleInfo.m_customRecommendEquipDictionary != null))
                {
                    ...
                    if ((info2 != null) && (uiEvent.m_srcWidget != null))
                    {
                        ...
                        if (transform != null)
                        {
                            ...
                            if (transform2 != null)
                            {
                                transform2.get_gameObject().CustomSetActive(masterRoleInfo.IsFreeHero(info2.dwCfgID));
                            }
                        }
                    }
                }
        ...
        private void OnRcmdListItemEnable(CUIEvent uiEvent)
        {
            if ((this.m_rcmdSymbolList != null) && (this.m_rcmdSymbolList[this.m_symbolRcmdLevel - 1] != null))
            {
                ...
                {
                    CSymbolSystem.RefreshSymbolItem(GameDataMgr.symbolInfoDatabin.GetDataByKey(this.m_rcmdSymbolList[this.m_symbolRcmdLevel - 1][uiEvent.m_srcWidgetIndexInBelongedList]), uiEvent.m_srcWidget, uiEvent.m_srcFormScript, enSymbolMakeSource.SymbolManage);
                }
            }
        }
        ...
        private void RefreshHeroImage()
        {
            ...
            if (null != form)
            {
                ...
                if (masterRoleInfo != null)
                {
                    GameObject obj2 = Utility.FindChild(form.get_gameObject(), "SymbolRecommend/Panel_SymbolRecommend/heroItemCell");
                    ...
                    if (dataByKey != null)
                    {
                        ...
                        CUICommonSystem.SetHeroItemImage(form, obj2.get_gameObject(), dataByKey.szImagePath, enHeroHeadType.enIcon, false, false);
                        GameObject obj3 = Utility.FindChild(form.get_gameObject(), "SymbolRecommend/Panel_SymbolRecommend/heroNameText");
                        if (obj3 != null)
                        {
                            obj3.GetComponent<Text>().set_text(dataByKey.szName);
                        }
                    }
                }
            }
        ...
        private void RefreshRcmdSymbolListPanel()
        {
            ...
            if (null != form)
            {
                ...
                GameObject obj2 = Utility.FindChild(form.get_gameObject(), "SymbolRecommend/Panel_SymbolRecommend/symbolRcmdList");
                if ((obj2 != null) && (this.m_rcmdSymbolList[this.m_symbolRcmdLevel - 1] != null))
                {
                    obj2.GetComponent<CUIListScript>().SetElementAmount(this.m_rcmdSymbolList[this.m_symbolRcmdLevel - 1].Count);
                    Transform transform = obj2.get_transform().Find("Text");
                    if (transform != null)
                    {
                        ResHeroCfgInfo dataByKey = GameDataMgr.heroDatabin.GetDataByKey(this.m_curHeroId);
        ...
        public void SwitchToSymbolRcmdPanel(CUIFormScript form)
        {
            if ((form != null) && !form.IsClosed())
            {
                ...
                else
                {
                    ...
                    CUICommonSystem.InitMenuPanel(Utility.FindChild(form.get_gameObject(), "SymbolRecommend/Panel_SymbolRecommend/Panel_SymbolLevel/levelList"), titleList, this.m_symbolRcmdLevel - 1, true);
                }
            }
        }
