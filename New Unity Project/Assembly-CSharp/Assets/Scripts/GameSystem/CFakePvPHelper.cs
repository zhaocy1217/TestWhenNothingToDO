namespace Assets.Scripts.GameSystem
{
    ...
    {
        [CompilerGenerated]
        private static bool <bInFakeConfirm>k__BackingField;
        [CompilerGenerated]
        private static bool <bInFakeSelect>k__BackingField;
        ...
        public static void BeginFakeSelectHero()
        {
            ...
            for (int i = 0; i < view.Count; i++)
            {
                ...
                if (info2.RoomMemberType == 2)
                {
                    ...
                    fakeSelect.FakePlayer = info2.WarmNpc;
                    if (Singleton<CHeroSelectBaseSystem>.instance.selectType == enSelectType.enRandom)
                    {
                        fakeSelect.maxChangeHeroCount = Random.Range(0, 2);
                        DoSelectAction(ref fakeSelect);
                    }
                    else
                    {
                        fakeSelect.maxChangeHeroCount = Random.Range(1, 4);
                    }
                    fakeSelect.nextActionSec = Random.Range(3, 6);
            ...
            if (IsInSelectHero())
            {
                if (fakeSelect.changeHeroCount < fakeSelect.maxChangeHeroCount)
                {
                    ...
                    else
                    {
                        ...
                        int num4 = num3 + ((masterRoleInfo == null) ? 0 : masterRoleInfo.freeHeroList.Count);
                        id = Random.Range(0, num4);
                        if (id < num3)
                        {
                ...
                else
                {
                    if (fakeSelect.FakePlayer.dwAcntPvpLevel <= 6)
                    {
                        ...
                        if (((hero2 != null) && (hero2.dwSkinID != 0)) && (Random.Range(0, 0x2710) < hero2.dwSkinRate))
                        {
                    ...
                    else
                    {
                        ...
                        if (((hero3 != null) && (hero3.dwSkinID != 0)) && (Random.Range(0, 0x2710) < hero3.dwSkinRate))
                        {
                    ...
                    if (dataByKey != null)
                    {
                        int index = Random.Range(0, dataByKey.SkillId.Length);
                        while (dataByKey.SkillId[index] == 0)
                        {
                            index = Random.Range(0, dataByKey.SkillId.Length);
                        }
        ...
        private static void FakeConfirmLater(int seq)
        {
            if (FakePlayerConfirmNum < (MapPlayerNum / 2))
            {
                ...
                if (form != null)
                {
                    CMatchingView.UpdateConfirmBox(form.get_gameObject(), confirm.FakePlayer.ullUid);
                }
            }
        }
            ...
            if (form != null)
            {
                ...
                while (enumerator.MoveNext())
                {
                    if (enumerator.Current.Computer)
                    {
                        ...
                        obj2 = (enumerator.Current.PlayerCamp != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? form.get_gameObject().get_transform().FindChild("DownPanel").FindChild(string.Format("Down_Player{0}", num)).get_gameObject() : form.get_gameObject().get_transform().FindChild("UpPanel").FindChild(string.Format("Up_Player{0}", num)).get_gameObject();
                        if (obj2 != null)
                        {
                            GameObject obj3 = obj2.get_transform().Find("Txt_LoadingPct").get_gameObject();
                            if (obj3 != null)
                            {
                                Text component = obj3.GetComponent<Text>();
                                string str = component.get_text();
                                if (str != "100%")
                                {
                                    int num4;
                                    int num2 = Random.Range(1, 30);
                                    int num3 = Math.Min(100, Convert.ToInt32((float) ((progress * 100f) + Random.Range(0, num2))));
                                    int.TryParse(str.Substring(0, 2), out num4);
                                    if (num3 > num4)
                                    {
                                        component.set_text(string.Format("{0}%", num3));
                                    }
                                }
                            }
                        }
        ...
        public static void FakeSendChat(string content)
        {
            try
            {
                ...
                chatEnt.ullUid = roomInfo.selfInfo.ullUid;
                chatEnt.iLogicWorldID = (uint) roomInfo.selfInfo.iLogicWorldId;
                chatEnt.type = EChaterType.Self;
                chatEnt.text = content;
        ...
        private static void OnSelectHeroTimeout(int seq)
        {
            if (Singleton<CUIManager>.GetInstance().GetForm(CHeroSelectNormalSystem.s_heroSelectFormPath) != null)
            {
                ...
                if ((roomInfo != null) && (roomInfo.GetMasterMemberInfo() != null))
                {
                    ...
                    int num = Random.Range(0, pvPHeroList.Count);
                    IHeroData data = pvPHeroList[num];
                    while (ChosenHeroes.ContainsValue(data.cfgID))
                    {
                        num = Random.Range(0, pvPHeroList.Count);
        ...
        {
            <SelectHeroForEnemyPlayer>c__AnonStorey67 storey = new <SelectHeroForEnemyPlayer>c__AnonStorey67();
            int id = 0;
            storey.heroID = 0;
            ushort dwSkinID = 0;
            uint num3 = 0;
            ...
            if (playerLevel <= 6)
            {
                ...
                while (EnemyChosenHeroes.FindIndex(new Predicate<uint>(storey.<>m__6C)) != -1)
                {
                    id = Random.Range(0, GameDataMgr.robotRookieHeroSkinDatabin.Count());
                    dataByIndex = GameDataMgr.robotRookieHeroSkinDatabin.GetDataByIndex(id);
                    if (dataByIndex == null)
                ...
                if (((dataByIndex != null) && (dataByIndex.dwSkinID != 0)) && (Random.Range(0, 0x2710) < dataByIndex.dwSkinRate))
                {
            ...
            else
            {
                ...
                int num6 = num5 + ((masterRoleInfo == null) ? 0 : masterRoleInfo.freeHeroList.Count);
                id = Random.Range(0, num6);
                if (id < num5)
                {
                    dataByIndex = GameDataMgr.robotVeteranHeroSkinDatabin.GetDataByIndex(id);
                    if (dataByIndex == null)
                ...
                while (EnemyChosenHeroes.FindIndex(new Predicate<uint>(storey.<>m__6D)) != -1)
                {
                    id = Random.Range(0, num6);
                    if (id < num5)
                    {
            ...
            for (COM_PLAYERCAMP com_playercamp = COM_PLAYERCAMP.COM_PLAYERCAMP_1; com_playercamp < COM_PLAYERCAMP.COM_PLAYERCAMP_COUNT; com_playercamp += 1)
            {
                ...
                for (int i = 0; i < view.Count; i++)
                {
                    ...
                    if (info2.RoomMemberType == 2)
                    {
                        ...
                        item.FakePlayer = info2.WarmNpc;
                        item.confirmWaitTime = Random.Range(2, 11);
        ...
        [CompilerGenerated]
        private sealed class <SelectHeroForEnemyPlayer>c__AnonStorey67
        {
            internal uint heroID;

            internal bool <>m__6C(uint x)
            {
                return (x == this.heroID);
            }

            internal bool <>m__6D(uint x)
            {
                return (x == this.heroID);
            }
        }
