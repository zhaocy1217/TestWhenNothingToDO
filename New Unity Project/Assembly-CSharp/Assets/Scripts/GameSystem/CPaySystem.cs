    ...
    public class CPaySystem : Singleton<CPaySystem>
    {
        ...
        private void NeedUpdateChaoZhieGift(CMallFactoryShopController.ShopProduct product)
        {
            ...
            if (((product2 == null) || (product2.IsOnSale != 1)) && ((product3 == null) || (product3.IsOnSale != 1)))
            {
                ...
                if ((form != null) && (form.get_transform().Find("DiamondPayBtn") != null))
                {
                    GameObject obj2 = form.get_transform().Find("DiamondPayBtn").get_gameObject();
                    if ((obj2 != null) && masterRoleInfo.IsClientBitsSet(0))
                    {
        ...
        private void OnDisplayChaoZhiGift(CUIEvent uiEvent)
        {
            ...
            Singleton<CUIManager>.GetInstance().OpenForm(formPath, false, true).get_gameObject().get_transform().FindChild("Panel/Title/Text").get_gameObject().GetComponent<Text>().set_text(Singleton<CTextManager>.instance.GetText("GotoTehuiShopTitle"));
        }
        ...
        private void OnGotoChaoZhiGift(CUIEvent uiEvent)
        {
            ...
            CUICommonSystem.JumpForm(RES_GAME_ENTRANCE_TYPE.RES_GAME_ENTRANCE_SHOP_DISCOUNT, 0, 0);
        }
        ...
        {
            Handheld.PlayFullScreenMovie("Video/HeroVideo.mp4", Color.get_black(), 2, 1);
        }
        ...
        public void RefreshBuyDianQuanPanel()
        {
            ...
            if (form != null)
            {
                ...
                CUIListScript component = form.get_transform().Find("pnlBg/pnlBody/List").GetComponent<CUIListScript>();
                ...
                for (index = 0; index < this.dianQuanBuyInfoList.Count; index++)
                {
                    ...
                    Image image = elemenet.get_transform().Find("imgIcon").GetComponent<Image>();
                    ...
                    elemenet.get_transform().Find("diamondCntText").GetComponent<Text>().set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("Pay_DianQuan_Cnt"), info.dwBuyCount));
                    GameObject obj2 = elemenet.get_transform().Find("buyPanel/buyBtn").get_gameObject();
                    obj2.get_transform().Find("Text").GetComponent<Text>().set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("Pay_DianQuan_Price"), info.dwBuyPrice));
                    GameObject obj3 = elemenet.get_transform().Find("additionPanel").get_gameObject();
                    obj3.CustomSetActive(false);
                    GameObject obj4 = elemenet.get_transform().Find("pnlRecommend").get_gameObject();
                    if ((info.bFirstGift > 0) && !this.IsDianQuanHaveFirstPay(info.dwID))
                    {
                        obj4.CustomSetActive(true);
                        obj4.get_transform().Find("txtDiscount").GetComponent<Text>().set_text(Singleton<CTextManager>.GetInstance().GetText("Pay_First_Pay_Double"));
                    }
                    else
                    {
                        if (info.dwExtraGiftCnt > 0)
                        {
                            ...
                            obj3.get_transform().Find("Text").GetComponent<Text>().set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("Pay_Gift_Diamond_Cnt"), info.dwExtraGiftCnt));
                        }
                        obj4.CustomSetActive(false);
                    }
                    CUIEventScript script4 = obj2.GetComponent<CUIEventScript>();
                ...
                if (CSysDynamicBlock.bLobbyEntryBlocked)
                {
                    Transform transform = form.get_transform().FindChild("Button_OpenNobe");
                    Transform transform2 = form.get_transform().FindChild("Button_HelpMe");
                    Transform transform3 = form.get_transform().FindChild("Button_TongCai");
                    if (transform != null)
                    {
        ...
        private void RefreshFirstPayPanel()
        {
            ...
            if (form != null)
            {
                ...
                Transform transform = form.get_transform().Find("Panel_FirstPay/bodyPanel");
                ...
                GameObject btn = transform.Find("payButton").get_gameObject();
                GameObject obj3 = transform.Find("getButton").get_gameObject();
                CUICommonSystem.SetButtonName(btn, Singleton<CTextManager>.GetInstance().GetText("Pay_Btn_Top_Up"));
                btn.CustomSetActive(!flag);
                CUICommonSystem.SetButtonName(obj3, Singleton<CTextManager>.GetInstance().GetText("Pay_Btn_Get_Reward"));
                obj3.CustomSetActive(flag && !flag2);
            }
        }
        ...
        private void RefreshRenewalPanel()
        {
            ...
            if (form != null)
            {
                ...
                ResRandomRewardStore dataByKey = GameDataMgr.randomRewardDB.GetDataByKey(dwConfValue);
                Transform transform = form.get_transform().Find("Panel_Renewal/bodyPanel");
                transform.Find("rewardDescText").GetComponent<Text>().set_text(StringHelper.UTF8BytesToString(ref dataByKey.szRewardDesc));
                GameObject itemCell = transform.Find("itemCellFirst").get_gameObject();
                ...
                CUICommonSystem.SetItemCell(form, itemCell, itemUseable, true, false, false, false);
                ...
                GameObject btn = transform.Find("payButton").get_gameObject();
                GameObject obj4 = transform.Find("getButton").get_gameObject();
                CUICommonSystem.SetButtonName(btn, Singleton<CTextManager>.GetInstance().GetText("Pay_Btn_Top_Up"));
                btn.CustomSetActive(!flag);
        ...
        private void RefreshRewardList(CUIFormScript form, CUIListScript listScript, ref ResDT_RandomRewardInfo[] rewardInfoArr, int index = 0)
        {
            if ((form != null) && (listScript != null))
            {
                ...
                for (int j = 0; j < amount; j++)
                {
                    ...
                    GameObject itemCell = elemenet.get_transform().Find("itemCell").get_gameObject();
                    CUICommonSystem.SetItemCell(form, itemCell, itemUseable, true, false, false, false);
                }
            }
        }
