    ...
    public class BannerImageCtrl : MonoBehaviour
    {
            ...
            if (((this.m_BannerImage != null) && (uiEvent.m_srcWidgetBelongedListScript == this.m_UIListScript)) && (this.m_AllLoadImageInfo != null))
            {
                ...
                if (srcWidgetIndexInBelongedList < length)
                {
                    ...
                    if (info != null)
                    {
                        ...
                        {
                            CUICommonSystem.OpenUrl(info.resImgInfo.szHttpUrl, true, 0);
                        }
                        else if (info.resImgInfo.dwBannerType == 2)
                        {
                            CUICommonSystem.JumpForm((RES_GAME_ENTRANCE_TYPE) info.resImgInfo.dwJumpEntrance, info.resImgInfo.iTargetID, info.resImgInfo.iTargetID2);
                        }
                    }
                }
            }
        ...
        public bool InitSys()
        {
            ...
            if (this.m_BannerImage != null)
            {
                ...
                if (amount > 0)
                {
                    ...
                    this.m_fBeginTime = Time.get_time();
                    return true;
                }
        ...
        private void SetElemntTexture(int idx, Texture2D texture2D)
        {
            if (this.m_UIListScript != null)
            {
                ...
                if (elemenet != null)
                {
                    elemenet.get_transform().Find("Img").GetComponent<Image>().SetSprite(Sprite.Create(texture2D, new Rect(0f, 0f, (float) texture2D.get_width(), (float) texture2D.get_height()), new Vector2(0.5f, 0.5f)), ImageAlphaTexLayout.None);
                }
            }
        }
        ...
        private void Update()
        {
            if ((this.m_bUpdateStart && !this.m_bStopAutoMove) && ((Time.get_time() - this.m_fBeginTime) >= this.m_fSpeed))
            {
                this.AutoMoveBannerImage();
                this.m_fBeginTime = Time.get_time();
            }
