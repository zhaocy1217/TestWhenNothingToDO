namespace Assets.Scripts.GameSystem
{
    ...
    public class HeroInfoItem
    {
        ...
        public HeroInfoItem(COM_PLAYERCAMP _listCamp, int _listIndex, HeroKDA _heroKDA, GameObject baseInfoItem, GameObject equipInfoItem)
        {
            ...
            this._baseInfoItem.GetComponent<CUIEventScript>().set_enabled(true);
            ...
            this._equipInfoItem.GetComponent<CUIEventScript>().set_enabled(true);
            this._equipIcons = new Image[6];
            for (int i = 0; i < 6; i++)
            {
                this._equipIcons[i] = Utility.GetComponetInChild<Image>(equipInfoItem, "Icon_" + i);
                this._equipIcons[i].get_gameObject().CustomSetActive(true);
            }
            this._headImg.SetSprite(string.Format("{0}{1}", CUIUtility.s_Sprite_Dynamic_BustCircle_Dir, CSkinInfo.GetHeroSkinPic((uint) this._heroInfo.HeroId, 0)), Singleton<CBattleSystem>.GetInstance().WatchFormScript, true, false, false, false);
            this.ValidateLevel();
            this.ValidateKDA();
            this.ValidateMoney();
            this.ValidateEquip();
        ...
        {
            baseInfoItem.GetComponent<CUIEventScript>().set_enabled(false);
            Utility.FindChild(baseInfoItem, "Empty").CustomSetActive(true);
            equipInfoItem.GetComponent<CUIEventScript>().set_enabled(false);
            for (int i = 0; i < 6; i++)
            {
        ...
        {
            this._kdaText.set_text(string.Concat(new object[] { this._heroInfo.numKill, " / ", this._heroInfo.numDead, " / ", this._heroInfo.numAssist }));
        }
        ...
        public void ValidateReviceCD()
        {
            ...
            {
                this._reviveTxt.set_text(string.Format("{0}", Mathf.FloorToInt(this._heroInfo.actorHero.handle.ActorControl.ReviveCooldown * 0.001f)));
