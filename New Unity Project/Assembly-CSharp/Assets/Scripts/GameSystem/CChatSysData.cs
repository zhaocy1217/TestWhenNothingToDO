        ...
        public void Add_NewContent_Entry_ColorFlag(string a, EChatChannel curChannel)
        {
            if ((curChannel != EChatChannel.Lobby) || !Singleton<CLoudSpeakerSys>.instance.IsSpeakerShowing())
            {
                ...
                if (this.entryEntity != null)
                {
                    ...
                    int actualWidth = 0;
                    List<string> list = Singleton<CChatController>.instance.ColorParser.Parse(lingWidth, this.entryEntity.text, Singleton<CChatController>.instance.view.ChatParser.viewFontSize, out actualWidth);
                    if (list.Count >= 1)
                    {
                        this.entryEntity.TextObjList.Add(new CTextImageNode(list[0], true, (float) actualWidth, 0f, 0f, 0f));
                    }
                }
            }
        }
