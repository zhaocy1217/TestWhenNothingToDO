namespace Assets.Scripts.GameSystem
{
    ...
    public class SoldierWave
    {
        ...
        private void OnNextRepeat(ref SoldierWaveParam inParam)
        {
            ...
            {
                if ((inParam.NextDuration >= 0) && this._countdownText.get_gameObject().get_activeSelf())
                {
                    this._countdown = inParam.NextDuration;
                }
                else
                {
                    this._countdownText.get_gameObject().CustomSetActive(false);
                    this._countdownTitle.get_gameObject().CustomSetActive(false);
                }
            }
        }
        ...
        public void Update()
        {
            ...
            if (!mapLogic.DoesSoldierOverNum())
            {
                ...
                {
                    int num = (int) (Time.get_deltaTime() * 1000f);
                    this._countdown -= num;
            ...
            if ((this._countdownText != null) && this._countdownText.get_gameObject().get_activeSelf())
            {
                int outMin = 0;
                int outSec = 0;
                this.CalcMinSec(this._countdown, out outSec, out outMin);
                this._countdownText.set_text(string.Format("{0:D2} : {1:D2}", outMin, outSec));
            }
        }
    }
}
