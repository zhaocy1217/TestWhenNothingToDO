    ...
    public class HeroInfoPanel
    {
                    ...
                    else
                    {
                        ...
                        for (index = 0; index < allPlayers.Count; index++)
                        {
                            ...
                            if ((allPlayers[index].PlayerCamp == hostPlayer.PlayerCamp) && (allPlayers[index] != hostPlayer))
                            {
                                ...
                                {
                                    elementScript.get_gameObject().CustomSetActive(true);
        ...
        private void InitHeroItemData(CUIListElementScript elementScript, PoolObjHandle<ActorRoot> actor)
        {
            if (((null != elementScript) && (actor != 0)) && (actor.handle.ValueComponent != null))
            {
                ...
                elementScript.get_gameObject().CustomSetActive(bActive || actor.handle.ActorControl.IsDeadState);
                ...
                if (obj4 != null)
                {
                    if (actor.handle.ActorControl.IsDeadState)
                    {
                        ...
                        obj4.GetComponent<Text>().set_text(string.Format("{0}", Mathf.RoundToInt(actor.handle.ActorControl.ReviveCooldown * 0.001f)));
                    }
                    else
                    {
                        obj4.CustomSetActive(false);
        ...
        private void RefreshEnemyHeroListUI()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                if (elementScript != null)
                {
                    elementScript.get_gameObject().CustomSetActive(i < count);
                    if (i < count)
                    {
        ...
        private void RefreshHeroIconImageColor(CUIListElementScript elementScript, PoolObjHandle<ActorRoot> actor)
        {
            if ((null != elementScript) && (actor != 0))
            {
                ...
                if (widget != null)
                {
                    ...
                    {
                        component.set_color(!actor.handle.ActorControl.IsDeadState ? CUIUtility.s_Color_White : CUIUtility.s_Color_GrayShader);
                    }
                    else
                    {
        ...
        private void RefreshHeroReviveCntDown(CUIListElementScript elementScript, PoolObjHandle<ActorRoot> actor)
        {
            if ((null != elementScript) && (actor != 0))
            {
                ...
                {
                    widget.GetComponent<Text>().set_text(SimpleNumericString.GetNumeric(Mathf.RoundToInt(actor.handle.ActorControl.ReviveCooldown * 0.001f)));
                }
            }
        }
                    ...
                    if (((handle != 0) && (handle.handle.SkillControl != null)) && (null != elemenet))
                    {
                        ...
                        if (num2 != 0)
                        {
                            if ((num2 & 1) != 0)
                            {
                                ...
                                if (hpImage != null)
                                {
                                    hpImage.set_fillAmount(handle.handle.ValueComponent.GetHpRate().single);
                                }
                                num2 &= -2;
                            }
                            ...
                            {
                                reviveText.set_text(SimpleNumericString.GetNumeric(Mathf.RoundToInt(handle.handle.ActorControl.ReviveCooldown * 0.001f)));
                            }
                ...
                for (index = 0; index < num4; index++)
                {
                    if ((this.heroInfoList.GetElemenet(index + 5) != null) && (this.m_enemyHeroList[index] != 0))
                    {
                        ...
                        if (text2 != null)
                        {
                            ...
                            text2.set_text(SimpleNumericString.GetNumeric(Mathf.RoundToInt(handle2.handle.ActorControl.ReviveCooldown * 0.001f)));
                        }
                    }
                }
            }
