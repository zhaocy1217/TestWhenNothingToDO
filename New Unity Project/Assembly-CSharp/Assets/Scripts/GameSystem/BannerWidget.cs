namespace Assets.Scripts.GameSystem
{
    ...
    public class BannerWidget : ActivityWidget
    {
        ...
        private void updateAutoScroll()
        {
            ...
            if (((count > 1) && (this._curStepIndex < count)) && (Time.get_time() > (this._lastScrollTime + (this._urlaList[this._curStepIndex].showTime * 0.001f))))
            {
                this._lastScrollTime = Time.get_time();
        ...
        private void updateOverTime()
        {
            ...
            if (action.overTime > 0L)
            {
                ...
                this._timeCD.get_gameObject().CustomSetActive(time >= time2);
                this._timeCD.set_text(str);
            }
            else
            {
                this._timeCD.get_gameObject().CustomSetActive(false);
            }
        }
