        ...
        public static CUseable CreateUsableByServerType(COM_REWARDS_TYPE type, int cnt, uint baseId)
        {
            ...
            switch (type)
            {
                ...
                case COM_REWARDS_TYPE.COM_REWARDS_TYPE_ACHIEVE:
                    CreateVirtualUseable(enVirtualItemType.enAchievementPoint, cnt);
                    return useable;

                case COM_REWARDS_TYPE.COM_REWARDS_TYPE_MASTERPOINT:
                    return CreateVirtualUseable(enVirtualItemType.enMentorPoint, cnt);
            }
            return useable;
        }
        ...
        public static ListView<CUseable> CreateUsableListByRandowReward(RES_RANDOM_REWARD_TYPE type, int cnt, uint baseId)
        {
            ...
            if (dataByKey != null)
            {
                for (int i = 0; i < dataByKey.astRewardDetail.Length; i++)
                {
                    ...
                    if (dataByKey.astRewardDetail[i].bItemType >= 0x12)
                    {
                        return view;
                    }
        ...
        public static ListView<CUseable> GetUseableListFromReward(COMDT_REWARD_DETAIL reward)
        {
            ...
            for (int i = 0; i < reward.bNum; i++)
            {
                ...
                switch (reward.astRewardDetail[i].bType)
                {
                    ...
                    case 0x15:
                    {
                        useable = CreateUsableByServerType((COM_REWARDS_TYPE) reward.astRewardDetail[i].bType, (int) reward.astRewardDetail[i].stRewardInfo.dwAchieve, 0);
                        if (useable != null)
                        {
                            view.Add(useable);
                        }
                    ...
                    case 0x16:
                    {
                        useable = CreateUsableByServerType((COM_REWARDS_TYPE) reward.astRewardDetail[i].bType, (int) reward.astRewardDetail[i].stRewardInfo.dwMasterPoint, 0);
                        if (useable != null)
                        {
                            view.Add(useable);
                        }
