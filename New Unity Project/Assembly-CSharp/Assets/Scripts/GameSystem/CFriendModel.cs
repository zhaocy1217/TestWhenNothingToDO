namespace Assets.Scripts.GameSystem
{
    ...
    public class CFriendModel
    {
        ...
        [CompilerGenerated]
        private bool <bShowUID>k__BackingField;
        ...
        public CFriendModel()
        {
            Array values = Enum.GetValues(typeof(FriendType));
        ...
        public void Add(FriendType type, COMDT_FRIEND_INFO data, bool ingore_worldid = false)
        {
            ...
            if (list != null)
            {
                ...
                if ((type == FriendType.RequestFriend) || (type == FriendType.MentorRequestList))
                {
        ...
        public void AddFriendIntimacy(ulong ullUid, uint dwLogicWorldID, uint lastTime, ushort wIntimacyValue, byte bIntimacyType, uint dwTerminateTime)
        {
            ...
            this.m_stFriendIntimacyList.Add(new stFriendIntimacy(ullUid, dwLogicWorldID, lastTime, wIntimacyValue, bIntimacyType, dwTerminateTime));
        }

        public void AddFriendVerifyContent(ulong ullUid, uint dwLogicWorldID, string content, enVerifyDataSet verifyType, COMDT_FRIEND_SOURCE friendSource, int mentor_type = 0)
        {
            int index = this.GetFriendVerifyIndex(ullUid, dwLogicWorldID, verifyType);
            if (index != -1)
            {
                this.m_friendVerifyList[(int) verifyType].RemoveAt(index);
            }
        ...
        {
            stBlackName name = new stBlackName();
            name.ullUid = info.stUin.ullUid;
            name.dwLogicWorldId = info.stUin.dwLogicWorldId;
            name.name = UT.Bytes2String(info.szUserName);
            name.bIsOnline = info.bIsOnline;
            name.dwLastLoginTime = info.dwLastLoginTime;
            name.bGender = info.bGender;
            name.szHeadUrl = UT.Bytes2String(info.szHeadUrl);
            name.dwPvpLvl = info.dwPvpLvl;
            return name;
        }
        ...
        {
            stBlackName name = new stBlackName();
            name.ullUid = info.ullUid;
            name.dwLogicWorldId = (uint) info.iLogicWorldID;
            name.name = UT.Bytes2String(info.szName);
            name.bIsOnline = 0;
            name.bGender = bGender;
            name.szHeadUrl = UT.Bytes2String(info.szHeadUrl);
            name.dwPvpLvl = info.dwLevel;
            name.dwLastLoginTime = dwLastLoginTime;
            return name;
        }
        ...
        public void FilterRecommendFriends()
        {
            ...
            if (view != null)
            {
                ...
                {
                    if (this.getFriendByUid(view[i].stUin.ullUid, FriendType.GameFriend, 0) != null)
                    {
                        view.RemoveAt(i);
                        i--;
                    }
        ...
        public ListView<COMDT_FRIEND_INFO> GetAllFriend(bool containMentorApprentice = true)
        {
            ...
            ListView<COMDT_FRIEND_INFO> view3 = this._map[6];
            ListView<COMDT_FRIEND_INFO> view4 = this._map[7];
            ...
            {
                for (int j = 0; j < view3.Count; j++)
                {
                    if (!Singleton<CFriendContoller>.instance.FilterSameFriend(view3[j], friendList))
                    {
                        friendList.Add(view3[j]);
                    }
                }
                for (int k = 0; k < view4.Count; k++)
                {
                    if (!Singleton<CFriendContoller>.instance.FilterSameFriend(view4[k], friendList))
                    {
                        friendList.Add(view4[k]);
                    }
                }
            }
            return friendList;
        ...
        public COMDT_FRIEND_INFO getFriendByUid(ulong uid, FriendType friendType, uint worldID = 0)
        {
            ListView<COMDT_FRIEND_INFO> view = this._map[(uint) friendType];
            for (int i = 0; i < view.Count; i++)
            {
                if ((view[i].stUin.ullUid == uid) && ((view[i].stUin.dwLogicWorldId == worldID) || (worldID == 0)))
                {
                    return view[i];
                }
            }
        ...
        public string GetFriendVerifyContent(ulong ullUid, uint dwLogicWorldID, enVerifyDataSet verifyType)
        {
            for (int i = 0; i < this.m_friendVerifyList[(int) verifyType].Count; i++)
            {
                stFriendVerifyContent content = this.m_friendVerifyList[(int) verifyType][i];
        ...
        public stFriendVerifyContent GetFriendVerifyData(ulong ullUid, uint dwLogicWorldID, enVerifyDataSet verifyType)
        {
            for (int i = 0; i < this.m_friendVerifyList[(int) verifyType].Count; i++)
            {
                stFriendVerifyContent content = this.m_friendVerifyList[(int) verifyType][i];
        ...
        private int GetFriendVerifyIndex(ulong ullUid, uint dwLogicWorldID, enVerifyDataSet verifyType)
        {
            for (int i = 0; i < this.m_friendVerifyList[(int) verifyType].Count; i++)
            {
                stFriendVerifyContent content = this.m_friendVerifyList[(int) verifyType][i];
        ...
        public COMDT_FRIEND_SOURCE GetFriendVerifySource(ulong ullUid, uint dwLogicWorldID, enVerifyDataSet verifyType)
        {
            for (int i = 0; i < this.m_friendVerifyList[(int) verifyType].Count; i++)
            {
                stFriendVerifyContent content = this.m_friendVerifyList[(int) verifyType][i];
        ...
        public COMDT_FRIEND_INFO GetGameOrSnsFriend(ulong ullUid, uint dwLogicWorldID)
        {
            ...
            if (comdt_friend_info == null)
            {
                comdt_friend_info = this.getFriendInfo(ullUid, dwLogicWorldID, this.GetList(FriendType.Mentor));
            }
            if (comdt_friend_info == null)
            {
                comdt_friend_info = this.getFriendInfo(ullUid, dwLogicWorldID, this.GetList(FriendType.Apprentice));
            }
        ...
        {
            ListView<COMDT_FRIEND_INFO> view;
            if (this._map.TryGetValue((uint) type, out view))
            {
                return view;
            }
            return null;
        ...
        {
            if (this.IntimacyNumMax == 0)
            {
                this.IntimacyNumMax = GameDataMgr.globalInfoDatabin.GetDataByKey((uint) 0xc3).dwConfValue;
            }
        ...
        public string GetRandomVerifyContent()
        {
            ...
            int num = Random.Range(0, this.m_preconfigVerifyContentList.Count - 1);
        ...
        public ListView<COMDT_FRIEND_INFO> GetValidChatFriendList()
        {
            ...
            ListView<COMDT_FRIEND_INFO> list = this.GetList(FriendType.GameFriend);
            ListView<COMDT_FRIEND_INFO> view2 = this.GetList(FriendType.SNS);
            ListView<COMDT_FRIEND_INFO> view3 = this.GetList(FriendType.Mentor);
            ListView<COMDT_FRIEND_INFO> view4 = this.GetList(FriendType.Apprentice);
            if (view2 != null)
            {
                for (int i = 0; i < view2.Count; i++)
                {
            ...
            if (view3 != null)
            {
                ...
                {
                    a = view3[k];
                    if ((a != null) && (this.GetItemIndex(a.stUin.ullUid, a.stUin.dwLogicWorldId, this.cache_chatFriends_Results) == -1))
                    {
                        this.cache_chatFriends_Results.Add(a);
                    }
                }
            ...
            {
                for (int n = 0; n < friendChannelList.Count; n++)
                {
                    channel = friendChannelList[n];
        ...
        public bool IsGameFriend(ulong ullUid, uint logicWorldId)
        {
            ...
            for (int i = 0; i < view.Count; i++)
            {
                if ((view[i].stUin.ullUid == ullUid) && Utility.IsSamePlatform(view[i].stUin.dwLogicWorldId, logicWorldId))
                {
                    return true;
                }
            }
        ...
        public COMDT_FRIEND_INFO Remove(FriendType type, COMDT_ACNT_UNIQ uniq)
        {
        ...
        public void Remove(FriendType type, COMDT_FRIEND_INFO data)
        {
            ...
            if (list != null)
            {
                ...
                if ((type == FriendType.Recommend) || (type == FriendType.MentorRequestList))
                {
        ...
        public void RemoveFriendVerifyContent(ulong ullUid, uint dwLogicWorldID, enVerifyDataSet verifyType)
        {
            int index = this.GetFriendVerifyIndex(ullUid, dwLogicWorldID, verifyType);
            if (index != -1)
            {
        ...
        public void SortShower(FriendType type)
        {
            ListView<COMDT_FRIEND_INFO> list = this.GetList(type);
            if (list != null)
            {
                if (type == FriendType.Mentor)
                {
        ...
        {
            ListView<COMDT_FRIEND_INFO> list = this.GetList(FriendType.SNS);
            if (list != null)
            {
                list.Sort(new Comparison<COMDT_FRIEND_INFO>(CFriendModel.FriendDataSort));
            }
        ...
        public bool bShowUID
        {
            ...
            get
            {
                return this.<bShowUID>k__BackingField;
            }
            [CompilerGenerated]
            set
            {
                this.<bShowUID>k__BackingField = value;
            }
        }
