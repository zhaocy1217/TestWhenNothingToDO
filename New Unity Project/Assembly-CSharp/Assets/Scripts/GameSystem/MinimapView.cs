    ...
    public class MinimapView
    {
            ...
            if ((inCharm != null) && !this.m_mapPointer.ContainsKey(inCharm))
            {
                ...
                if (this.m_mapPointerContainer != null)
                {
                    ...
                    if (element >= 0)
                    {
                        ...
                        if (obj2 != null)
                        {
                            mapPointerRectTransform = obj2.get_transform() as RectTransform;
                            obj2.GetComponent<Image>().SetSprite(string.Format("{0}{1}", "UGUI/Sprite/Battle/", "Img_Map_Base_Green"), Singleton<CBattleSystem>.instance.FormScript, true, false, false, false);
                        }
                        if (mapPointerRectTransform != null)
                        {
                            mapPointerRectTransform.SetAsFirstSibling();
                        }
                        Vector3 actorPosition = Vector3.get_zero();
                        if (inCharm.Presentation != null)
                        {
                            actorPosition = inCharm.Presentation.get_transform().get_position();
                        }
                        SLevelContext curLvelContext = Singleton<BattleLogic>.GetInstance().GetCurLvelContext();
                        if ((curLvelContext != null) && curLvelContext.IsMobaMode())
                        {
                            this.UpdateUIMap(mapPointerRectTransform, actorPosition, (float) curLvelContext.m_mapWidth, (float) curLvelContext.m_mapHeight);
                        }
        ...
        private void AddSpawnGroupCounter(int inCountdown, int inAlertPreroll, Vector3 inInitPos, SpawnerWrapper.ESpawnObjectType inObjType)
        {
            ...
            if (this.m_spawnGroupCounterContainer != null)
            {
                ...
                if (element >= 0)
                {
                    SpawnGroupCounter item = new SpawnGroupCounter();
                    item.CountdownTime = inCountdown;
                    item.timer = item.CountdownTime;
                    item.AlertTime = item.timer - inAlertPreroll;
                    item.bAlertPreroll = inAlertPreroll > 0;
                    item.bDidAlert = false;
                    RectTransform mapPointerRectTransform = null;
                    GameObject p = this.m_spawnGroupCounterContainer.GetElement(element);
                    if (p != null)
                    {
                        mapPointerRectTransform = p.get_transform() as RectTransform;
                        item.TextObj = p;
                        item.timerText = Utility.FindChild(p, "TimerText").GetComponent<Text>();
                    }
                    if (mapPointerRectTransform != null)
                    {
                        mapPointerRectTransform.SetAsFirstSibling();
                    ...
                    this.m_spawnGroupCounter.Add(item);
                }
            }
        }
        ...
        private void UpdateUIMap(RectTransform mapPointerRectTransform, Vector3 actorPosition, float mapWidth, float mapHeight)
        {
            ...
            {
                float num = actorPosition.x * Singleton<CBattleSystem>.instance.world_UI_Factor_Small.x;
                float num2 = actorPosition.z * Singleton<CBattleSystem>.instance.world_UI_Factor_Small.y;
                mapPointerRectTransform.set_anchoredPosition(new Vector2(num, num2));
            }
        }
