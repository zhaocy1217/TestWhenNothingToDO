    ...
    internal class CRoomSystem : Singleton<CRoomSystem>
    {
        ...
        public void BuildRoomInfo(COMDT_JOINMULTGAMERSP_SUCC roomData)
        {
            ...
            if (masterRoleInfo != null)
            {
                ...
                this.roomInfo.roomAttrib.judgeNum = (pvpMapCommonInfo == null) ? ((int) 0) : ((int) pvpMapCommonInfo.dwJudgeNum);
        ...
        private MemberInfo CreateMemInfo(ref COMDT_ROOMMEMBER_DT memberDT, COM_PLAYERCAMP camp, bool bWarmBattle)
        {
            ...
            info.RoomMemberType = memberDT.dwRoomMemberType;
            info.dwPosOfCamp = memberDT.dwPosOfCamp;
            info.camp = camp;
            if (memberDT.dwRoomMemberType == 1)
            {
                ...
                info.recentUsedHero = new COMDT_RECENT_USED_HERO();
        ...
        {
            CUIListScript component = form.get_transform().FindChild("Panel_Main/List").GetComponent<CUIListScript>();
        ...
        private void GetMemberPosInfo(GameObject go, out COM_PLAYERCAMP Camp, out int Pos)
        {
            ...
            if (go.get_name().StartsWith("Left"))
            {
                Camp = COM_PLAYERCAMP.COM_PLAYERCAMP_1;
            }
            else if (go.get_name().StartsWith("Right"))
            {
            ...
            else if (go.get_name().EndsWith("4"))
            {
                Pos = 3;
            }
            else if (go.get_name().EndsWith("5"))
            {
                Pos = 4;
            }
        }
        ...
        private void InitMaps(CUIFormScript rootFormScript)
        {
            ...
            for (int i = 0; i < numArray.Length; i++)
            {
                if ((numArray[i] != 0) && (CLevelCfgLogicManager.GetPvpMapCommonInfo((byte) numArray2[i], numArray[i]) != null))
                {
                    ...
                    item.mapType = (byte) numArray2[i];
                    item.mapID = numArray[i];
                    this.mapList.Add(item);
                }
            }
            CUIListScript component = rootFormScript.get_transform().Find("Panel_Main/List").get_gameObject().GetComponent<CUIListScript>();
            ...
            for (int j = 0; j < component.m_elementAmount; j++)
            {
                ...
                Image image = elemenet.get_transform().GetComponent<Image>();
                string prefabPath = CUIUtility.s_Sprite_Dynamic_PvpEntry_Dir + this.mapList[j].mapID;
                image.SetSprite(prefabPath, rootFormScript, true, false, false, false);
                uint result = 0;
                uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_5V5Miwu"), out result);
                if (this.mapList[j].mapID == result)
                {
                    elemenet.get_transform().FindChild("Flag").get_gameObject().CustomSetActive(true);
                }
            }
            component.SelectElement(-1, true);
            if (CSysDynamicBlock.bLobbyEntryBlocked)
            {
                Transform transform = rootFormScript.get_transform().Find("panelGroupBottom");
                if (transform != null)
                {
        ...
        public static void OnPlayerJoinRoom(CSPkg msg)
        {
            ...
            if (msg.stPkgData.stJoinMultGameRsp.iErrCode == 0)
            {
                ...
                CChatUT.EnterRoom();
                CRoomView.SetRoomData(script.get_gameObject(), instance.roomInfo);
                CRoomObserve.SetObservers(Utility.FindChild(script.get_gameObject(), "Panel_Main/Observers"), instance.roomInfo.roomAttrib.judgeNum, instance.roomInfo[COM_PLAYERCAMP.COM_PLAYERCAMP_MID], instance.roomInfo.GetMasterMemberInfo());
                ...
                if (MonoSingleton<ShareSys>.instance.IsQQGameTeamCreate())
                {
                    ...
                    MonoSingleton<ShareSys>.instance.SendQQGameTeamStateChgMsg(ShareSys.QQGameTeamEventType.join, COM_ROOM_TYPE.COM_ROOM_TYPE_NORMAL, instance.roomInfo.roomAttrib.bMapType, instance.roomInfo.roomAttrib.dwMapId, roomStr);
                }
        ...
        {
            GameObject go = uiEvent.m_srcWidget.get_transform().get_parent().get_parent().get_gameObject();
            COM_PLAYERCAMP camp = COM_PLAYERCAMP.COM_PLAYERCAMP_1;
            int pos = 0;
            this.GetMemberPosInfo(go, out camp, out pos);
        ...
        private void OnRoom_CreateRoom(CUIEvent uiEvent)
        {
            if (this.mapId > 0)
            {
                ...
                ReqCreateRoom(this.mapId, this.mapType, false);
            }
        }
        ...
        public static void OnRoomChange(CSPkg msg)
        {
            ...
            else
            {
                ...
                else if (msg.stPkgData.stRoomChgNtf.stRoomChgInfo.iChgType != 1)
                {
                    ...
                    else if (msg.stPkgData.stRoomChgNtf.stRoomChgInfo.iChgType == 3)
                    {
                        ...
                        if ((bOldState == enRoomState.E_ROOM_WAIT) && (bNewState == enRoomState.E_ROOM_CONFIRM))
                        {
                            ...
                            uiEvent.m_eventID = enUIEventID.Matching_OpenConfirmBox;
                ...
                if (form != null)
                {
                    if (flag)
                    {
                        CRoomView.SetRoomData(form.get_gameObject(), roomInfo);
                        CRoomObserve.SetObservers(Utility.FindChild(form.get_gameObject(), "Panel_Main/Observers"), roomInfo.roomAttrib.judgeNum, roomInfo[COM_PLAYERCAMP.COM_PLAYERCAMP_MID], roomInfo.GetMasterMemberInfo());
                    }
                    if (flag2)
                    {
                        Singleton<CRoomSystem>.instance.RestMasterSwapInfo();
        ...
        public static void ReqCreateRoomAndInvite(uint mapId, COM_BATTLE_MAP_TYPE mapType, CInviteSystem.stInviteInfo inviteInfo)
        {
            ...
            ReqCreateRoom(mapId, (byte) mapType, true);
        }
            ...
            if (this.roomInfo == null)
            {
                ...
                for (int i = 0; i < inCampInfo.Length; i++)
                {
                    ...
                    for (int j = 0; j < csdt_campinfo.dwPlayerNum; j++)
                    {
                        ...
                        item.recentUsedHero = csdt_campinfo.astCampPlayerInfo[j].stRecentUsedHero;
                ...
                for (int k = 0; k < inCampInfo.Length; k++)
                {
                    ...
                    for (int m = 0; m < csdt_campinfo2.dwPlayerNum; m++)
                    {
                        ...
                        if (memberInfo != null)
                        {
                            ...
                            memberInfo.recentUsedHero = csdt_campinfo2.astCampPlayerInfo[m].stRecentUsedHero;
        ...
        public void UpdateRoomInfoReconnectPick(COMDT_DESKINFO inDeskInfo, CSDT_RECONN_CAMPPICKINFO[] inCampInfo)
        {
            ...
            for (int i = 0; i < inCampInfo.Length; i++)
            {
                ...
                for (int j = 0; j < csdt_reconn_camppickinfo.dwPlayerNum; j++)
                {
                    ...
                    item.ChoiceHero = stPlayerInfo.astChoiceHero;
                    item.canUseHero = stUsableHero.HeroDetail;
                    item.recentUsedHero = csdt_reconn_camppickinfo.astPlayerInfo[j].stPickHeroInfo.stRecentUsedHero;
