namespace Assets.Scripts.GameSystem
{
    ...
    public class CEquipRelationPath
    {
        ...
        private void DisplayEquipLineSet(ushort equipID1, ushort equipID2)
        {
            ...
            if ((this.m_equipPositionDictionary.TryGetValue(equipID1, out vector) && this.m_equipPositionDictionary.TryGetValue(equipID2, out vector2)) && (Mathf.Abs(vector.x - vector2.x) == 1f))
            {
                Vector2 vector3 = (vector.x >= vector2.x) ? vector2 : vector;
                Vector2 vector4 = (vector.x >= vector2.x) ? vector : vector2;
                int x = (int) vector3.x;
        ...
        {
            Vector2 vector = Vector2.get_zero();
            if (((this.m_equipPositionDictionary.TryGetValue(equipID, out vector) && (vector.x >= 0f)) && ((vector.x < this.m_equipNodes.Length) && (vector.y >= 0f))) && (vector.y < this.m_equipNodes[(int) vector.x].Length))
            {
                this.m_equipNodes[(int) vector.x][(int) vector.y] = true;
            }
        }
