namespace Assets.Scripts.GameSystem
{
    ...
    public class MiniMapCameraFrame_3DUI
    {
        ...
        {
            this.m_node = node;
            this.m_frameImgNormal = this.m_node.get_transform().Find("normal").GetComponent<Image>();
            this.m_frameImgRed = this.m_node.get_transform().Find("red").GetComponent<Image>();
            ...
            if ((this.m_frameImgNormal != null) && (this.m_frameImgRed != null))
            {
                ...
                if (component != null)
                {
                    ...
                    if (transform2 != null)
                    {
                        if (flag)
                        {
                            component.set_sizeDelta(new Vector2(this.m_SFrameWidth, this.m_SFrameHeight));
                            transform2.set_sizeDelta(new Vector2(this.m_SFrameWidth, this.m_SFrameHeight));
                            this.curFrameHalfWidth = this.m_SFrameWidth * 0.5f;
                            this.curFrameHalfHeight = this.m_SFrameHeight * 0.5f;
                        }
                        else
                        {
                            component.set_sizeDelta(new Vector2(this.m_BFrameWidth, this.m_BFrameHeight));
                            transform2.set_sizeDelta(new Vector2(this.m_BFrameWidth, this.m_BFrameHeight));
        ...
        public void SetPos(float x, float y)
        {
            if ((this.m_frameImgNormal != null) && (this.m_frameImgRed != null))
            {
                ...
                {
                    component.set_anchoredPosition(new Vector2(num, num2));
                    transform2.set_anchoredPosition(new Vector2(num, num2));
                }
            }
        }
        ...
        public void ShowNormal()
        {
            ...
            {
                this.m_frameImgNormal.get_gameObject().CustomSetActive(true);
            }
            if (this.m_frameImgRed != null)
            {
                this.m_frameImgRed.get_gameObject().CustomSetActive(false);
            }
            this.m_startCooldownTimestamp = 0L;
        }
        ...
        public void ShowRed()
        {
            if (this.m_frameImgNormal != null)
            {
                this.m_frameImgNormal.get_gameObject().CustomSetActive(false);
            }
            if (this.m_frameImgRed != null)
            {
                this.m_frameImgRed.get_gameObject().CustomSetActive(true);
            }
