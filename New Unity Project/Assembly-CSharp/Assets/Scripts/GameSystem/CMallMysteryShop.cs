namespace Assets.Scripts.GameSystem
{
    ...
    public class CMallMysteryShop : Singleton<CMallMysteryShop>
    {
        ...
        public static void GetRollDiscount(CSPkg msg)
        {
            ...
            if (script != null)
            {
                CUICommonSystem.PlayAnimator(Utility.FindChild(script.get_gameObject(), "Panel_NewDiscount/Content/Discount"), string.Format("Discount_{0}", bZheKou));
            }
        }
        ...
        public void InitData()
        {
            ...
            if (!this.m_Inited)
            {
                ...
                if (masterRoleInfo != null)
                {
                    ...
                    {
                        DictionaryView<uint, ResAkaliShopGoods>.Enumerator enumerator2;
                    ...
                    Label_0153:
                        enumerator2 = GameDataMgr.mysteryShopProductDict.GetEnumerator();
                        ...
                        Label_0215:
                            ...
                            if (GameDataMgr.IsSkinAvailable(heroSkin.dwID) && GameDataMgr.IsHeroAvailable(info3.dwCfgID))
                            {
                                this.m_Products.Add(item);
                            }
                            goto Label_027A;
        ...
        private void OnOpenBuyForm(CUIEvent uiEvent)
        {
            ...
            if ((this.m_Products != null) && (srcWidgetIndexInBelongedList <= this.m_Products.Count))
            {
                ...
                if (product != null)
                {
                    product.OpenBuy(uiEvent.m_srcFormScript, uiEvent.m_srcWidget.get_transform());
                }
            }
        }
        ...
        private void RefreshDiscount()
        {
            ...
            if ((mallForm != null) && Singleton<CMallSystem>.GetInstance().m_IsMallFormOpen)
            {
                ...
                GameObject obj4 = Utility.FindChild(mallForm.get_gameObject(), "pnlBodyBg/pnlMystery/Content/Banner/Discount/DiscountBg");
                ...
                if (this.m_HasGotDiscount)
                {
                    ...
                    if (obj3 != null)
                    {
                        ...
                        if (text != null)
                        {
                            text.set_text(string.Format("{0}/{1}", this.BoughtCnt, this.LimitBuyCnt));
                        }
                    }
                }
                else
        ...
        private void RefreshProducts()
        {
            ...
            storeyc.role = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
            DebugHelper.Assert(storeyc.role != null, "master roleInfo is null");
            if (storeyc.role != null)
            {
                if (this.m_Products != null)
                {
                    this.m_Products.Sort(new Comparison<CMallMysteryProduct>(storeyc.<>m__6D));
                }
                CUIFormScript mallForm = Singleton<CMallSystem>.GetInstance().m_MallForm;
                if ((mallForm != null) && Singleton<CMallSystem>.GetInstance().m_IsMallFormOpen)
                {
        ...
        private sealed class <RefreshProducts>c__AnonStorey7C
        {
            ...
            internal int <>m__6D(CMallMysteryProduct l, CMallMysteryProduct r)
            {
                if (l == null)
                {
                    return -1;
