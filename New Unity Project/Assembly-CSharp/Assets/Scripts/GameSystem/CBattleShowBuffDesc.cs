namespace Assets.Scripts.GameSystem
{
    ...
    public class CBattleShowBuffDesc
    {
        ...
        private bool InitShowBuffDesc()
        {
            ...
            this.m_textBgImage.get_gameObject().CustomSetActive(true);
            return true;
        }
            ...
            if (uiEvent.m_srcWidget != null)
            {
                ...
                if ((index < 5) && ((this.m_ArrShowBuffSkill != null) && (index < this.m_ArrShowBuffSkill.Length)))
                {
                    ...
                    if (this.m_selectBuff != 0)
                    {
                        ...
                        this.ShowBuffSkillDesc(iCfgID, uiEvent.m_srcWidget.get_transform().get_localPosition());
                    }
                }
            }
        }
        ...
        private void ShowBuffSkillDesc(uint uiBuffId, Vector3 BtnPos)
        {
            ...
            if (((dataByKey != null) && !string.IsNullOrEmpty(dataByKey.szSkillCombineDesc)) && (this.m_BuffSkillPanel != null))
            {
                ...
                Vector2 vector = this.m_textBgImage.get_rectTransform().get_sizeDelta();
                num += ((this.m_textBgImage.get_gameObject().get_transform().get_localPosition().y - component.get_gameObject().get_transform().get_localPosition().y) * 2f) + 10f;
                this.m_textBgImage.get_rectTransform().set_sizeDelta(new Vector2(vector.x, num));
                Vector3 vector2 = BtnPos;
                RectTransform transform = this.m_BuffBtn0.GetComponent<RectTransform>();
                vector2.y += (((this.m_BuffBtn0.get_transform().get_localScale().y * transform.get_rect().get_height()) / 2f) + (num / 2f)) + 15f;
                this.m_BuffDescNodeObj.get_transform().set_localPosition(vector2);
            }
        }
                    ...
                    if (i < this.m_CurShowBuffCount)
                    {
                        if (((this.m_ArrShowBuffSkill[i].stBuffSkill != 0) && (this.m_ArrShowBuffSkill[i].stBuffSkill.handle.cfgData != null)) && !string.IsNullOrEmpty(this.m_ArrShowBuffSkill[i].stBuffSkill.handle.cfgData.szIconPath))
                        {
                            ...
                            if (obj4 != null)
                            {
                                ...
                                {
                                    text.set_text(this.m_ArrShowBuffSkill[i].stBuffSkill.handle.GetOverlayCount().ToString());
