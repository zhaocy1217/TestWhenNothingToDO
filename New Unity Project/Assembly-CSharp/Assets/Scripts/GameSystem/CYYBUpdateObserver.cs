namespace Assets.Scripts.GameSystem
{
    ...
    public class CYYBUpdateObserver : MonoBehaviour
    {
        ...
        public CYYBUpdateObserver()
        {
            ...
            {
                <>f__am$cache4 = new CYYBUpdateManager.OnYYBCheckNeedUpdateInfo(CYYBUpdateObserver.<m_onYYBCheckNeedUpdateInfoHandler>m__82);
            }
            this.m_onYYBCheckNeedUpdateInfoHandler = <>f__am$cache4;
            if (<>f__am$cache5 == null)
            {
                <>f__am$cache5 = new CYYBUpdateManager.OnDownloadYYBProgressChanged(CYYBUpdateObserver.<m_onDownloadYYBProgressChangedHandler>m__83);
            }
            this.m_onDownloadYYBProgressChangedHandler = <>f__am$cache5;
            if (<>f__am$cache6 == null)
            {
                <>f__am$cache6 = new CYYBUpdateManager.OnDownloadYYBStateChanged(CYYBUpdateObserver.<m_onDownloadYYBStateChangedHandler>m__84);
            }
            this.m_onDownloadYYBStateChangedHandler = <>f__am$cache6;
        }

        [CompilerGenerated]
        private static void <m_onDownloadYYBProgressChangedHandler>m__83(string)
        {
        }

        [CompilerGenerated]
        private static void <m_onDownloadYYBStateChangedHandler>m__84(string)
        {
        }

        [CompilerGenerated]
        private static void <m_onYYBCheckNeedUpdateInfoHandler>m__82(string)
        {
        }
