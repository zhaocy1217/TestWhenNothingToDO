        ...
        public void SetEuipItems(ref COMDT_PLAYER_FIGHT_DATA playerData, CUIFormScript form)
        {
            if ((playerData != null) && (form != null))
            {
                for (int i = 0; i < 6; i++)
                {
                    ...
                    int num2 = i + 1;
                    Image component = this.equipObj.get_transform().FindChild(string.Format("TianFuIcon{0}", num2.ToString())).GetComponent<Image>();
                    ...
                    else
                    {
                        ...
                        CUICommonSystem.SetEquipIcon((ushort) comdt_ingame_equip_info.dwEquipID, component.get_gameObject(), form);
                    }
                }
            }
        }
