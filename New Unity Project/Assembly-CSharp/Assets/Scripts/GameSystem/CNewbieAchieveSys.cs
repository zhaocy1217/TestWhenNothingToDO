    ...
    internal class CNewbieAchieveSys : Singleton<CNewbieAchieveSys>
    {
        ...
        public void ShowAchieve(enNewbieAchieve achieveType)
        {
            if (!MonoSingleton<Reconnection>.instance.isProcessingRelayRecover && !Singleton<WatchController>.GetInstance().IsWatching)
            {
                ...
                script.get_gameObject().get_transform().Find("Bg/TxtTitle").get_gameObject().GetComponent<Text>().set_text(Singleton<CTextManager>.GetInstance().GetText(string.Format("{0}Title", achieveType.ToString())));
                script.get_gameObject().get_transform().Find("Bg/Txt").get_gameObject().GetComponent<Text>().set_text(Singleton<CTextManager>.GetInstance().GetText(achieveType.ToString()));
            }
        }
