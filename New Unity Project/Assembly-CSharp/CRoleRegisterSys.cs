...
public class CRoleRegisterSys : Singleton<CRoleRegisterSys>
{
    ...
    public string GetRandomName()
    {
        ...
        while (ec == null)
        {
            num2 = (uint) this.m_random.Next(0, GameDataMgr.robotSubNameC.count);
            ec = GameDataMgr.robotSubNameC.GetDataByKey((uint) (num2 + 1));
        }
        while (ea == null)
        {
            num3 = (uint) this.m_random.Next(0, GameDataMgr.robotSubNameA.count);
            ea = GameDataMgr.robotSubNameA.GetDataByKey((uint) (num3 + 1));
        }
        while (eb == null)
        {
            num4 = (uint) this.m_random.Next(0, GameDataMgr.robotSubNameB.count);
            eb = GameDataMgr.robotSubNameB.GetDataByKey((uint) (num4 + 1));
        }
        object[] args = new object[] { Utility.UTF8Convert(dataByKey.szName), Utility.UTF8Convert(ec.szName), Utility.UTF8Convert(ea.szName), Utility.UTF8Convert(eb.szName) };
    ...
    public void OnGameDifConfirm(CUIEvent cuiEvent)
    {
        ...
        LobbyLogic.ReqStartGuideLevel11(false, 0);
    }
    ...
    public void OnHeroTypeConfirm(CUIEvent uiEvent)
    {
        ...
        {
            CSPkg msg = NetworkModule.CreateDefaultCSPKG(0x147d);
            msg.stPkgData.stSelectNewbieHeroReq.iHeroType = (int) uiEvent.m_eventParams.tagUInt;
            if (Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref msg, false))
            {
    ...
    public void SetRoleCreateFormVisible(bool bVisible)
    {
        ...
        if (form != null)
        {
            form.get_gameObject().CustomSetActive(bVisible);
        }
    }
