...
public class CTongCaiSys : MonoSingleton<CTongCaiSys>
{
    ...
    {
        <CheckCanUseTongCai>c__Iterator30 iterator = new <CheckCanUseTongCai>c__Iterator30();
        iterator.info = info;
        iterator.<$>info = info;
    ...
    public string GetCheckUrl(ApolloAccountInfo info)
    {
        if (info.OpenId.Equals("$OPID$"))
        {
            ...
            events.Add(new KeyValuePair<string, string>("openid", info.OpenId));
            string eventName = "OpenIdIsError";
            eventName = eventName + "_ANDROID";
            if (info.Platform == ApolloPlatform.Wechat)
            {
                eventName = eventName + "_Wechat";
            }
            else if (info.Platform == ApolloPlatform.QQ)
            {
                eventName = eventName + "_QQ";
            }
        ...
        object[] args = new object[] { info.OpenId, (int) info.Platform, str2, str4 };
    ...
    public bool IsCanUseTongCai()
    {
        ...
        {
            return ((this.isCanUseTongCai && this.isTongCaiValid) && (Application.get_internetReachability() == 1));
        }
        return ((ApolloConfig.loginOnlyVPort >= 0xea60) && ((this.isCanUseTongCai && this.isTongCaiValid) && (Application.get_internetReachability() == 1)));
    }
    ...
    public void OpenTongCaiH5(ApolloAccountInfo info)
    {
        if ((info != null) && !string.IsNullOrEmpty(info.OpenId))
        {
            ...
            CUICommonSystem.OpenUrl(string.Format(this.ScanH5UrlFormat, args), true, 0);
        }
    }
    ...
    [CompilerGenerated]
    private sealed class <CheckCanUseTongCai>c__Iterator30 : IDisposable, IEnumerator, IEnumerator<object>
    {
        ...
        public bool MoveNext()
        {
            ...
            if (!this.<www>__3.get_isDone())
            {
                if (this.<timer>__0 <= this.<>f__this.timeOut)
                {
                    this.<timer>__0 += Time.get_deltaTime();
                    this.$current = null;
                    this.$PC = 1;
                    return true;
                }
            ...
            else if (string.IsNullOrEmpty(this.<www>__3.get_error()))
            {
                this.<dataString>__4 = this.<www>__3.get_text();
                this.<>f__this.CheckDataString(this.<dataString>__4);
                this.<>f__this.isCanUseTongCai = true;
            }
            this.<>f__this.isChecked = true;
        Label_013D:
            Debug.Log("*Login* tdir " + Time.get_realtimeSinceStartup());
