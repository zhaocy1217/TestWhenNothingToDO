...
public class InBattleMsgInputView
{
    ...
    private void CalcLineWidth(out int lineWidth, out int fontSize)
    {
        ...
        else
        {
            ...
            {
                lineWidth = (int) transform.get_sizeDelta().x;
                Text component = obj2.get_transform().Find("Text").GetComponent<Text>();
                if (component == null)
                {
                    lineWidth = 200;
                    fontSize = 0x12;
    ...
    private void GetCom(int index, out Text txtCom)
    {
        ...
        if (index < InBattleInputChat.CHAT_LINE_COUNT)
        {
            ...
            if (obj2 != null)
            {
                GameObject obj3 = obj2.get_transform().Find("Text").get_gameObject();
                DebugHelper.Assert(obj3 != null, string.Format("---InBattleMsgInputView.GetCom txtObj != null, index:{0}", index));
                txtCom = obj3.GetComponent<Text>();
            }
        }
    }
    ...
    public void Init(CUIFormScript battleFormScript)
    {
        if (battleFormScript != null)
        {
            int num2;
            int num3;
            this.m_lineNode = Utility.FindChild(battleFormScript.get_gameObject(), "InputChat_Lines");
            CUIFormScript form = Singleton<CUIManager>.instance.GetForm(InBattleShortcut.InBattleMsgView_FORM_PATH);
            this.m_campTxt = Utility.FindChild(form.get_gameObject(), "chatTools/node/InputChat_Buttons/Button_Camp/Text").GetComponent<Text>();
            this.m_buttonNode = Utility.FindChild(form.get_gameObject(), "chatTools/node/InputChat_Buttons");
            this.m_inputField = Utility.FindChild(form.get_gameObject(), "chatTools/node/InputChat_Buttons/Button_Input/InputField").GetComponent<InputField>();
            this.m_inputText = Utility.FindChild(this.m_inputField.get_gameObject(), "Text");
            GameObject button = Utility.FindChild(form.get_gameObject(), "chatTools/node/InputChat_Buttons/Button_Input/CDButton");
            if (button != null)
            {
            ...
            {
                this.m_cacheNode[i] = Utility.FindChild(battleFormScript.get_gameObject(), string.Format("InputChat_Lines/line{0}", i));
            ...
            if (this.m_inputField != null)
            {
                this.m_inputField.get_onEndEdit().AddListener(new UnityAction<string>(this, (IntPtr) this.On_EndEdit));
            }
        ...
        if (this.intimeMSecond_StartTime != 0)
        {
            ...
            if (num < this.intimeMSecond)
            {
                if ((this.intimeSendCount + 1) > this.intimeMaxCount)
                {
                    if (this.m_inputButton != null)
                    {
                        this.m_inputButton.StartCooldown(this.intime_cd, new Action(this, (IntPtr) this.OnBlockCDEnd));
                    }
    ...
    public void Refresh(ListView<InBattleInputChat.InBatChatEntity> chatEntityList)
    {
        if (chatEntityList != null)
        {
            ...
            for (int i = 0; (i < chatEntityList.Count) && (index < InBattleInputChat.CHAT_LINE_COUNT); i++)
            {
                ...
                if (entity != null)
                {
                    if (entity.type != InBattleInputChat.InBatChatEntity.EType.ParseAllColor)
                    {
    ...
    public void SetChatButtonNodeCheckable(bool bEnable)
    {
        if (this.m_buttonNode != null)
        {
            GameObject obj2 = this.m_buttonNode.get_transform().Find("Button_Camp").get_gameObject();
            GameObject obj3 = this.m_buttonNode.get_transform().Find("Button_Input").get_gameObject();
            DebugHelper.Assert(obj2 != null, "---InBattleMsgInputView.SetChatButtonNodeCheckable campObj  != null");
            DebugHelper.Assert(obj3 != null, "---InBattleMsgInputView.SetChatButtonNodeCheckable inputObj != null");
            if (obj2 != null)
            {
                obj2.GetComponent<CUIEventScript>().set_enabled(bEnable);
            }
            if (obj3 != null)
            {
                obj3.GetComponent<CUIEventScript>().set_enabled(bEnable);
            }
        }
    }
    ...
    public void SetChatButtonNodeShowable(bool bShow)
    {
        if (this.m_buttonNode != null)
        {
            this.m_buttonNode.get_gameObject().CustomSetActive(bShow);
        }
    }
    ...
    public void SetChatLineNodeShowable(bool bShow)
    {
        if (this.m_lineNode != null)
        {
            this.m_lineNode.get_gameObject().CustomSetActive(bShow);
        }
    }
