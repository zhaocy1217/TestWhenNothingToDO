...
public class SceneManagement : Singleton<SceneManagement>
{
    ...
    public void GetCoord(ref Coordinate coord, VInt2 origin, VInt2 size)
    {
        ...
        coord.NumX = Mathf.Min((((origin.x - (coord.X * 0x2710)) + size.x) / 0x2710) + 1, this.TileCountX - coord.X);
        coord.NumY = Mathf.Min((((origin.y - (coord.Y * 0x2710)) + size.y) / 0x2710) + 1, this.TileCountY - coord.Y);
    }
    ...
    public void GetCoord_Center(ref Coordinate coord, VInt2 origin, int radius)
    {
        ...
        coord.X = Mathf.Clamp(origin.x / 0x2710, 0, this.TileIndexMax_X);
        coord.Y = Mathf.Clamp(origin.y / 0x2710, 0, this.TileIndexMax_Y);
        coord.NumX = Mathf.Min((((origin.x - (coord.X * 0x2710)) + num) / 0x2710) + 1, this.TileCountX - coord.X);
        coord.NumY = Mathf.Min((((origin.y - (coord.Y * 0x2710)) + num) / 0x2710) + 1, this.TileCountY - coord.Y);
    }
    ...
    public void InitScene()
    {
        ...
        if (((active != null) && (active.astarData != null)) && (active.astarData.recastGraph != null))
        {
            ...
            VInt3 num = forcedBounds.get_size();
            this.TileCountX = (num.x / 0x2710) + (((num.x % 0x2710) <= 0) ? 0 : 1);
            this.TileCountY = (num.z / 0x2710) + (((num.z % 0x2710) <= 0) ? 0 : 1);
            this.TileIndexMax_X = this.TileCountX - 1;
            this.TileIndexMax_Y = this.TileCountY - 1;
            VInt3 num3 = forcedBounds.get_min();
            this.TileOrigin = num3.xz;
    ...
    public class Node : PooledClassObject
    {
        ...
        public bool attached
        {
            ...
            get
            {
                return this.<attached>k__BackingField;
            }
            [CompilerGenerated]
            private set
            {
                this.<attached>k__BackingField = value;
            }
        }
