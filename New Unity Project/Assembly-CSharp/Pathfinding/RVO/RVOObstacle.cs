namespace Pathfinding.RVO
{
    ...
    public abstract class RVOObstacle : MonoBehaviour
    {
        ...
        protected virtual Matrix4x4 GetMatrix()
        {
            ...
            {
                return base.get_transform().get_localToWorldMatrix();
            }
        ...
        public void OnDrawGizmos(bool selected)
        {
            ...
            Gizmos.set_color(new Color(0.615f, 1f, 0.06f, !selected ? 0.7f : 1f));
            ...
            for (int i = 0; i < this.gizmoVerts.Count; i++)
            {
                ...
                if (selected)
                {
                    ...
                    {
                        Gizmos.DrawLine(matrix.MultiplyPoint3x4((Vector3) numArray[num5]) + ((Vector3) (Vector3.get_up() * num4)), matrix.MultiplyPoint3x4((Vector3) numArray[k]) + ((Vector3) (Vector3.get_up() * num4)));
                        Gizmos.DrawLine(matrix.MultiplyPoint3x4((Vector3) numArray[num5]), matrix.MultiplyPoint3x4((Vector3) numArray[num5]) + ((Vector3) (Vector3.get_up() * num4)));
                    }
                    int num7 = 0;
                    for (int m = numArray.Length - 1; num7 < numArray.Length; m = num7++)
                    {
                        ...
                        Vector3 vector4 = (vector2 - vector).get_normalized();
                        if (vector4 != Vector3.get_zero())
                        {
                            Vector3 vector5 = Vector3.Cross(Vector3.get_up(), vector4);
