namespace Pathfinding.RVO
{
    ...
    {
        [CompilerGenerated]
        private Agent <rvoAgent>k__BackingField;
        ...
        public void DoUpdate(float dt)
        {
            if (this.rvoAgent != null)
            {
                ...
                if (this.enableRotation && (this.velocity != VInt3.zero))
                {
                    ...
                    transform.set_rotation(Quaternion.Lerp(transform.get_rotation(), Quaternion.LookRotation(velocity), (dt * this.rotationSpeed) * Mathf.Min((float) this.velocity.magnitude, 0.2f)));
                }
            }
        }
        ...
        public void EnsureActorAndSimulator()
        {
            if (this.actor == 0)
            {
                this.actor = ActorHelper.GetActorRoot(base.get_gameObject());
            }
            ...
            if (this.simulator != null)
            {
                ...
                {
                    location = base.get_transform().get_position();
                }
                if (this.rvoAgent != null)
                {
                ...
                if (this.rvoAgent != null)
                {
                    this.rvoAgent.owner = base.get_gameObject();
                }
                this.UpdateAgentProperties();
        ...
        {
            if (!base.get_enabled() || (this.rvoAgent == null))
            {
                return null;
            }
        ...
        public void OnDrawGizmos()
        {
            ...
            Gizmos.DrawWireSphere((Vector3) (((base.get_transform().get_position() + center) - ((Vector3.get_up() * height) * 0.5f)) + ((Vector3.get_up() * radius) * 0.5f)), radius);
            Gizmos.DrawLine((base.get_transform().get_position() + center) - ((Vector3) ((Vector3.get_up() * height) * 0.5f)), (base.get_transform().get_position() + center) + ((Vector3) ((Vector3.get_up() * height) * 0.5f)));
            Gizmos.DrawWireSphere((Vector3) (((base.get_transform().get_position() + center) + ((Vector3.get_up() * height) * 0.5f)) - ((Vector3.get_up() * radius) * 0.5f)), radius);
        }
        ...
        public void Update()
        {
            ...
            {
                this.DoUpdate(Time.get_deltaTime());
            }
        }
        ...
        public Agent rvoAgent
        {
            ...
            get
            {
                return this.<rvoAgent>k__BackingField;
            }
            [CompilerGenerated]
            private set
            {
                this.<rvoAgent>k__BackingField = value;
            }
        }
