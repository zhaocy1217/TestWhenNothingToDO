...
public class CMallItem
{
    ...
    public bool TagInfo(ref string iconPath, ref string text, bool owned = false)
    {
        ...
        RES_LUCKYDRAW_ITEMTAG res_luckydraw_itemtag = this.TagType(ref heroPromotion, ref skinPromotion);
        string registerSalesHeroDay = null;
        if (this.m_type == ItemType.Hero)
        {
            ResHeroShop shop = null;
            GameDataMgr.heroShopInfoDict.TryGetValue(this.m_heroData.heroCfgInfo.dwCfgID, out shop);
            registerSalesHeroDay = CMallSystem.GetRegisterSalesHeroDay(ref heroPromotion, shop);
        }
        else if (this.m_type == ItemType.Skin)
        {
            ResHeroSkinShop shop2 = null;
            GameDataMgr.skinShopInfoDict.TryGetValue(this.m_skinData.dwID, out shop2);
            registerSalesHeroDay = CMallSystem.GetRegisterSalesSkinDay(ref skinPromotion, shop2);
        }
        if (registerSalesHeroDay != null)
        {
            iconPath = "UGUI/Sprite/Common/Product_Unusual.prefab";
            text = registerSalesHeroDay;
            return true;
        }
        switch (res_luckydraw_itemtag)
        {
            ...
            {
                if ((this.m_type != ItemType.Hero) || (this.m_heroData.heroCfgInfo.bTag != 1))
                {
                    break;
                }
                CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
                if (masterRoleInfo != null)
                {
                    if (GameDataMgr.svr2CltCfgDict.ContainsKey(0x18))
                    {
                        ResGlobalInfo info2 = new ResGlobalInfo();
                        if (GameDataMgr.svr2CltCfgDict.TryGetValue(0x18, out info2))
                        {
                            uint dwConfValue = info2.dwConfValue;
            ...
            case RES_LUCKYDRAW_ITEMTAG.RES_LUCKYDRAW_ITEMTAG_UNUSUAL:
            {
                ...
                switch (this.m_type)
                {
                    case ItemType.Hero:
                        ...
                        {
                            num3 = (int) (heroPromotion.dwOffTimeGen - heroPromotion.dwOnTimeGen);
                        }
                        else
                        {
                            num3 = (int) (heroPromotion.dwOffTimeGen - currentUTCTime);
                        }
                        goto Label_0323;
                    ...
                    case ItemType.Skin:
                        ...
                        {
                            num3 = (int) (skinPromotion.dwOffTimeGen - skinPromotion.dwOnTimeGen);
                        }
                        else
                        {
                            num3 = (int) (skinPromotion.dwOffTimeGen - currentUTCTime);
                        }
                        goto Label_0323;
                }
                goto Label_0323;
            ...
            case RES_LUCKYDRAW_ITEMTAG.RES_LUCKYDRAW_ITEMTAG_DISCOUNT:
                ...
                switch (this.m_type)
                {
                    case ItemType.Hero:
                        ...
                        num2 = ((float) heroPromotion.dwDiscount) / 10f;
                        goto Label_01F1;
                    ...
                    case ItemType.Skin:
                        ...
                        num2 = ((float) skinPromotion.dwDiscount) / 10f;
                        goto Label_01F1;
    ...
    Label_0323:
        if (num3 > 0)
        {
            int num5 = (int) Math.Ceiling((double) (((double) num3) / 86400.0));
            if (num5 > 0)
            {
                iconPath = "UGUI/Sprite/Common/Product_Unusual.prefab";
