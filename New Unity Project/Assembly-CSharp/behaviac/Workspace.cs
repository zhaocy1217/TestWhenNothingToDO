namespace behaviac
{
    ...
    public static class Workspace
    {
        ...
        {
            <IsRegisterd>c__AnonStorey97 storey = new <IsRegisterd>c__AnonStorey97();
            storey.type = type;
            return (ms_agentTypes.FindIndex(new Predicate<TypeInfo_t>(storey.<>m__AE)) != -1);
        }
            ...
            switch (fileFormat)
            {
                ...
                case EFileFormat.EFF_cs:
                    ...
                    try
                    {
                        ...
                        {
                            Type type = Utils.GetType("behaviac.bt_" + relativePath.Replace("/", "_"));
                            if (type != null)
                            {
        ...
        public static void RegisterBehaviorNode(Assembly a)
        {
            ...
            {
                ms_behaviorNodeTypes = new DictionaryView<string, Type>();
            }
        ...
        {
            ListView<Type> view = new ListView<Type>();
            foreach (Type type in a.GetTypes())
            {
            ...
            <RegisterMetas>c__AnonStorey98 storey = new <RegisterMetas>c__AnonStorey98();
            using (ListView<Type>.Enumerator enumerator = view.GetEnumerator())
            {
                while (enumerator.MoveNext())
                {
                    storey.type = enumerator.Current;
                    if (!IsRegisterd(storey.type) && (ms_agentTypes.Find(new Predicate<TypeInfo_t>(storey.<>m__AF)) == null))
                    {
                        TypeInfo_t _t3 = new TypeInfo_t();
                        _t3.type = storey.type;
                        _t3.bIsInherited = true;
            ...
            if (!bIsAgentType || (customAttributes.Length > 0))
            {
                ...
                if (!Utils.IsEnumType(type))
                {
                    ...
                    if (bIsAgentType)
                    {
                        ...
                        foreach (Type type2 in type.GetNestedTypes(bindingAttr))
                        {
        ...
        [CompilerGenerated]
        private sealed class <IsRegisterd>c__AnonStorey97
        {
            internal Type type;

            internal bool <>m__AE(Workspace.TypeInfo_t t)
            {
        ...
        [CompilerGenerated]
        private sealed class <RegisterMetas>c__AnonStorey98
        {
            internal Type type;

            internal bool <>m__AF(Workspace.TypeInfo_t t)
            {
                return (t.type == this.type);
            }
        }
