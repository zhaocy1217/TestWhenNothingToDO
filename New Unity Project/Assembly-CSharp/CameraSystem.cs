...
public class CameraSystem : MonoSingleton<CameraSystem>
{
    ...
    {
        if (((prm.src != 0) && ActorHelper.IsHostCtrlActor(ref prm.src)) && !Singleton<WatchController>.GetInstance().IsWatching)
        {
    ...
    public void UpdateCameraMovement(ref Vector2 axis)
    {
        if (this.bEnableCameraMovment)
        {
            ...
            float num2 = (MonoSingleton<GlobalConfig>.instance.CameraMoveSpeedMax - this.CurrentSpeed) / MonoSingleton<GlobalConfig>.instance.CameraMoveAcceleration;
            float num3 = Mathf.Min(num2, num);
            this.CurrentSpeed += MonoSingleton<GlobalConfig>.instance.CameraMoveAcceleration * num3;
            Vector2 inOffset = (Vector2) (((axis.get_normalized() * this.CurrentSpeed) * num) + ((((axis.get_normalized() * 0.5f) * MonoSingleton<GlobalConfig>.instance.CameraMoveAcceleration) * num3) * num3));
