...
public class ApolloHelper : Singleton<ApolloHelper>
{
    ...
    public ApolloHelper()
    {
        ...
        if (File.Exists(Application.get_persistentDataPath() + "/customOpenId.txt"))
        {
    ...
    public void CustomLog(string str)
    {
        try
        {
            ...
            object[] objArray1 = new object[] { DateTime.Now.ToString("yyyyMMdd_HHmmss ") + str };
            class2.CallStatic("dtLog", objArray1);
            class2.Dispose();
        }
        catch (Exception)
        {
    ...
    public void Login(ApolloPlatform platform, ulong uin = 0, string pwd = new string())
    {
        switch (platform)
        {
    ...
    {
        Debug.Log(string.Concat(new object[] { "*Login*OnLoginEvent called ", loginResult, " ", Time.get_realtimeSinceStartup() }));
        this.m_IsSwitchToLoginPlatform = false;
    ...
    private void OnPaySuccess(ApolloBufferBase info)
    {
        ...
        else
        {
            ...
            Singleton<BeaconHelper>.GetInstance().m_curBuyDianInfo.call_back_time = Time.get_time() - Singleton<BeaconHelper>.GetInstance().m_curBuyDianInfo.call_back_time;
    ...
    {
        Debug.Log(string.Concat(new object[] { "*Login*OnRefreshAccess called ", result, " ", Time.get_realtimeSinceStartup() }));
        if (result == ApolloResult.Success)
        {
            ApolloAccountInfo accountInfo = this.GetAccountInfo(false);
    ...
    public void ShareRedBox(int shareType, string url, string title, string desc)
    {
        if (MonoSingleton<ShareSys>.GetInstance().IsInstallPlatform())
        {
            byte[] imageData = (Resources.Load("Share/redbox", typeof(Texture2D)) as Texture2D).EncodeToPNG();
            int imageDataLen = 0;
            if (imageData != null)
            {
            ...
            {
                string str = url;
                string thumbImageUrl = "http://image.smoba.qq.com/yywj/share_pic/redbox.png";
                service.SendToQQ((ApolloShareScene) shareType, title, desc, str, thumbImageUrl);
            }
            else if (Singleton<ApolloHelper>.GetInstance().CurPlatform == ApolloPlatform.Wechat)
            {
                service.SendToWeixinWithUrl((ApolloShareScene) shareType, title, desc, url, "MSG_INVITE", imageData, imageDataLen, "message Ext");
            }
        }
    }
    ...
    public void ShowIOSGuestNotice()
    {
        ...
        item.MsgType = APOLLO_NOTICETYPE.APO_NOTICETYPE_ALERT;
        item.ContentType = APOLLO_NOTICE_CONTENTTYPE.APO_SCONTENTTYPE_TEXT;
        item.MsgTitle = "游客模式特别说明";
        item.MsgContent = "敬爱的玩家，您正在使用游客模式进行游戏，游客模式下的游戏数据（包含付费数据）会在删除游戏、更换设备后清空。为了保障您的虚拟财产安全，以及让您获得更完善的游戏体验，我们建议您使用QQ/微信登录进行游戏！\n\n《王者荣耀》运营团队";
