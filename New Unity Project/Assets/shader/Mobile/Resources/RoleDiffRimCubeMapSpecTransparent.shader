...
Shader "GOE/Role/RoleDiffRimCubeMapSpecTransparent" {
	Properties
	{
		...
		[HideInInspector]
		_CrossfadeAuto("Crossfade(x:from, y:to, z:beginTime, w:timeLong)", vector) = (0, 0, 0, 1)
	}
	SubShader
	{
		...
		Pass
		{
			...
			#pragma multi_compile CROSSFADE_ON CROSSFADE_OFF
			...
			fixed4 frag(v2f i) : COLOR
			{
				...
				fixed3 reflectDirection = reflect(-viewDirection, normalDirection);
				...
				tex.rgb *= _Color.rgb;
				fixed3 emissive = lerp(tex.rgb* _EmissionIntensity, texCUBE(_CubeMap, reflectDirection).rgb*_CubeMapIntensity, tex.a);
