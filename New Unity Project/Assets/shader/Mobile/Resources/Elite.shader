Shader "GOE/Other/Elite" {
	Properties {
		_Color ("Main Color", Color) = (1,1,1,1)
		//_EliteColor("Elite", Color) = (1,1,1,1)
	...
	Pass 
	{
		...
		CGPROGRAM
			...
			float4 frag(v2f i) : COLOR
			{
				...
				_EliteColor = fixed4(0.376, 0, 0, 1);
				float alpha = sin( 2 * _Time.w) * 0.5 + 0.5;
				fixed4 lastColor =  _Color * alpha * _EliteColor;

				diff.rgb = diff.rgb + lastColor.rgb;
				//LightFaceAlpha(diff.rgba, 0.15);
