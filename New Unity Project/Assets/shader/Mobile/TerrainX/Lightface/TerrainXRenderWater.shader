...
Shader "GOE/Scene/TerrainXRenderWater" 
{
Properties 
{
	...
	_Layer2BumpIntensity("Layer2 BumpIntensity", Range(0,1)) = 1
	...
	Pass 
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f i) : COLOR
			{
				...
				float diff = saturate(dot(normalDirection, pointLitDir));
				diff = pow(diff, _DiffGloss);

				float3 diffColor = diff * _MyDiffColor* splat_control.b;
